...
1. Go into a folder with your Terraform configurations (`.tf` files) and create a `terraform.tfvars` file with a
   ...
   section for the types of configuration Terragrunt supports. For example, the following configuration tells 
   Terragrunt to always set the `lock-timeout` flag when calling `terraform`:
...
1. [Add custom locking with DynamoDB](#custom-locking-with-dynamodb)
...
### Add custom locking with DynamoDB

#### Motivation

Before version 0.9, Terraform did not support locking for remote state, so Terragrunt included a locking feature that
used Amazon's [DynamoDB](https://aws.amazon.com/dynamodb/) under the hood. As of Terraform 0.9, locking with DynamoDB
is built into the [S3 backend](https://www.terraform.io/docs/backends/types/s3.html), **so you no longer need to rely 
on Terragrunt for this feature**.

However, for backwards compatibility, and for [backend types](https://www.terraform.io/docs/backends/types/index.html)
that don't support locking, you can still use Terragrunt's built-in support for locking with DynamoDB. DynamoDB supports
...
1. Set your AWS credentials in the environment using one of the following options:
    1. Set your credentials as the environment variables `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` (and also 
       `AWS_SESSION_TOKEN` if using [STS temporary 
...
To enable DynamoDB locking, add the following settings in `terraform.tfvars`:
