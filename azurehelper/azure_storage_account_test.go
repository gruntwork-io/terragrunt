...
// TestCompareStringMaps tests the helper function for comparing tag maps
func TestCompareStringMaps(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:     "Nil existing, empty desired",
			existing: nil,
			desired:  map[string]string{},
			expected: true,
		},
		{
			name:     "Empty existing, nil desired",
			existing: map[string]*string{},
			desired:  nil,
			expected: true,
		},
...
// TestConvertToPointerMap tests the helper function for converting string maps to pointer maps
...
// TestCompareAccessTier tests the helper function for comparing access tiers
