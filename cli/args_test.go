...
func TestParseTerragruntOptionsFromArgs(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			[]string{"--terragrunt-sts-duration", "36000"},
			mockOptionsWithStsDuration(t, util.JoinPath(workingDir, config.DefaultTerragruntConfigPath), workingDir, []string{}, false, "", false, 36000),
			nil,
		},
...
func mockOptionsWithStsDuration(t *testing.T, terragruntConfigPath string, workingDir string, terraformCliArgs []string, nonInteractive bool, terragruntSource string, ignoreDependencyErrors bool, stsDuration int64) *options.TerragruntOptions {
	opts := mockOptions(t, terragruntConfigPath, workingDir, terraformCliArgs, nonInteractive, terragruntSource, ignoreDependencyErrors, false, defaultLogLevel, false)
	opts.StsDuration = stsDuration
