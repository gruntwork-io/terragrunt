...
func TestFrontmatter(t *testing.T) {
	...
	for i, testCase := range testCases {
		...
		t.Run(fmt.Sprintf("testCase-%d", i), func(t *testing.T) {
			...
			assert.Equal(t, testCase.expectedName, doc.FrontmatterName(), "Frontmatter Name")
			assert.Equal(t, testCase.expectedDesc, doc.FrontmatterDescription(), "Frontmatter Description")
		})
	}
...
func TestH1(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			200,
			"Amazon ECS Cluster",
			"This service contains Terraform code to deploy a production-grade ECS cluster on AWS using Elastic Container Service (ECS). This service launches an ECS cluster on top of an Auto Scaling Group that you manage. If you wish to launch an ECS cluster on top of Fargate that is completely managed by AWS, refer to the ecs-fargate-cluster module. Refer to the section EC2 vs Fargate Launch Types for more information on the differences between the two flavors.",
		},
		// {
	...
	for i, testCase := range testCases {
		...
		t.Run(fmt.Sprintf("testCase-%d", i), func(t *testing.T) {
			...
			assert.Equal(t, testCase.expectedTitle, doc.Name(), "Name")
