...
func Run(ctx context.Context, l log.Logger, opts *options.TerragruntOptions, moduleURL, templateURL string) error {
	...
	boilerplateDir, err := prepareBoilerplateFiles(ctx, l, opts, templateURL, tempDir, requiredVariables, optionalVariables)
	if err != nil {
		return errors.New(err)
	}
...
func generateDefaultTemplate(boilerplateDir string, requiredVariables, optionalVariables []*config.ParsedVariable) (string, error) {
	...
	// Render the boilerplate config template with the required and optional variables
	tmpl, err := template.New("boilerplate").Parse(DefaultBoilerplateConfig)
	if err != nil {
		return "", errors.New(err)
	}
	...
	if err := os.WriteFile(util.JoinPath(boilerplateDir, "boilerplate.yml"), buf.Bytes(), ownerWriteGlobalReadPerms); err != nil {
		return "", errors.New(err)
	}
...
func prepareBoilerplateFiles(ctx context.Context, l log.Logger, opts *options.TerragruntOptions, templateURL string, tempDir string, requiredVariables, optionalVariables []*config.ParsedVariable) (string, error) {
	...
	if !files.IsExistingDir(boilerplateDir) {
		...
		if config != nil && config.DefaultTemplate != "" {
			...
			boilerplateDir, err = generateDefaultTemplate(boilerplateDir, requiredVariables, optionalVariables)
			if err != nil {
				return "", errors.New(err)
			}
		}
