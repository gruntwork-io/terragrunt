...
const (
	// TerragruntGeneratedSignature is a comment that is added to the top of
	// generated files to indicate that they were generated by Terragrunt.
	// We use a hardcoded random string at the end to ensure the uniqueness of the signature.
	TerragruntGeneratedSignature = "Generated by Terragrunt. Sig: nIlQXj57tbuaRZEa"

	// DefaultCommentPrefix is the default prefix to use for comments in the generated file.
...
const (
	// ExistsError indicates that an error should be returned if the file already exists.
	ExistsError GenerateConfigExists = iota
	// ExistsSkip indicates that the file should not be generated if it already exists.
	ExistsSkip
	// ExistsOverwrite indicates that the file should be overwritten if it already exists.
	ExistsOverwrite
	// ExistsOverwriteTerragrunt indicates that the file should be overwritten if it already exists and was generated by
	// Terragrunt.
	ExistsOverwriteTerragrunt
	// ExistsUnknown indicates that the value of if_exists is unknown.
	ExistsUnknown
)
...
const (
	// DisabledSkip indicates that the file should not be generated if it is disabled.
	DisabledSkip GenerateConfigDisabled = iota
	// DisabledRemove indicates that the file should be removed if it is disabled.
	DisabledRemove
	// DisabledRemoveTerragrunt indicates that the file should be
	// removed if it is disabled and was generated by Terragrunt.
	DisabledRemoveTerragrunt
	// DisabledUnknown indicates that the value of if_disabled is unknown.
	DisabledUnknown
)

const (
	// ExistsErrorStr is the string representation of ExistsError.
	ExistsErrorStr = "error"
	// ExistsSkipStr is the string representation of ExistsSkip.
	ExistsSkipStr = "skip"
	// ExistsOverwriteStr is the string representation of ExistsOverwrite.
	ExistsOverwriteStr = "overwrite"
	// ExistsOverwriteTerragruntStr is the string representation of ExistsOverwriteTerragrunt.
	ExistsOverwriteTerragruntStr = "overwrite_terragrunt"

	// DisabledSkipStr is the string representation of DisabledSkip.
	DisabledSkipStr = "skip"
	// DisabledRemoveStr is the string representation of DisabledRemove.
	DisabledRemoveStr = "remove"
	// DisabledRemoveTerragruntStr is the string representation of DisabledRemoveTerragrunt.
	DisabledRemoveTerragruntStr = "remove_terragrunt"

	assumeRoleConfigKey = "assume_role"
)

// GenerateConfig is the configuration for generating code.
...
func WriteToFile(terragruntOptions *options.TerragruntOptions, basePath string, config GenerateConfig) error {
		...
		if targetFileExists {
			if shouldRemove, err := shouldRemoveWithFileExists(terragruntOptions, targetPath, config.IfDisabled); err != nil {
				...
				if err := os.Remove(targetPath); err != nil {
					return fmt.Errorf("failed to remove file %s: %w", targetPath, errors.WithStackTrace(err))
				}
			}
		}
	...
	if err := os.WriteFile(targetPath, []byte(contentsToWrite), ownerWriteGlobalReadPerms); err != nil {
		return fmt.Errorf("failed to write file %s: %w", targetPath, errors.WithStackTrace(err))
	}
...
func shouldContinueWithFileExists(terragruntOptions *options.TerragruntOptions, path string, ifExists GenerateConfigExists) (bool, error) { //nolint:lll
	// TODO: Make exhaustive
	switch ifExists { //nolint:exhaustive
	case ExistsError:
		return false, fmt.Errorf(
			"file %s already exists: %w",
			path,
			errors.WithStackTrace(GenerateFileExistsError{path: path}),
		)

	case ExistsSkip:
		// Do nothing since file exists and skip was configured
		terragruntOptions.Logger.Debugf(
			"The file path %s already exists and if_exists for code generation set to \"skip\". Will not regenerate file.",
			path,
		)
	...
	case ExistsOverwrite:
		...
		// exists.
		terragruntOptions.Logger.Debugf(
			"The file path %s already exists and if_exists for code generation set to \"overwrite\". Regenerating file.",
			path,
		)
	...
	case ExistsOverwriteTerragrunt:
		...
		if !wasGenerated {
			...
			return false, fmt.Errorf(
				"file %s already exists and was not generated by terragrunt: %w",
				path,
				errors.WithStackTrace(GenerateFileExistsError{path: path}),
			)
		}
		...
		terragruntOptions.Logger.Debugf(
			"The file path %s already exists, but was a previously generated file by terragrunt. Since if_exists for code generation is set to \"overwrite_terragrunt\", regenerating file.", //nolint:lll
			path,
		)
	...
	default:
		...
		return false, fmt.Errorf(
			"unknown if_exists value: %v: %w",
			ifExists,
			errors.WithStackTrace(UnknownGenerateIfExistsValError{""}),
		)
	}
}

// shouldRemoveWithFileExists returns true if the already existing file should be removed.
func shouldRemoveWithFileExists(terragruntOptions *options.TerragruntOptions, path string, ifDisable GenerateConfigDisabled) (bool, error) { //nolint:lll
	// TODO: Make exhaustive
	switch ifDisable { //nolint:exhaustive
	case DisabledSkip:
		// Do nothing since skip was configured.
		terragruntOptions.Logger.Debugf(
			"The file path %s already exists and if_disabled for code generation set to \"skip\", will not remove file.",
			path,
		)
	...
	case DisabledRemove:
		...
		terragruntOptions.Logger.Debugf(
			"The file path %s already exists and if_disabled for code generation set to \"remove\", removing file.",
			path,
		)

		return true, nil
	case DisabledRemoveTerragrunt:
		// If file was not generated, error out because remove_terragrunt if_disabled
		// setting only handles if the existing file was generated by terragrunt.
		wasGenerated, err := fileWasGeneratedByTerragrunt(path)
		if err != nil {
			return false, err
		}
		...
		if !wasGenerated {
			...
			return false, fmt.Errorf(
				"file %s already exists and was not generated by terragrunt: %w",
				path,
				errors.WithStackTrace(GenerateFileRemoveError{path: path}),
			)
		}

		// Since file was generated by terragrunt, removing.
		terragruntOptions.Logger.Debugf(
			"The file path %s already exists, but was a previously generated file by terragrunt. Since if_disabled for code generation is set to \"remove_terragrunt\", removing file.", //nolint:lll
			path,
		)
	...
	default:
		// This shouldn't happen, but we add this case anyway for defensive coding.
		return false, fmt.Errorf(
			"unknown if_disabled value: %v: %w",
			ifDisable,
			errors.WithStackTrace(UnknownGenerateIfDisabledValError{""}),
		)
	}
}
...
func fileWasGeneratedByTerragrunt(path string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to open file %s: %w", path, errors.WithStackTrace(err))
	}
	defer file.Close()
	...
	if err != nil {
		return false, fmt.Errorf("failed to read first line of file %s: %w", path, errors.WithStackTrace(err))
	}

	return strings.HasSuffix(strings.TrimSpace(firstLine), TerragruntGeneratedSignature), nil
}

// RemoteStateConfigToTerraformCode converts the arbitrary map that represents a remote state config
// into HCL code to configure that remote state.
func RemoteStateConfigToTerraformCode(backend string, config map[string]interface{}) ([]byte, error) {
	...
	for _, key := range backendKeys {
		...
		if key == assumeRoleConfigKey {
			...
			if err != nil {
				return nil, fmt.Errorf("failed to decode assume role config: %w", errors.WithStackTrace(err))
			}

			// write assume role map as HCL object
			ctyVal, err := convertValue(assumeRoleMap)
			if err != nil {
				return nil, fmt.Errorf("failed to convert assume role config to HCL: %w", errors.WithStackTrace(err))
			}
		...
		if err != nil {
			return nil, fmt.Errorf("failed to convert config value to HCL: %w", errors.WithStackTrace(err))
		}
...
func convertValue(v interface{}) (ctyjson.SimpleJSONValue, error) {
	...
	if err != nil {
		return ctyjson.SimpleJSONValue{}, fmt.Errorf("failed to marshal value to JSON: %w", errors.WithStackTrace(err))
	}

	var ctyVal ctyjson.SimpleJSONValue
	if err := ctyVal.UnmarshalJSON(jsonBytes); err != nil {
		return ctyjson.SimpleJSONValue{}, fmt.Errorf("failed to unmarshal JSON to cty: %w", errors.WithStackTrace(err))
	}
...
func GenerateConfigExistsFromString(val string) (GenerateConfigExists, error) {
	...
	return ExistsUnknown, fmt.Errorf(
		"unknown if_exists value: %s: %w",
		val,
		errors.WithStackTrace(UnknownGenerateIfExistsValError{val: val}),
	)
}

// GenerateConfigDisabledFromString converts a string representation of if_disabled into the enum,
// returning an error if it is not set to one of the known values.
func GenerateConfigDisabledFromString(val string) (GenerateConfigDisabled, error) {
	...
	return DisabledUnknown, fmt.Errorf(
		"unknown if_disabled value: %s: %w",
		val,
		errors.WithStackTrace(
			UnknownGenerateIfDisabledValError{val: val},
		),
	)
}
