...
const (
	// FuncNameFindInParentFolders is the name of the function that finds a parent Terragrunt configuration file in the
	// parent folders above the current Terragrunt configuration file and returns its path.
	FuncNameFindInParentFolders = "find_in_parent_folders"
	// FuncNamePathRelativeToInclude is the name of the function that returns the relative path between the included
	// Terragrunt configuration file and the current Terragrunt configuration file.
	FuncNamePathRelativeToInclude = "path_relative_to_include"
	// FuncNamePathRelativeFromInclude is the name of the function that returns the relative path from the current
	// Terragrunt configuration to the included Terragrunt configuration file.
	FuncNamePathRelativeFromInclude = "path_relative_from_include"
	// FuncNameGetEnv is the name of the function that returns the value of an environment variable.
	FuncNameGetEnv = "get_env"
	// FuncNameRunCmd is the name of the function that runs a command and returns the result.
	FuncNameRunCmd = "run_cmd"
	// FuncNameReadTerragruntConfig is the name of the function that reads and parses a Terragrunt configuration file.
	FuncNameReadTerragruntConfig = "read_terragrunt_config"
	// FuncNameGetPlatform is the name of the function that returns the OS platform.
	FuncNameGetPlatform = "get_platform"
	// FuncNameGetRepoRoot is the name of the function that returns the repository root as an absolute path.
	FuncNameGetRepoRoot = "get_repo_root"
	// FuncNameGetPathFromRepoRoot is the name of the function that returns the path from the repository root.
	FuncNameGetPathFromRepoRoot = "get_path_from_repo_root"
	// FuncNameGetPathToRepoRoot is the name of the function that returns the path to the repository root.
	FuncNameGetPathToRepoRoot = "get_path_to_repo_root"
	// FuncNameGetTerragruntDir is the name of the function that returns the directory
	// where the Terragrunt configuration file lives.
	FuncNameGetTerragruntDir = "get_terragrunt_dir"
	// FuncNameGetOriginalTerragruntDir is the name of the function that returns the
	// directory where the original Terragrunt configuration file lives.
	FuncNameGetOriginalTerragruntDir = "get_original_terragrunt_dir"
	// FuncNameGetTerraformCommand is the name of the function that returns the current terraform command in execution.
	FuncNameGetTerraformCommand = "get_terraform_command"
	// FuncNameGetTerraformCLIArgs is the name of the function that returns the cli args for terraform.
	FuncNameGetTerraformCLIArgs = "get_terraform_cli_args"
	// FuncNameGetParentTerragruntDir is the name of the function that returns the parent directory where the Terragrunt
	// configuration file lives.
	FuncNameGetParentTerragruntDir = "get_parent_terragrunt_dir"
	// FuncNameGetAWSAccountID is the name of the function that returns the AWS account id associated to the current set
	// of credentials.
	FuncNameGetAWSAccountID = "get_aws_account_id"
	// FuncNameGetAWSCallerIdentityArn is the name of the function that returns the ARN of the AWS identity associated
	// with the current set of credentials.
	FuncNameGetAWSCallerIdentityArn = "get_aws_caller_identity_arn"
	// FuncNameGetAWSCallerIdentityUserID is the name of the function that returns the UserID of the AWS identity
	// associated with the current set of credentials.
	FuncNameGetAWSCallerIdentityUserID = "get_aws_caller_identity_user_id"
	// FuncNameGetTerraformCommandsThatNeedVars is the name of the function that returns the list of terraform commands
	// that need vars.
	FuncNameGetTerraformCommandsThatNeedVars = "get_terraform_commands_that_need_vars"
	// FuncNameGetTerraformCommandsThatNeedLocking is the name of the function that returns the list of terraform commands
	// that need locking.
	FuncNameGetTerraformCommandsThatNeedLocking = "get_terraform_commands_that_need_locking"
	// FuncNameGetTerraformCommandsThatNeedInput is the name of the function that returns the list of terraform commands
	// that need input.
	FuncNameGetTerraformCommandsThatNeedInput = "get_terraform_commands_that_need_input"
	// FuncNameGetTerraformCommandsThatNeedParallelism = the name of the function that returns the list of tofu/terraform
	...
	// FuncNameSopsDecryptFile is the name of the function that decrypts a file using sops.
	FuncNameSopsDecryptFile = "sops_decrypt_file"
	// FuncNameGetTerragruntSourceCLIFlag is the name of the function that returns the terragrunt source cli flag.
	FuncNameGetTerragruntSourceCLIFlag = "get_terragrunt_source_cli_flag"
	// FuncNameGetDefaultRetryableErrors is the name of the function that returns the default retryable errors.
	FuncNameGetDefaultRetryableErrors = "get_default_retryable_errors"
	// FuncNameReadTfvarsFile is the name of the function that reads a tfvars file.
	FuncNameReadTfvarsFile = "read_tfvars_file"
	// FuncNameGetWorkingDir is the name of the function that returns the current working dir.
	FuncNameGetWorkingDir = "get_working_dir"
	// FuncNameStartsWith is the name of the function that checks if a string starts with a prefix.
	FuncNameStartsWith = "startswith"
	// FuncNameEndsWith is the name of the function that checks if a string ends with a suffix.
	FuncNameEndsWith = "endswith"
	// FuncNameStrContains is the name of the function that checks if a string contains a substring.
	FuncNameStrContains = "strcontains"
	// FuncNameTimeCmp is the name of the function that compares two time strings.
	FuncNameTimeCmp = "timecmp"
	// sopsCacheName is the name of the cache used for sops decryption.
	sopsCacheName = "sopsCache"
)

// TerraformCommandsNeedLocking is a list of terraform commands that accept -lock-timeout.
var TerraformCommandsNeedLocking = []string{ //nolint:gochecknoglobals
	"apply",
	"destroy",
	"import",
	"plan",
...
var TerraformCommandsNeedVars = []string{ //nolint:gochecknoglobals
	"apply",
	"console",
	"destroy",
	"import",
...
// TerraformCommandsNeedInput is a list of terraform commands that accept -input=.
var TerraformCommandsNeedInput = []string{ //nolint:gochecknoglobals
	"apply",
	"import",
	"init",
	"plan",
...
var TerraformCommandsNeedParallelism = []string{ //nolint:gochecknoglobals
	"apply",
	"plan",
	"destroy",
}

// EnvVar is a struct that represents an environment variable that can be retrieved using the get_env function.
type EnvVar struct {
	Name         string
	DefaultValue string
	IsRequired   bool
...
func getPlatform(_ *ParsingContext) (string, error) {
...
func getPathFromRepoRoot(ctx *ParsingContext) (string, error) {
	repoAbsPath, err := getRepoRoot(ctx)
	if err != nil {
		return "", err
	}
...
// getOriginalTerragruntDir returns the directory where the original Terragrunt configuration file lives.
// This is primarily useful when one Terragrunt config is being read from another.
//
// e.g. if /terraform-code/terragrunt.hcl
...
func RunCommand(ctx *ParsingContext, args []string) (string, error) {
	...
	// To avoid re-run of the same run_cmd command, is used in memory cache for command results,
	// with caching key path + arguments
	// see: https://github.com/gruntwork-io/terragrunt/issues/1427
	...
	cmdOutput, err := shell.RunShellCommandWithOutput(
		ctx,
...
// FindInParentFolders finds a parent Terragrunt configuration file in the parent folders above the
// current Terragrunt configuration file and return its path.
...
// PathRelativeToInclude returns the relative path between the included Terragrunt
// configuration file and the current Terragrunt configuration file. Name param is required
// and used to lookup the relevant import block when called in a child config with multiple
// import blocks.
func PathRelativeToInclude(ctx *ParsingContext, params []string) (string, error) {
	...
	case len(ctx.TrackInclude.CurrentList) > 0:
		...
		if err != nil && goErrors.Is(err, ErrImportMapEmpty) {
			return "", err
		}
...
// configuration to the included Terragrunt configuration file.
...
func getWorkingDir(ctx *ParsingContext) (string, error) {
	...
	source, err := terraform.NewSource(
		sourceURL,
		ctx.TerragruntOptions.DownloadDir,
...
func getDefaultRetryableErrors(_ *ParsingContext) ([]string, error) {
...
// If given a default value, this will return the default if the terragrunt config file does not exist.
...
// GetTerragruntSourceForModule returns the --terragrunt-source parameter for a module. This parameter is used to
// specify the source URL for the module.
//
...
// git::git@github.com:acme/infrastructure-modules.git//networking/vpc?ref=v0.0.1
//
// This method will return: /source/infrastructure-modules//networking/vpc.
func GetTerragruntSourceForModule(sourcePath string, modulePath string, moduleTerragruntConfig *TerragruntConfig) (string, error) { //nolint:lll
	if !sourceAvailable(sourcePath, moduleTerragruntConfig) {
		return "", nil
	}
...
// getModulePathFromSourceURL gets the module path from the source URL.
...
var sopsCache = cache.NewCache[string](sopsCacheName) //nolint:gochecknoglobals
...
var sopsFormatToString = map[formats.Format]string{ //nolint:gochecknoglobals
...
var (
	// ErrImportMapEmpty is returned when the import map is empty.
	ErrImportMapEmpty = goErrors.New("import map is empty")
)
...
var (
	// ErrTwoTimestampsRequired is returned when the function is called with an incorrect number of parameters.
	ErrTwoTimestampsRequired = goErrors.New("function can take only two parameters: timestamp_a and timestamp_b")
)
...
var (
	// ErrUnknownSopsError is returned when an unknown error occurs during sops decryption.
	ErrUnknownSopsError = goErrors.New("unknown error occurred during sops decryption")
)
		...
		if groupResultsField.IsValid() && groupResultsField.Kind() == reflect.Slice {
			for i := range groupResultsField.Len() {
				...
				if groupErr.CanInterface() {
					err, ok := groupErr.Interface().(error)
					if ok {
						errs = multierror.Append(errs, err)
					} else {
						errs = multierror.Append(errs, ErrUnknownSopsError)
					}
				}
			}
		}
...
// sourceAvailable checks if the source is available.
func sourceAvailable(sourcePath string, moduleTerragruntConfig *TerragruntConfig) bool {
	return sourcePath != "" &&
		moduleTerragruntConfig.Terraform != nil &&
