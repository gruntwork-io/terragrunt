...
func TestPartialParseDoesNotResolveIgnoredBlockEvenInParent(t *testing.T) {
	...
	_, err := config.PartialParseConfigFile(ctx.WithDecodeList(config.TerragruntFlags), l, opts.Config.TerragruntConfigPath, nil)
	require.NoError(t, err)

	_, err = config.PartialParseConfigFile(ctx.WithDecodeList(config.DependenciesBlock), l, opts.Config.TerragruntConfigPath, nil)
	assert.Error(t, err)
}
...
func TestPartialParseOnlyInheritsSelectedBlocksFlags(t *testing.T) {
	...
	terragruntConfig, err := config.PartialParseConfigFile(ctx, l, opts.Config.TerragruntConfigPath, nil)
	require.NoError(t, err)
