...
`
	...
	terragruntConfig, err := ParseConfigString(config, mockOptionsForTest(t), nil, DefaultTerragruntConfigPath, &Context{})
...
`
	...
	terragruntConfig, err := ParseConfigString(config, mockOptionsForTest(t), nil, DefaultTerragruntJsonConfigPath, &Context{})
	require.NoError(t, err)
...
`
	...
	_, err := ParseConfigString(config, mockOptionsForTest(t), nil, DefaultTerragruntConfigPath, &Context{})
	require.Error(t, err)
...
`, DefaultTerragruntConfigPath)
	...
	terragruntConfig, err := ParseConfigString(config, &opts, nil, opts.TerragruntConfigPath, &Context{})
...
func TestParseTerragruntConfigTwoLevels(t *testing.T) {
	...
	_, actualErr := ParseConfigString(config, opts, nil, configPath, &Context{})
...
func TestParseTerragruntConfigEmptyConfig(t *testing.T) {
	...
	cfg, err := ParseConfigString(config, mockOptionsForTest(t), nil, DefaultTerragruntConfigPath, &Context{})
...
`
	...
	terragruntConfig, err := ParseConfigString(config, &opts, nil, DefaultTerragruntConfigPath, &Context{})
	if err != nil {
		t.Fatal(err)
	}
