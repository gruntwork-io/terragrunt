...
func TestParseDependencyBlockMultiple(t *testing.T) {
	...
	ctx := config.NewParsingContext(t.Context(), log.New(), mockOptionsForTestWithConfigPath(t, filename))
	...
	tfConfig, err := config.ParseConfigFile(ctx, log.New(), filename, nil)
	require.NoError(t, err)
