...
//
//   - Continuously evaluate the block until all references are evaluated, defering evaluation of
//     anything that references other locals until those references are evaluated.
//
...
func attemptEvaluateLocals( //nolint:nonamedreturns
	ctx *ParsingContext,
	file *hclparse.File,
...
func canEvaluateLocals(expression hcl.Expression, evaluatedLocals map[string]cty.Value) hcl.Diagnostics {
	...
	for _, localVar := range localVars {
		...
		case localVar.IsRelative():
			...
			detail = "This caused an impossible condition, tnis is almost certainly a bug in Terragrunt. Please open an issue at github.com/gruntwork-io/terragrunt with this message and the contents of your terragrunt.hcl file that caused this." //nolint:lll
		...
		case rootName != "local":
			...
			detail = fmt.Sprintf(
				"You can only reference to other local variables here, but it looks like you're referencing something else (%q is not defined)", //nolint:lll
				rootName,
			)
		...
		case !hasEvaluated:
			...
			detail = fmt.Sprintf(
				"The local reference '%s' is not evaluated. Either it is not ready yet in the current pass, or there was an error evaluating it in an earlier stage.", //nolint:lll
				localName,
			)
		}
...
// CouldNotEvaluateAllLocalsError is an error that is returned when we could not evaluate all locals in a block. This
// is most likely due to a circular reference in the locals block.
...
// MaxIterError is an error that is returned when we reach the maximum number of iterations in evaluating locals. This
// is most likely due to a bug in Terragrunt.
type MaxIterError struct{}

func (err MaxIterError) Error() string {
	return "Maximum iterations reached in attempting to evaluate locals. This is most likely a bug in Terragrunt. Please file an issue on the project: https://github.com/gruntwork-io/terragrunt/issues" //nolint:lll
}
