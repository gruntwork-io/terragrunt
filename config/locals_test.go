...
func TestEvaluateLocalsBlock(t *testing.T) {
	...
	ctx := config.NewParsingContext(t.Context(), log.New(), terragruntOptions)
	evaluatedLocals, err := config.EvaluateLocalsBlock(ctx, log.New(), file)
	require.NoError(t, err)
...
func TestEvaluateLocalsBlockImpossibleWillFail(t *testing.T) {
	...
	ctx := config.NewParsingContext(t.Context(), log.New(), terragruntOptions)
	_, err = config.EvaluateLocalsBlock(ctx, log.New(), file)
	require.Error(t, err)
