...
const (
	...
	stackAutoGeneratedComment = "# Auto-generated by the terragrunt.stack.hcl file by Terragrunt. Do not edit manually"
)
...
func writeValues(l log.Logger, values *cty.Value, directory string) error {
	...
	if values == nil {
		...
		fileHandler, errOpenFIle := os.Open(filePath)
		if errOpenFIle != nil {
			if os.IsNotExist(errOpenFIle) {
				l.Debugf("No old values file found in %s", directory)
				return nil
			}

			return errors.Errorf("failed to open values file %s: %w", filePath, errOpenFIle)
		}
		...
		if scanner.Scan() {
			if !strings.HasPrefix(scanner.Text(), stackAutoGeneratedComment) {
				l.Debugf("customer provided values file %s found, skipping", filePath)
				return nil
			}
		}
		...
		if err := os.Remove(filePath); err != nil {
			...
			return errors.Errorf("failed to remove old values file %s: %w", filePath, err)
		}
	...
	prefix := string(stackAutoGeneratedComment) + "\n"
	body := file.Body()
