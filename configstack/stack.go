// Package configstack provides the ability to manage a stack of Terragrunt Units
// (i.e. files named terragrunt.hcl) and their dependencies.
package configstack
...
// of the given TerragruntOptions and assembles them into a Stack object that can be
// applied or destroyed in a single command.
func FindStackInSubfolders(ctx context.Context, terragruntOptions *options.TerragruntOptions, opts ...Option) (*Stack, error) { //nolint:lll
...
// WithOptions applies the given options to the stack.
...
func (stack *Stack) LogModuleDeployOrder(logger *logrus.Entry, terraformCommand string) error {
	outStr := fmt.Sprintf(
		"The stack at %s will be processed in the following order for command %s:\n",
...
// Run the given Terraform command on all modules in the stack. The order in which the modules are run is determined by
// the dependencies between the modules. If the IgnoreDependencyOrder flag is set, the modules will be run in parallel.
	...
	for _, module := range stack.Modules {
		...
		if planFile != "" {
			terragruntOptions.Logger.Debugf(
				"Using output file %s for module %s",
				planFile,
				module.TerragruntOptions.TerragruntConfigPath,
			)

			if module.TerragruntOptions.TerraformCommand == terraform.CommandNamePlan {
				// for plan command add -out=<file> to the terraform cli args
				module.TerragruntOptions.TerraformCliArgs = util.StringListInsert(
					module.TerragruntOptions.TerraformCliArgs,
					"-out="+planFile,
					len(module.TerragruntOptions.TerraformCliArgs),
				)
			} else {
				module.TerragruntOptions.TerraformCliArgs = util.StringListInsert(
					module.TerragruntOptions.TerraformCliArgs,
...
func (stack *Stack) createStackForTerragruntConfigPaths(ctx context.Context, terragruntConfigPaths []string) error {
	err := telemetry.Telemetry(ctx, stack.terragruntOptions, "create_stack_for_terragrunt_config_paths", map[string]interface{}{ //nolint:lll
...
func (stack *Stack) ResolveTerraformModules(ctx context.Context, terragruntConfigPaths []string) (TerraformModules, error) { //nolint:lll
	...
	err = telemetry.Telemetry(ctx, stack.terragruntOptions, "resolve_external_dependencies_for_modules", map[string]interface{}{ //nolint:lll
		"working_dir": stack.terragruntOptions.WorkingDir,
	}, func(_ context.Context) error {
...
func (stack *Stack) resolveModules(ctx context.Context, canonicalTerragruntConfigPaths []string, howTheseModulesWereFound string) (TerraformModulesMap, error) { //nolint:lll
	...
	for _, terragruntConfigPath := range canonicalTerragruntConfigPaths {
		if !util.FileExists(terragruntConfigPath) {
			return nil, ProcessingModuleError{
				UnderlyingError:       os.ErrNotExist,
				ModulePath:            terragruntConfigPath,
				HowThisModuleWasFound: howTheseModulesWereFound,
			}
		}
...
func (stack *Stack) resolveTerraformModule(ctx context.Context, terragruntConfigPath string, modulesMap TerraformModulesMap, howThisModuleWasFound string) (*TerraformModule, error) { //nolint:lll
	...
	// which implies that `TerragruntConfigPath` must refer to a child configuration file, and the defined `IncludeConfig`
	// must contain the path to the file itself
	...
	if stack.childTerragruntConfig != nil &&
		stack.childTerragruntConfig.ProcessedIncludes.ContainsPath(
	...
	terragruntSource, err := config.GetTerragruntSourceForModule(
		stack.terragruntOptions.Source,
		modulePath,
		terragruntConfig,
	)
	if err != nil {
	...
	if (terragruntConfig.Terraform == nil ||
		terragruntConfig.Terraform.Source == nil ||
		*terragruntConfig.Terraform.Source == "") &&
		matches == nil {
		stack.terragruntOptions.Logger.Debugf(
			"Module %s does not have an associated terraform configuration and will be skipped.",
			filepath.Dir(terragruntConfigPath),
		)
...
// resolveDependenciesForModule looks through the dependencies of the given module and
// resolve the dependency paths listed in the module's config.
// If `skipExternal` is true, the func returns only dependencies that are inside
// of the current working directory, which means they are part of the environment the
// user is trying to apply-all or destroy-all. Note that this method will NOT fill
// in the Dependencies field of the TerraformModule struct (see the crosslinkDependencies method for that).
func (stack *Stack) resolveDependenciesForModule(ctx context.Context, module *TerraformModule, modulesMap TerraformModulesMap, skipExternal bool) (TerraformModulesMap, error) { //nolint:lll
	...
	key := fmt.Sprintf(
		...
		stack.terragruntOptions.WorkingDir,
		skipExternal,
		stack.terragruntOptions.TerraformCommand,
	)
...
// Look through the dependencies of the modules in the given map
// and resolve the "external" dependency paths listed in
// each modules config (i.e. those dependencies not in the given
// list of Terragrunt config canonical file paths).
// These external dependencies are outside of the current working directory,
// which means they may not be part of the
// environment the user is trying to apply-all or destroy-all.
// Therefore, this method also confirms whether the user wants
// to actually apply those dependencies or just assume they are
// already applied. Note that this method will NOT fill in
// the Dependencies field of the TerraformModule struct
// (see the crosslinkDependencies method for that).
func (stack *Stack) resolveExternalDependenciesForModules(ctx context.Context, modulesMap, modulesAlreadyProcessed TerraformModulesMap, recursionLevel int) (TerraformModulesMap, error) { //nolint:lll
	...
	if recursionLevel > maxLevelsOfRecursion {
		return allExternalDependencies, errors.WithStackTrace(
			InfiniteRecursionError{
				RecursionLevel: maxLevelsOfRecursion,
				Modules:        modulesToSkip,
			},
		)
	}
	...
	if len(allExternalDependencies) > 0 {
		recursiveDependencies, err := stack.resolveExternalDependenciesForModules(
			ctx,
