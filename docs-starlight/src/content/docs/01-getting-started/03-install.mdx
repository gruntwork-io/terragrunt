...
The quickest way to install Terragrunt on Linux or macOS is using the install script:
...
2. Download the latest Terragrunt binary
...
| `--verify-gpg` | Also verify GPG signature | Off |
| `--no-verify` | Skip checksum verification | On |
...
<Code
  lang="bash"
  code={`curl -sL https://terragrunt.gruntwork.io/scripts/install.sh | bash -s -- -v ${version}`}
  frame="terminal"
/>
...
Install with GPG signature verification:
...
```bash
curl -sL https://terragrunt.gruntwork.io/scripts/install.sh -o install.sh
less install.sh
bash install.sh
```

:::note[Windows Users]
This script is for Unix-like systems (Linux/macOS). Windows users should use the PowerShell snippet in the Convenience Scripts section above, or install via [Chocolatey](https://chocolatey.org/).
:::
