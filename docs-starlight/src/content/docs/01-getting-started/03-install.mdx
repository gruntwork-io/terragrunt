...
:::tip[Script Options]
Run `curl -sL https://terragrunt.gruntwork.io/install | bash -s -- --help` to see all options, or [view the script on GitHub](https://github.com/gruntwork-io/terragrunt/blob/main/docs-starlight/public/install).
:::
...
The asdf-terragrunt plugin used by both `mise` and `asdf` is maintained by Gruntwork:
