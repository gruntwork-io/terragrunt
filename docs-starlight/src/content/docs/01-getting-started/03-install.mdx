...
:::note[Windows Users]
This script is for Unix-like systems (Linux/macOS). Windows users should use the PowerShell snippet in the [Convenience Scripts](#convenience-scripts) section below, or install via [Chocolatey](https://chocolatey.org/).
:::
...
|--------|-------------|---------|
| `-v, --version VERSION` | Install specific version | Latest |
| `-d, --dir PATH` | Installation directory | `/usr/local/bin` |
| `-f, --force` | Overwrite existing installation | Off |
...
| Option | Description | Default |
|--------|-------------|---------|
| `--verify-sig` | Verify signature (auto-detect gpg/cosign) | On (v0.98.0+) |
| `--verify-gpg` | Force GPG signature verification | - |
| `--verify-cosign` | Force Cosign signature verification | - |
| `--no-verify-sig` | Skip signature verification | Off |
| `--no-verify` | Skip checksum verification | Off |

:::tip[Signature Verification]
Signature verification is enabled by default for Terragrunt v0.98.0 and later. The script auto-detects whether `gpg` or `cosign` is available.
:::
...
<Code
  lang="bash"
  code={`curl -sL https://terragrunt.gruntwork.io/install.sh | bash -s -- -v ${version}`}
  frame="terminal"
/>
...
```bash
curl -sL https://terragrunt.gruntwork.io/install.sh | bash -s -- --no-verify-sig
```
...
```bash
curl -sL https://terragrunt.gruntwork.io/install.sh -o install.sh
less install.sh
bash install.sh
```
