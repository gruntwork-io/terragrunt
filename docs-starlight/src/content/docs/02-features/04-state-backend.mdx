...
- **Azure Storage container**: If you are using the [Azure Storage backend](https://opentofu.org/docs/language/settings/backends/azurerm) for remote state storage and the `container_name` you specify in `remote_state.config` doesn't already exist, Terragrunt will create it automatically, with [blob versioning](https://learn.microsoft.com/en-us/azure/storage/blobs/versioning-overview) enabled. You will need to specify `storage_account_name` and authentication credentials in `remote_state.config`. Authentication can be provided through:
  - Storage account key via `storage_account_key`
  - SAS token via `sas_token`
  - Azure AD credentials (automatically used if no other auth method is specified)

  If you want to skip blob versioning, set `skip_blob_versioning` to `true`. You can also tag the storage container using `container_tags`.

  We strongly recommend enabling [Azure Storage logging and metrics](https://learn.microsoft.com/en-us/azure/storage/common/storage-analytics-logging) to audit and track operations against the state container.
...
remote_state {
  ...
  config = {
    ...
    # Authentication via storage account key
    storage_account_key = "your-storage-account-key"
    
    # Or authentication via SAS token
    # sas_token         = "your-sas-token"
    
    # Or use Azure AD authentication by not specifying either
    # storage_account_key or sas_token
    
    # Optional: Tag the storage container
    container_tags     = {
      Environment = "Production"
      Project     = "MyApp"
    }
    
    # Optional: Skip blob versioning
...
For the `azurerm` backend, the following config options can be used to customize Azure Storage behavior:
...
remote_state {
  ...
  config = {
    skip_blob_versioning = true         # use only if blob versioning should be disabled
    container_tags      = {             # optional tags to apply to the Azure Storage container
      Environment = "Production"
      Project     = "MyApp"
    }
    disable_container_update = true     # disable automatic container updates by Terragrunt

    # Authentication options (choose one):
    storage_account_key = "your-key"    # authenticate using storage account key
    sas_token          = "your-token"   # authenticate using SAS token
    # If neither is specified, Azure AD authentication will be used
  }
}
```

The config options `container_tags`, `skip_blob_versioning`, and `disable_container_update` are only valid for backend `azurerm`. They are used by Terragrunt and are **not** passed on to OpenTofu/Terraform.
