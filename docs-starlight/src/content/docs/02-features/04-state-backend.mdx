...
remote_state {
  ...
  config = {
    ...
    # 1. Azure AD Authentication (recommended - default)
...
1. **Azure AD Only**: SAS token and storage account key authentication methods are no longer supported
...
6. **Legacy Support**: If you see warnings about `ARM_ACCESS_KEY`, `AZURE_STORAGE_KEY`, or `sas_token` configuration, update to Azure AD authentication
...
| `SAS token authentication is no longer supported` | Switch to Azure AD authentication, MSI, or service principal |
...
Azure AD authentication is the only supported authentication method for Azure remote state backend. Storage account key and SAS token authentication are no longer supported.
...
- `sas_token` - Removed in favor of Azure AD authentication
