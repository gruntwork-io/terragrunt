...
{/*
    To avoid having to install d2 in vercel, or require that contributors have it installed,
    we can simply reference the built artifact.

    If you want to edit the diagram, install d2 locally following these instructions:
    https://github.com/terrastruct/d2/blob/master/docs/INSTALL.md

    Once you have d2 installed, you can uncomment the diagram above, edit it, and see it change live,
    then commit the updated diagram to the repo.
*/}

![Example DAG](../../../../public/d2/docs/02-features/05-run-queue-0.svg)
...
<Aside type="caution">
When using `run --all` with `apply` or `destroy`, Terragrunt automatically adds the `-auto-approve` flag due to limitations with shared stdin making individual approvals impossible. Use [`--no-auto-approve`](/docs/reference/cli/flags/no-auto-approve) to override this, but be aware you might need alternative approval workflows.
</Aside>
