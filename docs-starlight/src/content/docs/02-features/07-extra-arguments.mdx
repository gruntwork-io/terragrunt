...
slug: docs/features/extra-arguments
sidebar:
  order: 7
---
...
You can even use built-in functions such as [get\_terraform\_commands\_that\_need\_locking](/docs/reference/built-in-functions/#get_terraform_commands_that_need_locking) to conveniently populate the list of OpenTofu/Terraform commands that need locking:
...
In addition to being appended to the `tofu init`/`terraform init` command that is run when you explicitly run `terragrunt init`, `extra_arguments` for `init` will also be appended to the `init` commands that are automatically run during other commands (see [Auto-Init](/docs/features/auto-init)).
...
Note that you're encouraged to use the [Provider Caching](/docs/features/provider-caching) feature instead of manually installing plugins in most cases.
...
See the [get\_terragrunt\_dir()](/docs/reference/built-in-functions/#get_terragrunt_dir) and [get\_parent\_terragrunt\_dir()](/docs/reference/built-in-functions/#get_parent_terragrunt_dir) documentation for more details.
