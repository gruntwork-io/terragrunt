...
TIP: You can also use multiple `--queue-include-dir` flags, and globs for pattern matching, as you'll see later in the guide.
...
NOTE: Those `//` are there on purpose. They're how `go-getter`, the library that Terragrunt uses (just like OpenTofu), indicates that it's working with a directory *within* a module source. This allows relative references like `../s3` to work within the `best_cat` module.
