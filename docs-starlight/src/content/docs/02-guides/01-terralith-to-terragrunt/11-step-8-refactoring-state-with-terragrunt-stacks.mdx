...
We can now remove the `.gitignore` files and the generated directories, to prove to ourselves that state has migrated successfully!

```bash
# live

rm -f ./***/.gitignore ./***/ddb ./***/iam ./***/lambda ./***/s3
terragrunt run --all plan

# No changes!
```

## Understanding the final project layout

The content that's left in your repository that isn't ignored by Git should now look like the following.
...
That's all you need to hold onto in your repository to reliably reproduce your infrastructure estate using Terragrunt,
while preserving the ability to regenerate that same IaC with low blast radius, and clear isolation between components.

On generation, you'll have the following `.gitignore`'ed content in your repository:
...
It might be easier to see what's going on without the `.terragrunt-stack-manifest` and `.terraform.lock.hcl` files. Those files are automatically generated by Terragrunt and OpenTofu/Terraform respectively to track metadata about the unit.
