...
<Aside type="caution">
A directory cannot contain both a `terragrunt.hcl` and a `terragrunt.stack.hcl` file. Each directory must be either a unit or a stack, not both. See [Known Limitations](#a-directory-cannot-contain-both-terragrunthcl-and-terragruntstackhcl) for details.
</Aside>
...
### A directory cannot contain both `terragrunt.hcl` and `terragrunt.stack.hcl`

A directory must be either a unit (with `terragrunt.hcl`) or a stack (with `terragrunt.stack.hcl`), not both. Placing both files in the same directory will result in a discovery error:

```
directory "/path/to/dir" contains both terragrunt.hcl and terragrunt.stack.hcl;
a directory must be either a unit or a stack, not both
```

If you need both unit and stack behavior, separate them into distinct directories.
