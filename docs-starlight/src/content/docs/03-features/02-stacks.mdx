...
<Aside type="note">
Note that the contents are generated into a <code>.terragrunt-stack</code> directory. This is to make it convenient to add <code>.terragrunt-stack</code> to your <code>.gitignore</code> file, and always generate the stack on demand instead of checking it in.

Also note the <code>terragrunt.values.hcl</code> files generated next to the <code>terragrunt.hcl</code> files of the units. These files contain the values specified in the <code>values</code> block for the unit.
...
When you apply this unit, the output will be read from the `vpc` unit and passed in as an input variable right before calling `tofu apply`/`terraform apply`.
...
So for the above example, the order for the `run --all apply` command would be:
...
Using a mock output is typically the best solution here, as you typically don't actually care that an accurate value is used at this stage, just that it will plan successfully. When you actually apply the unit, that's when you want to be sure that a real value is used.
...
What if you wanted to restrict this behavior to only the `validate` command?

You can use the `mock_outputs_allowed_terraform_commands` attribute to indicate that the `mock_outputs` should only be used when running those OpenTofu/Terraform commands.
...
The `backend-app` unit would need to access the outputs of the `mysql` and `valkey` units to use as inputs. To do this, you can use the `dependency` block to access the outputs of the `mysql` and `valkey` units.
...
Update the `terragrunt.stack.hcl` file to pass dependency paths through `values`:
...
Terragrunt will only include the units in the `us-east-1` stack and its children in the queue of units to run (unless external dependencies are pulled in, as discussed in the `run --all` docs).

Generally speaking, this is the primary tool Terragrunt users use to control the blast radius of their changes.

In addition to using your working directory to control what's included in a run queue, you can also use flags like `--include-dir` and `--exclude-dir` to explicitly control what's included.
...
## Recommended operational pattern when stacks depend on each other

In practice, when you need references across generated stacks (or across environments), the most important thing is to ensure a **single generation boundary** so that all generated units live under a single `.terragrunt-stack/` tree.

This is the operational pattern we have converged on:

* Use one **root stack** that composes all other stacks that need to reference each other.
* Pass cross-stack (or cross-environment) dependency paths as `values` (for example `vpc_config_path`, `shared_kms_path`, etc.).
* Declare the actual `dependency` blocks inside **units**, using those `values` paths.

This preserves the intended separation of responsibilities:

* Stacks define structure and wiring via values.
* Units define execution-time dependency resolution.

### Root stack per environment (recommended default)

When environments are isolated, define one root stack per environment (e.g., `live/dev`, `live/prod`) and run `terragrunt stack ...` from there.

This keeps blast radius small and makes it difficult to accidentally couple environments.

### “All” stack (single root that generates dev/stg/prod under one tree)

If you intentionally need cross-environment references (migrations, controlled promotions, shared bootstrap flows), define a single `live/all` stack that generates multiple environments under one `.terragrunt-stack/` directory.
...
Example `live/all/terragrunt.stack.hcl`:
...
stack "stg" {
  ...
  values = {
    ...
    # Example cross-env reference to a unit path inside the same generated tree
...
Cross-environment references are then declared at unit level:

```hcl
# Example in a unit terragrunt.hcl
dependency "dev_vpc" {
...
This approach ensures all environments share a single generated tree, making relative paths stable.

### When the “All” stack is justified

Use an “all” stack when:

* Cross-environment references are intentional and part of the workflow.
* You need a single generation boundary so paths are stable.
* You are operating promotion/migration/cutover workflows at the Terragrunt layer.

### When the “All” stack is an anti-pattern

Avoid the “all” stack when:

* Environments should be strictly isolated.
* You want strict blast-radius separation by directory.
* Cross-environment references would be accidental rather than intentional.
* You expect fully independent CI pipelines per environment without coordination.
