...
**Note**: You can disable automatic remote state backend bootstrapping by setting `remote_state.disable_init = true`. This prevents Terragrunt from creating S3 buckets, DynamoDB tables, or other backend resources, but still allows OpenTofu/Terraform to initialize its backend configuration. This is particularly useful for CI/CD environments where you want to run `validate`, `fmt`, or `plan` without provisioning backend resources (or attempting to). Note that this will be the default behavior in a future version of Terragrunt, and backend bootstrapping will require explicit opt-in via the [`--backend-bootstrap`](https://terragrunt-v1.gruntwork.io/docs/reference/cli/commands/backend/bootstrap/) flag.
