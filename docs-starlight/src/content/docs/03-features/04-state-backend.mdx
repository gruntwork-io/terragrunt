...
- **Azure Storage container**: If you are using the [Azure Storage backend](https://opentofu.org/docs/language/settings/backends/azurerm) for remote state storage and the `container_name` you specify in `remote_state.config` doesn't already exist, Terragrunt will create it automatically. The container will be created with:
  ...
  2. Managed Service Identity (for Azure environments)
     - Automatic authentication when running in Azure
     - No credentials needed in configuration
     - Secure and managed by Azure platform
...
remote_state {
  ...
  config = {
    ...
    # 1. Azure AD / Default credential chain (recommended)
    #    Leave all other authentication flags unset to let Terragrunt discover credentials automatically.
    ...
    # 4. SAS token (data-plane access only)
    # use_azuread_auth = false
    # sas_token        = get_env("AZURE_STORAGE_SAS", "")
    
    # Optional: Disable Terragrunt's automatic storage account updating
    skip_storage_account_update = true  # Set to true if you want to manage storage account settings manually
  }
}
```
...
Terragrunt routes all Azure credential handling through the `azureauth` package. Azure AD via the DefaultAzureCredential chain remains the recommended approach because it supports MFA, conditional access, and detailed auditing. Managed identity, explicit service principal credentials, and storage SAS tokens are still supported for compatibility scenarios. Only one authentication method can be active at a time—`use_azuread_auth` defaults to `true`, so set it to `false` when you switch to another option.

**Azure AD / DefaultAzureCredential (recommended)**

When no other method is selected, Terragrunt automatically uses the [Azure Default Credential Chain](https://learn.microsoft.com/en-us/dotnet/api/azure.identity.defaultazurecredential), which attempts credentials in this order:
...
3. Azure CLI sign-in (`az login`)
4. Visual Studio Code Azure extension

No additional configuration is required unless you need to specify a `subscription_id` for storage-account bootstrap operations.
...
config = {
  use_msi          = true
  use_azuread_auth = false
  subscription_id  = "00000000-0000-0000-0000-000000000000" # required for bootstrap features
}
```

Use this when Terragrunt runs inside Azure with a system- or user-assigned managed identity that has access to the target storage account.
...
Provide an app registration with Storage Blob Data Owner (or equivalent) permissions on the storage account or resource group.

**SAS token (data-plane access only)**
...
SAS tokens work for existing storage accounts and support Terragrunt's data-plane operations (for example, reading, writing, and locking state). They do **not** provide access to Azure Resource Manager APIs, so features such as `create_storage_account_if_not_exists`, RBAC assignment, or storage-account configuration updates require Azure AD authentication instead.

**Deprecated or unsupported authentication options:**
...
1. **One method at a time**: `use_azuread_auth` defaults to `true`. Set it to `false` when you configure `use_msi`, service principal credentials, or `sas_token` to avoid the “multiple authentication methods” validation error.
...
4. **Access Control**: RBAC role assignment during bootstrap requires Azure AD credentials (Azure AD default, MSI, or service principal). SAS tokens skip RBAC updates because they only grant data-plane access.
5. **Locking**: Azure Blob lease is used for state locking, no additional resources needed.
6. **Legacy Support**: Storage access keys, connection strings, and related environment variables (`ARM_ACCESS_KEY`, `AZURE_STORAGE_KEY`) are no longer supported.
...
| `no valid authentication method found` | Configure a supported authentication method (Azure AD, MSI, service principal, or SAS) |
| `multiple authentication methods specified` | Ensure only one of `use_azuread_auth`, `use_msi`, service principal fields, or `sas_token` is set |
...
1. **Authentication**
  - Prefer Azure AD / DefaultAzureCredential when possible
  - Use managed identities in Azure environments for workload identities
  - Rotate service principal credentials regularly when they are used
  - Limit SAS tokens to short-lived, scoped scenarios
...
| Option | Description | Default | Updatable After Creation |
|--------|-------------|---------|--------------------------|
| `create_storage_account_if_not_exists` | Whether to check if the storage account exists and create it if needed | `false` | N/A |
| `resource_group_name` | Resource group for the storage account | Same as storage account name with `-rg` suffix | No |
| `location` | Azure region where the storage account should be created | `eastus` | No |
| `enable_versioning` | Whether blob versioning should be enabled on the storage account | `true` | Yes |
| `allow_blob_public_access` | Whether the storage account should allow public blob access | `false` | Yes |
| `account_kind` | Storage account kind | `StorageV2` | No |
| `account_tier` | Storage account performance tier | `Standard` | No |
| `access_tier` | Access tier for the storage account | `Hot` | Yes |
| `replication_type` | Data replication type | `LRS` | No |
| `storage_account_tags` | Tags to apply to the storage account | `{ created-by = "terragrunt" }` | Yes |
...
If the storage account already exists, Terragrunt will check its configuration and update certain settings like versioning, access tier, and tags if they don't match what's specified in your configuration. Read-only properties like storage account kind, tier, and replication type cannot be updated after creation and will generate warnings if they differ.
