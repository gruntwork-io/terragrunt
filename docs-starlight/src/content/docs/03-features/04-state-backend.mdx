...
OpenTofu/Terraform supports [remote state storage](https://www.terraform.io/state/remote.html) via various [backends](https://www.terraform.io/backends) that you normally configure in your `.tf` files as follows:
...
The next time you run `terragrunt`, it will automatically configure all the settings for the backend, if they aren’t configured already, by calling [tofu/terraform init](https://opentofu.org/cli/commands/init/).
...
state? For example, when using the [s3 backend](https://opentofu.org/language/settings/backends/s3/), OpenTofu/Terraform
...
- [s3 backend](https://opentofu.org/language/settings/backends/s3)
- [gcs backend](https://opentofu.org/language/settings/backends/gcs)
...
- **S3 bucket**: If you are using the [S3 backend](https://opentofu.org/language/settings/backends/s3) for remote state storage and the `bucket` you specify in `remote_state.config` doesn’t already exist, Terragrunt will create it automatically, with [versioning](https://docs.aws.amazon.com/AmazonS3/latest/dev/Versioning.html), [server-side encryption](https://docs.aws.amazon.com/AmazonS3/latest/dev/UsingServerSideEncryption.html), and [access logging](https://docs.aws.amazon.com/AmazonS3/latest/dev/ServerLogs.html) enabled.
...
- **DynamoDB table**: If you are using the [S3 backend](https://opentofu.org/language/settings/backends/s3) for remote state storage and/or you specify a `dynamodb_table` (a [DynamoDB table used for locking](https://opentofu.org/language/settings/backends/s3/#dynamodb-state-locking)) in `remote_state.config`, Terragrunt will create them automatically if they don't already exist. They will be created with [server-side encryption](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/EncryptionAtRest.html) enabled, and the DynamoDB table will use the primary key `LockID`.
...
remote_state {
 ...
 enable_bucket_policy_only = false # use only if uniform bucket-level access is needed (https://cloud.google.com/storage/uniform-bucket-level-access)
