...
The `--filter` flag is available in the `find`, `list`, `run`, `hcl fmt`, and `hcl validate` commands. Support for `dag graph` is planned for a future release.
</Aside>
...
- [ ] [dag graph](/docs/reference/cli/commands/dag/graph) (planned for a future release)
