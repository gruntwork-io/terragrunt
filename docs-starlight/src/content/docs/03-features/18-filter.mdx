...
```bash
terragrunt find --filter './apps/** | name=web'
```
...
terragrunt find --external --filter 'external=false'
terragrunt find --external --filter 'external=true'
...
- ..
  - dependencies \<-- Note that this directory is sibling to the current working directory
    - **dependency-of-app1** \<-- Matched by the third filter, but not the fourth filter
        - terragrunt.hcl
</FileTree>
...
```bash
# Find units named 'unit1' OR 'stack1'
terragrunt find --filter unit1 --filter stack1

# Find units in ./envs/prod/* OR ./envs/stage/*
