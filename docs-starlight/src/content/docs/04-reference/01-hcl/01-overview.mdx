...
Terragrunt configuration is defined in `terragrunt.hcl` files. These files use the same HCL syntax as OpenTofu/Terraform configurations.
...
Terragrunt figures out the path to its configuration file according to the following rules:
