...
- `disable_init` (attribute): When `true`, skip automatic creation and management of remote state resources by Terragrunt.
  Some backends have support in Terragrunt to be automatically created if the storage does not exist. Currently, `s3` and `gcs` are the
  ...
  but Terraform will still initialize the backend normally. Defaults to `false`.

  **Note:** When using `generate` with `disable_init = true`, the backend configuration is written to the generated `.tf` file. Terraform will still attempt to connect to the backend during init.

  | `disable_init` | `--backend-bootstrap` | Backend exists | Result |
  |---|---|---|---|
  | `false` | `true` | No | Terragrunt creates backend resources, init succeeds |
  | `false` | `false` | No | No creation, Terraform init fails (bucket not found) |
  | `false` | `false` | Yes | No creation, init succeeds |
  | `true` | any | Yes | No creation, Terraform inits normally ✓ |
  | `true` | any | No | No creation, Terraform init fails (bucket not found) |
