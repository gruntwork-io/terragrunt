...
The `run_cmd` function supports special `--terragrunt-*` parameters that must be passed as the **first arguments** before the actual command. These parameters can be combined in any order:

| Parameter                   | Description                                          | Use Case                                                                                      |
|-----------------------------|------------------------------------------------------|-----------------------------------------------------------------------------------------------|
| `--terragrunt-quiet`        | Suppresses command output from appearing in logs     | For commands that output sensitive values (credentials, secrets, etc.)                        |
| `--terragrunt-global-cache` | Uses global cache instead of directory-based cache   | For commands that produce the same output regardless of directory (e.g., AWS account lookup)  |
| `--terragrunt-no-cache`     | Disables caching completely, runs command every time | For commands that produce dynamic/random values each time (e.g., UUID generation, timestamps) |
...
# Gets current timestamp on each run
...
value = run_cmd("--terragrunt-global-cache", "--terragrunt-no-cache", "--terragrunt-quiet", "./command.sh")
```
...
- `carrot` and `potato` appear only once because subsequent invocations used the cached value
- `uuid1`, `uuid2`, and `uuid3` appear multiple times because each call includes a different UUID from `uuid()`, creating different cache keys
- `uuid3` appears one extra time compared to `uuid1` and `uuid2` because it's declared in both `locals` and `inputs`
- `uuid4` appears only once since it's declared only in `inputs`, which is evaluated once
