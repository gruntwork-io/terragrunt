...
Terragrunt inspects the `run_cmd` arguments for special `--terragrunt-*` options **before** executing your command. Place these options before the command name—Terragrunt removes them, applies the requested behavior, and forwards the remaining arguments to your command. You can mix and match the options in any order:
