...
`get_default_retryable_errors()` returns the default list of retryable errors. This function is designed to be used within the `errors` block with `retry` sub-blocks:
