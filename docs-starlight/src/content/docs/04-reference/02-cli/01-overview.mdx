...
export const mainCommands = commands.filter((command) => {
    return command.data.path === 'run' || command.data.path === 'exec';
});

export const backendCommands = commands.filter((command) => {
    return command.data.path.startsWith('backend');
});

export const stackCommands = commands.filter((command) => {
    return command.data.path.startsWith('stack');
});

export const catalogCommands = commands.filter((command) => {
    return command.data.path === 'catalog' || command.data.path === 'scaffold';
});

export const discoveryCommands = commands.filter((command) => {
    return command.data.path === 'list' || command.data.path === 'find';
});

export const hclCommands = commands.filter((command) => {
    return command.data.path.startsWith('hcl');
});

export const dagCommands = commands.filter((command) => {
    return command.data.path.startsWith('dag');
});

export const renderCommands = commands.filter((command) => {
    return command.data.path === 'render';
});
