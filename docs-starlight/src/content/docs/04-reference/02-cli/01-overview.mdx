...
export const backendCommands = [
    await getEntry('docs', 'docs/reference/cli/commands/backend/bootstrap'),
    await getEntry('docs', 'docs/reference/cli/commands/backend/migrate'),
    await getEntry('docs', 'docs/reference/cli/commands/backend/delete'),
];
...

{
    backendCommands.map((doc) => (
        <LinkCard title={"backend " + doc.data.title} href={`/${doc.id}`} description={doc.data.description} />
    ))
}
