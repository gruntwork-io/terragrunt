...
The `**` glob pattern in [queue-exclude-dir](/docs/reference/cli/commands/run#queue-exclude-dir) and [queue-include-dir](/docs/reference/cli/commands/run#queue-include-dir) now matches all subdirectories regardless of depth, and `**/*` matches subdirectories with a depth of at least one. This behavior is now the default and this strict control is no longer needed.
