...
|----------|-------------|
| `/api/compatibility.json` | All entries |
| `/api/compatibility.json?tool=opentofu` | OpenTofu only |
| `/api/compatibility.json?tool=terraform` | Terraform only |

**Example:**

```bash
curl https://terragrunt.gruntwork.io/api/compatibility.json?tool=opentofu
```
