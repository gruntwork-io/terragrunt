...
<Aside type="note">
Hidden directories (paths whose name begins with a dot like `.cloud/`) are not discovered by default. If your modules live under hidden directories, include them explicitly using [`--queue-include-dir`](/docs/reference/cli/commands/run#queue-include-dir). For example:

```bash
terragrunt run --all plan \
  --queue-include-dir "**/.cloud/**/**"
```

This tells Terragrunt to include any modules under hidden `.cloud` subtrees when building the run queue.
</Aside>
