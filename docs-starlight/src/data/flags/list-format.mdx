...
name: format
description: |
  Format the results as specified. Supported values (text, long, tree, json). Default: text.
type: string
env:
  - TG_FORMAT
---
...
- `json`: Structured JSON output for programmatic processing. Ideal for automation and tooling integration.
...
```bash
# JSON format - Ideal for automation
$ terragrunt list --format json | jq
[
  {
    "path": "live",
    "children": [
