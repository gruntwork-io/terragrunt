...
When enabled, Terragrunt will ignore the dependency order when running commands with [`--all`](#all). This means units will be executed in arbitrary order, which can be useful for read-only operations like `plan`.
