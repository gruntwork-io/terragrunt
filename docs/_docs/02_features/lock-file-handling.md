...
In certain use cases, like when using a remote source module containing the lock file within it, you probably
don't want Terragrunt to also copy the lock file into your working directory, in such scenarios, you can opt out of the copy
feature by using `copy_terraform_lock_file = false` in the `terragrunt.hcl` file as follows:
