...
When `values` are specified, generated units will have access to those values via a special values file generated next to the `terragrunt.hcl` file of the unit. By default, this file is generated in HCL format (`terragrunt.values.hcl`), but can be generated in JSON format (`terragrunt.values.json`) using the `--json-values` flag.
...
The values file will contain the values specified in the `values` block as top-level attributes:
...
Or in JSON format (when using `--json-values`):
...
The unit will be able to leverage those values via `values` variables regardless of format:
...
- A pre-created values file (`terragrunt.values.hcl` or `terragrunt.values.json`) can be provided in the unit source (sibling to the `terragrunt.hcl` file used as the source of the unit). If present, this file will be used as the default values for the unit. However, if the values attribute is defined in the unit block, the generated values file will replace the pre-existing file.
...
- A pre-created values file (`terragrunt.values.hcl` or `terragrunt.values.json`) can be provided in the stack source (sibling to the `terragrunt.stack.hcl` file used as the source of the stack). If present, this file will be used as the default values for the stack. However, if the values attribute is defined in the stack block, the generated values file will replace the pre-existing file.
