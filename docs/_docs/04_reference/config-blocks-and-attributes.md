...
- `source` (attribute): Specifies where to find Terraform configuration files. This parameter supports the exact same syntax as the
  ...
  relative paths work correctly between modules in that repo.
    - NOTE: the `source` parameter can be configured to pull Terraform modules from any Terraform module registry using
      ...
      `tfr://registry.terraform.io/terraform-aws-modules/vpc/aws?version=3.3.0`. If you wish to access a private module
      registry (e.g., [Terraform Cloud/Enterprise](https://www.terraform.io/docs/cloud/registry/index.html)), you can
      provide the authentication to Terragrunt as an environment variable with the key `TG_TF_REGISTRY_TOKEN`. This
      token can be any registry API token.
