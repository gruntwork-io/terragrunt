...
- `skip_bucket_accesslogging`: _DEPRECATED_ If provided, will be ignored. A log warning will be issues in the console output to notify the user.
...
- `accesslogging_bucket_name`: (Optional) When provided as a valid `string`, the S3 bucket that is created to store the terraform state will create an additional S3 bucket as a `target bucket` for the logs output. If not provided, or sting is empty or invalid S3 bucket name, then server access logging for the S3 bucket storing the terraform state will be disabled.
