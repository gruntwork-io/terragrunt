...
<Aside type="caution" title="Public beta">
  The Terragrunt docs MCP server is in public beta. It has no availability guarantee and might change significantly in the future.
</Aside>

If you use an AI coding assistant that speaks MCP, the Terragrunt docs MCP server at [mcp.docs.terragrunt.com](https://mcp.docs.terragrunt.com) answers Terragrunt questions from the official docs, the CLI reference, a curated design-pattern library, and real example config. It is read-only, public, and needs no authentication.
