...
A stack is a collection of units managed by Terragrunt. A stack is either a directory with a tree of units (an [implicit stack](/features/stacks/implicit)) or a `terragrunt.stack.hcl` file that generates them (an [explicit stack](/features/stacks/explicit)). Units within a stack can be dependent on each other, and can be updated in a specific order to ensure that dependencies are resolved in the correct order.
...
A dependency is a relationship between two units in a stack that results in data being passed from the dependency to the dependent unit. A unit can also depend on a whole stack, in which case it receives the outputs of every unit in that stack. See [Depending on a stack](/features/stacks/stack-operations#depending-on-a-stack).
...
Terragrunt also has the ability to fetch outputs without interacting with OpenTofu/Terraform via [--dependency-fetch-output-from-state](/reference/cli/commands/run#dependency-fetch-output-from-state) for dependencies using a supported remote-state backend. <Before version="v1.2.0">This is an experimental feature, and more tooling is planned to make this easier to use.</Before><Since version="v1.2.0">Terragrunt does this by default for supported backends. Pass `--no-dependency-fetch-output-from-state` to read outputs through OpenTofu/Terraform instead.</Since>
