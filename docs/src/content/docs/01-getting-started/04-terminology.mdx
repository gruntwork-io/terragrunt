...
Terragrunt also has the ability to fetch outputs without interacting with OpenTofu/Terraform via [--dependency-fetch-output-from-state](/reference/cli/commands/run#dependency-fetch-output-from-state) for dependencies using a supported remote-state backend. <Before version="1.2.0">This is an experimental feature, and more tooling is planned to make this easier to use.</Before><Since version="1.2.0">Terragrunt does this by default for supported backends. Pass `--no-dependency-fetch-output-from-state` to read outputs through OpenTofu/Terraform instead.</Since>
