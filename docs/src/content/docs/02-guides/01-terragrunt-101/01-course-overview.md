...
Terragrunt started as a thin wrapper that managed remote state for you and provided locking via DynamoDB. Over the next few years, Terraform adopted some of that (e.g., DynamoDB-style state locking landed in Terraform v0.9 in 2017). As those gaps closed, different pains became obvious: the `backend` block still didn't support variables, so teams kept copy-pasting. Common CLI flags had to be repeated on every run. Promoting the same module from dev to staging to prod meant copying whole directory trees. Coordinating multiple state files in the right order meant custom scripting. Terragrunt evolved to address those problems and kept evolving into what it is today: an orchestration tool built around splitting infrastructure into stacks of units that could be run independently in a coordinated manner, and helping teams keep their config DRY.
