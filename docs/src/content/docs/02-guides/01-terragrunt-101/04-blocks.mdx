...
Need to consistently pass additional arguments in certain units? Use **`extra_arguments`**:
...
It pulls in partial configuration from parent files, letting you **define settings once** and **include them everywhere**.

:::tip
Instead of copying remote state configuration, provider settings, or common tags into every `terragrunt.hcl` file, you define them as partial configuration once at the top and include them everywhere. It's the foundation of keeping your configurations **DRY**.
:::
