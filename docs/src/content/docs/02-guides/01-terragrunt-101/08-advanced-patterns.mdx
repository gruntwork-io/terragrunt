...
Control how many units run simultaneously with the [**`--parallelism`**](/reference/cli/commands/run#parallelism) flag:
...
Target specific units without running the entire stack using the [**`--filter`**](/features/filter) flag:
...
| **Both directions**       | `...service...`               | Unit, its dependencies, and its dependents  |
