...
- [azurerm backend](https://opentofu.org/docs/language/settings/backends/azurerm) (experimental; <Before version="1.1.1">see the [`azure-backend`](/reference/experiments/active#azure-backend) experiment, which currently does not change behavior</Before><Since version="1.1.1">bootstrap, delete, and migrate require the [`azure-backend`](/reference/experiments/active#azure-backend) experiment</Since>)
...
<Aside type="caution" title="Experimental">
Azure backend support is delivered through the [`azure-backend`](/reference/experiments/active#azure-backend) experiment.
With the experiment enabled, Terragrunt can bootstrap, delete, and migrate Azure Storage state resources; without it,
those lifecycle operations return an error. Reading and writing the state itself is always handled by
...
remote_state {
  ...
  config = {
    ...
    # Terragrunt-only bootstrap settings
    location                   = "eastus"
...
- `soft_delete_retention_days` - Soft delete retention in days, 1 to 365. Defaults to 7; out-of-range values are reset to 7.
