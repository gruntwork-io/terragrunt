...
Otherwise, these operations return an error. Normal state reading and writing is handled by
OpenTofu/Terraform's native `azurerm` backend.

<Since version="1.1.4">
When both `azure-backend` and [`dependency-fetch-output-from-state`](/reference/experiments/active#dependency-fetch-output-from-state) are enabled, Terragrunt reads dependency outputs directly from the Azure state blob. Normal state reads and writes remain with the native backend.
</Since>
</Since>
</Aside>
