...
- [azurerm backend](https://opentofu.org/docs/language/settings/backends/azurerm) (experimental; requires the [`azure-backend`](/reference/experiments/active#azure-backend) experiment)
...
(unless it exists or creation is skipped), the storage account, and the blob container, then converges blob versioning
and optional soft delete on both new and pre-existing accounts.
...
Sovereign clouds are selected with the `environment` key (`public`, `usgovernment`, or `china`).
...
- With SAS token or access key authentication, Terragrunt cannot verify blob versioning (it is an ARM management-plane property), so `backend delete` and `backend migrate` treat the account as unversioned and require the `--force` flag even when versioning is enabled.
