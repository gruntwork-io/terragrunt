...
<Aside type="note">
When [`dependency-fetch-output-from-state`](/reference/experiments/active#dependency-fetch-output-from-state) is enabled<Before version="1.2.0"> along with `azure-backend`</Before>, and the Azure backend configuration is supported by the direct reader, Terragrunt reads dependency outputs directly from the Azure state blob. Otherwise it uses the native output path.
</Aside>
