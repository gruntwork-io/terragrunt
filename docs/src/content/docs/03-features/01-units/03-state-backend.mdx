...
- [azurerm backend](https://opentofu.org/docs/language/settings/backends/azurerm)<Before version="1.2.0"> (experimental; bootstrap, delete, and migrate require the [`azure-backend`](/reference/experiments/active#azure-backend) experiment)</Before>
...
<Aside type="note">
<Before version="1.2.0">When `azure-backend` is enabled and the</Before><Since version="1.2.0">When the</Since> Azure backend configuration is [supported by the direct reader](/reference/cli/commands/run#dependency-fetch-output-from-state), Terragrunt reads dependency outputs directly from the Azure state blob. Otherwise it uses the native output path.
</Aside>
