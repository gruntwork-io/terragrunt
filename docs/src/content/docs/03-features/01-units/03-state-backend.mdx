...
- [azurerm backend](https://opentofu.org/docs/language/settings/backends/azurerm) (experimental; <Before version="1.1.2">see the [`azure-backend`](/reference/experiments/active#azure-backend) experiment, which currently does not change behavior</Before><Since version="1.1.2">bootstrap, delete, and migrate require the [`azure-backend`](/reference/experiments/active#azure-backend) experiment</Since>)
...
Until that experiment lands functional behavior, Terragrunt does not bootstrap, delete, or migrate Azure resources;
OpenTofu/Terraform's native `azurerm` backend is used as-is.
</Before>
<Since version="1.1.2">
...
<Before version="1.1.2">
Further configuration keys, authentication methods, and Terragrunt-side bootstrap behavior will be documented as the experiment matures.
</Before>
