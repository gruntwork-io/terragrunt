...
To skip every hook for a single run without editing configuration, pass [`--no-hooks`](/reference/cli/commands/run#no-hooks):
...
if you are migrating between OpenTofu and Terraform.
