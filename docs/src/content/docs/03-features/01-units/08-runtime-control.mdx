...
By default, retry rules apply to run-phase operations like hooks and OpenTofu/Terraform invocations. With the [retry-parse-errors](/reference/experiments/active#retry-parse-errors) experiment enabled, they also cover transient failures raised while parsing configuration itself, such as `sops_decrypt_file` timing out against a KMS endpoint.
