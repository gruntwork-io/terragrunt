...
Terragrunt builds the run queue before it reads dependency outputs, so don't read them in an `exclude` block. See the [`exclude-dependency-outputs`](/reference/strict-controls/active#exclude-dependency-outputs) strict control.
