...
- Docker-style `NAME:TAG` and `NAME@DIGEST` suffixes are not part of the source syntax (matching OpenTofu's behavior). Terragrunt rejects them with an error that shows the source rewritten in the `?tag=`/`?digest=` form.
