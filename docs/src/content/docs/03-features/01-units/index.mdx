...
See the [Lock File Handling docs](/features/units/lock-files) for more details.
...
Every unit that includes `env.hcl` generates the same `provider.tf` content, so Terragrunt stores that content once in the [Content Addressable Store (CAS)](/features/caching/cas) and hard links it into each unit's working directory. The linked file is read-only. If a hook or script writes to a generated file, set [`mutable = true`](/reference/hcl/blocks#generate) on its `generate` block to give each unit a writable file of its own. With `--no-cas`, Terragrunt writes generated files directly.
...
- [Lock File Handling](/features/units/lock-files) — details on how Terragrunt manages `.terraform.lock.hcl` files
