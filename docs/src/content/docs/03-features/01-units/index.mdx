...
you may be interested in the dedicated documentation on [working with multiple AWS accounts](/features/units/authentication).

## Sub-pages

| Page | Description |
|------|-------------|
| [Includes](/features/units/includes) | Reuse partial Terragrunt configurations to DRY up your configurations. |
| [State Backend](/features/units/state-backend) | Create and manage remote state backends. |
| [Extra Arguments](/features/units/extra-arguments) | Pass extra arguments to every OpenTofu/Terraform run. |
| [Authentication](/features/units/authentication) | Automate authentication workflows. |
| [Hooks](/features/units/hooks) | Execute custom code before or after running OpenTofu/Terraform. |
| [Auto-init](/features/units/auto-init) | Automatically run `init` when necessary. |
| [Runtime Control](/features/units/runtime-control) | Feature flags, error handling, and excludes. |
| [IaC Engines](/features/units/engine) | Customize how OpenTofu/Terraform runs are executed using pluggable engines. |
