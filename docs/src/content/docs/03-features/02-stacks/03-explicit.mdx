...
By default, `terragrunt stack output` fetches outputs from every unit concurrently. On large stacks this is much faster than reading them one at a time, but each fetch spawns a `tofu output` child process, so very large stacks may need a cap to avoid file-descriptor exhaustion or hitting cloud-provider rate limits.

Use the `--parallelism` flag to set the maximum number of concurrent unit fetches:

```sh
terragrunt stack output --parallelism 4
```

This mirrors how [`run --all`](/features/stacks/stack-operations/#limiting-run-parallelism) uses the same flag to bound unit execution.
