...
The `unit.vpc.path` variable resolves to the generated directory of the `vpc` unit. Running `terragrunt stack generate` writes the dependency into a `terragrunt.autoinclude.hcl` file next to the `app` unit's `terragrunt.hcl`:
...
Terragrunt merges this file into the `app` unit's configuration whenever the unit is parsed, so `run --all` orders `vpc` before `app`, and `app` receives `vpc_id` as an input. The `vpc` unit's own `terragrunt.hcl` stays untouched.

The same block can depend on a sibling stack through `stack.<name>.path`, and read that stack's outputs keyed by unit name. See [Depending on a stack](/features/stacks/stack-operations#depending-on-a-stack) for how such a dependency behaves, and the [`autoinclude` block reference](/reference/hcl/blocks#autoinclude) for the full contract: what an autoinclude body may contain, how the merge works, and stack-level autoincludes.
...
### Stacks cannot declare dependencies

A unit can [depend on a stack](/features/stacks/stack-operations#depending-on-a-stack), but a `terragrunt.stack.hcl` file has no `dependency` block of its own, and a stack-level `autoinclude` rejects one. To make every unit in a stack wait on something, declare the dependency on each unit, either in the unit source or through the unit's `autoinclude` block.
...
### Includes in `terragrunt.stack.hcl` files only add components

An `include` block in a `terragrunt.stack.hcl` file pulls in the `unit` and `stack` blocks of another file. It does not merge locals or values, and the included file cannot include anything itself. Nesting stacks remains the way to share a whole pattern. See [Includes in stack files](/reference/hcl/blocks#includes-in-stack-files).
