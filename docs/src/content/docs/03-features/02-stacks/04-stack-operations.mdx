...
`terragrunt stack output` fetches outputs from units concurrently. Each fetch spawns a `tofu output` child process. By default the number of concurrent fetches matches the number of logical CPUs; the same `--parallelism` flag (and `TG_PARALLELISM`) raises or lowers that cap when you need more throughput or want to avoid file-descriptor exhaustion and cloud-provider rate limits:
