...
To skip outputs for every `dependency` block in a run without editing configuration, pass [`--no-dependency-outputs`](/reference/cli/commands/run#no-dependency-outputs). It has the same effect as setting `skip_outputs = true` on each block, and suits commands that do not read dependency outputs:
...
When a `terragrunt.stack.hcl` file generates the units, the unit sources usually live in a catalog that knows nothing about the other units in the stack. There are two ways to wire them together.

The first is to declare the dependency in the stack file with an `autoinclude` block, so the catalog source stays generic and the stack file owns the relationship. See [Declaring dependencies between units](/features/stacks/explicit#declaring-dependencies-between-units). Prefer this when you control the stack file.

The second is to have the catalog unit declare the `dependency` block itself and read the path from `values`. This suits catalog units that always depend on the same kind of sibling.
...
Following this pattern, the path to dependencies are passed in as `values` to the unit, and units themselves define dependency blocks that use those values.
...
A `dependency` block can point at a stack directory instead of a unit. Say a `networking` stack is defined next to a hand-written `app` unit:
...
unit "subnets" {
  source = "github.com/acme/infrastructure-catalog//units/subnets?ref=v1.0.0"
  path   = "subnets"
}
```

The `app` unit depends on the whole stack and reads each unit's outputs under that unit's name:
...
Running `run --all apply` from `root` generates the `networking` stack, applies `vpc` and `subnets`, then applies `app`. The dependency expands to every unit the stack generates, so `app` waits for all of them. On `destroy`, the order flips and `app` goes first.

`mock_outputs` is keyed by unit name too. Before anything in the stack is applied, `plan` on `app` uses the mocks. Once `vpc` is applied but `subnets` is not, the aggregated outputs have a `vpc` key and no `subnets` key, so reading `subnets` fails. Add `mock_outputs_merge_strategy_with_state = "shallow"` to the dependency to fill the missing unit from its mock. See [Depending on a stack](/reference/hcl/blocks#depending-on-a-stack) for the other option.

Units generated by a nested `stack` block appear under that stack's name, e.g. `dependency.networking.outputs.nested.vpc.vpc_id`, and so do their entries in `mock_outputs`.

Two limits apply:

- The stack's units need to exist on disk before their outputs can be read. Run from a directory that contains both the stack and the dependent unit, or run `terragrunt stack generate` in the stack directory first.
- The relationship is one-way. A unit can depend on a stack, but a stack cannot declare dependencies of its own. To make every unit in a stack wait on something, add the dependency to each unit, or declare it in the stack file with an [`autoinclude` block](/reference/hcl/blocks#autoinclude).

A [`dependencies` block](/reference/hcl/blocks#dependencies) accepts stack directories in `paths` as well, for ordering without outputs. Inside a `terragrunt.stack.hcl` file, `stack.<name>.path` resolves to a sibling stack's generated directory, so an `autoinclude` block can declare the same kind of dependency without hardcoding the path. See [Depending on a sibling stack](/reference/hcl/blocks#depending-on-a-sibling-stack).
