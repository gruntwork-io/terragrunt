...
`terragrunt stack output` fetches outputs from every unit concurrently, capped by default at the number of CPU cores. Each fetch spawns a `tofu output` child process, so very large stacks may need a different cap to avoid file-descriptor exhaustion or hitting cloud-provider rate limits. The same `--parallelism` flag bounds the number of concurrent unit fetches:
