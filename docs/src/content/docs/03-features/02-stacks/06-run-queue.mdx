...
The Run Queue relies on a [Directed Acyclic Graph (DAG)](/getting-started/terminology#directed-acyclic-graph-dag) built from the dependencies defined between your Terragrunt units. These dependencies are typically established using [`dependency`](/reference/hcl/blocks#dependency) or [`dependencies`](/reference/hcl/blocks#dependencies) blocks in your `terragrunt.hcl` files. A dependency that points at a stack directory expands to one edge per unit the stack generates, so the dependent unit waits for all of them. See [Depending on a stack](/features/stacks/stack-operations#depending-on-a-stack).
