...
- Configure OpenTofu/Terraform instances to use the Terragrunt Provider Cache server as a remote registry:
  ...
  - Create local CLI config file `.terraformrc` for each module that concatenates the user configuration from the OpenTofu/Terraform [CLI config file](https://opentofu.org/docs/cli/config/config-file/) with additional sections:
    ...
    The user configuration is read from the file locations the detected implementation would use:
    ...
    - For both implementations, Terragrunt also merges the `*.tfrc` and `*.tfrc.json` fragments from the CLI config directory (`~/.terraform.d`, or `$XDG_CONFIG_HOME/opentofu` for OpenTofu when `~/.terraform.d` does not exist).
    - Setting `TF_CLI_CONFIG_FILE` overrides the config file location for both implementations.
