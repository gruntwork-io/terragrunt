...
- Configure OpenTofu/Terraform instances to use the Terragrunt Provider Cache server as a remote registry:
  ...
  - Create local CLI config file `.terraformrc` for each module that concatenates the user configuration from the OpenTofu/Terraform [CLI config file](https://opentofu.org/docs/cli/config/config-file/) with additional sections:
    ...
    The user configuration is read from the file locations the detected implementation would read. OpenTofu reads `~/.tofurc`, then `~/.terraformrc`, then `$XDG_CONFIG_HOME/opentofu/tofurc` (`%APPDATA%\tofu.rc`, then `%APPDATA%\terraform.rc` on Windows), while Terraform's only primary config file is `~/.terraformrc` (`%APPDATA%\terraform.rc` on Windows). Terragrunt also merges, for both implementations, the `*.tfrc` and `*.tfrc.json` fragments from the CLI config directory (`~/.terraform.d`, or `$XDG_CONFIG_HOME/opentofu` for OpenTofu when `~/.terraform.d` does not exist). Setting `TF_CLI_CONFIG_FILE` overrides the config file location for both implementations. The implementation is detected from the binary Terragrunt is configured to run at startup (`--tf-path`, `TG_TF_PATH`, or the first of `tofu`/`terraform` found on `PATH`); a `terraform_binary` setting inside a unit's configuration does not change which files the cache server reads, and Terragrunt warns when a run's implementation differs from the one the cache server was configured for.
  ...
  - Set environment variables:
    ...
    - [`TF_TOKEN_*`](https://opentofu.org/docs/cli/config/config-file/#environment-variable-credentials) sets per-remote-registry tokens for authentication to Terragrunt Provider Cache server.
