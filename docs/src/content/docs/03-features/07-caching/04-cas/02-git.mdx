...
1. Branch and tag refs use a shallow fetch. A full commit SHA is asked for on its own, one commit deep, which transfers that commit rather than every branch and tag. Remotes that will not serve a commit by name, such as an older or locked-down server, fall back to the full-history fetch, so a pinned SHA resolves either way.
