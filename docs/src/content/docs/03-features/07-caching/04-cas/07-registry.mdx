...
The probe Terragrunt uses to attempt to avoid fetching tfr sources is the registry's service-discovery and module-download protocol against the registry host. When the URL omits a host (the `tfr:///` form), the default registry is used. The probe requires an explicit `version` in the URL.<Since version="v1.2.0"> A [`version`](/reference/hcl/blocks#terraform) constraint on the `terraform` block satisfies this, because Terragrunt resolves it to an exact version before the probe runs.</Since>
