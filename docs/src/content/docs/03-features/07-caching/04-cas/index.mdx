...
Files produced by [`generate`](/reference/hcl/blocks#generate) blocks are stored in the CAS too. Terragrunt hard links the stored content read-only into each working directory, so units generating identical content share one file. A `generate` block with `mutable = true` gets an ordinary writable file of its own. The backend file generated by a `remote_state` block is written directly and is not stored in the CAS.
