...
When you need an editable working copy, set `mutable = true`. <Before version="1.1.5">Terragrunt then copies the content into the target with its write permissions intact rather than hard linking it, producing a tree that is independent of the store and safe to modify. This trades the disk savings of shared inodes for a writable copy, since each `mutable` target keeps its own.</Before><Since version="1.1.5">Terragrunt then gives the target a file of its own, with its write permissions intact, instead of hard linking the stored one.</Since> The attribute is available on the `terraform` block, and on the `unit` and `stack` blocks of a stack configuration.
...
A [`mutable = true`](#immutable-by-default) source on a filesystem with no copy-on-write clone is copied the same way.
...
A [`mutable = true`](#immutable-by-default) source cannot share the stored file. Where the filesystem offers reflinks, Terragrunt clones it, so the target shares the stored content except for the parts you edit and is writable from the start.
