...
The boundary must be an existing directory. Terragrunt resolves a relative path against the working directory. A dependent-side boundary must be the working directory or one of its parents, since the search for dependents starts there. A dependency-side boundary can be any directory.
