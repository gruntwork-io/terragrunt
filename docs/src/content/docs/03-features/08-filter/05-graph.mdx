...
The boundary must be an existing directory. Terragrunt resolves a relative path against the working directory. A dependent-side boundary must be the working directory, one of its parents, or a directory inside it. The search for dependents starts at the working directory, or at the boundary when the boundary is inside the working directory. A dependency-side boundary can be any directory. For [Git-based expressions](/features/filter/git), a dependent-side boundary applies to both compared commits, so Terragrunt ignores a changed unit outside it. A dependency-side boundary only bounds traversal and does not hide changed units.
