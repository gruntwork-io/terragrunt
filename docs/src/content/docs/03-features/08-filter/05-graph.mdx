...
For [Git-based expressions](/features/filter/git), Terragrunt resolves a relative boundary against the repository root instead. `(./live/staging)...[main...HEAD]` means `live/staging` from any directory. The boundary does not have to contain the working directory, and it only has to exist in the compared commits. Terragrunt ignores a changed unit outside a dependent-side boundary in both commits. A dependency-side boundary only limits dependency traversal, so it never hides a changed unit.
