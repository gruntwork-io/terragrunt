...
The boundary must be an existing directory. Terragrunt resolves a relative path against the working directory. A dependent-side boundary must be the working directory, one of its parents, or a directory inside it. The search for dependents starts at the working directory, or at the boundary when the boundary is inside it. A dependency-side boundary can be any directory. For [Git-based expressions](/features/filter/git), the boundary also limits discovery in the compared worktrees, so a changed unit outside the boundary is not a target.
