...
With the [`canonical-worktree-paths` experiment](/reference/experiments/active#canonical-worktree-paths) enabled, a unit discovered in a worktree whose configuration still exists in your working directory is instead selected, parsed, and run at its working-directory path. Removed units keep their worktree paths so destroys keep working.
