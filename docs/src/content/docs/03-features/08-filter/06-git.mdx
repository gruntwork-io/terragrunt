...
With the [`canonical-worktree-paths` experiment](/reference/experiments/active#canonical-worktree-paths) enabled, a unit discovered in a worktree that still exists at the compared "to" reference and in your working directory is instead selected, parsed, and run at its working-directory path. Removed units keep their worktree paths so destroys keep working, even when an untracked file occupies the removed unit's directory.
