...
1. `terragrunt.values.hcl` file, if present (values referencing `dependency` outputs receive placeholder values in this pass)

2. `include` block

3. `locals` block

4. Evaluation of values for `iam_role`, `iam_assume_role_duration`, `iam_assume_role_session_name`, and `iam_web_identity_token` attributes, if defined

5. `dependencies` block

6. `dependency` blocks, including calling `terragrunt output` on the dependent units to retrieve the outputs

7. `terragrunt.values.hcl` file is re-read if `dependency` blocks were resolved, replacing placeholder values with real dependency outputs

8. Everything else

9. The config referenced by `include`

10. A merge operation between the config referenced by `include` and the current config.
...
Note that `terragrunt.values.hcl` files in unit contexts (`terragrunt.hcl`) can reference `dependency` outputs (e.g. `dependency.vpc.outputs.vpc_id`). Values files in stack contexts (`terragrunt.stack.hcl`) cannot reference `dependency` blocks.
