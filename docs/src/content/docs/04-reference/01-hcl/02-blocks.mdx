...
In unit contexts (directories with a `terragrunt.hcl`), the values file can also reference `dependency` outputs from the unit's configuration:
...
Dependency references in values files are resolved during the normal config parsing flow — values are read once before dependency resolution (with placeholders for any `dependency` references), then re-read after dependencies are resolved with real outputs. Values files in stack contexts (`terragrunt.stack.hcl`) do not support `dependency` references.
