...
- `disable_signature` (attribute): When `true`, disables including a signature in the generated file. This means that
  ...
  `overwrite_terragrunt_or_skip`, files generated without a signature will not be recognizable as Terragrunt-owned
  in future runs, so they will be treated as non-Terragrunt files and skipped. Defaults to `false`. Optional.
