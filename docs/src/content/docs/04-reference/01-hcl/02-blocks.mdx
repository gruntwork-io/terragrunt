...
```bash
terragrunt run --all apply --filter '[main...HEAD]'
```
...
A JSON configuration (`terragrunt.hcl.json`) renders the same way. `render` writes the block as the HCL that means the same thing and previews the elements under it. `terragrunt render --format json` has nowhere to put a comment, so it emits the block as written, with its `expansion` block and references. Rendering either output again returns it unchanged. The [`render` command reference](/reference/cli/commands/render#expanded-dependencies) has examples of both.
