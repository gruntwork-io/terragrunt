...
- `source` (attribute): Specifies where to find OpenTofu/Terraform configuration files. This parameter supports the same syntax as the
  ...
  - The `tfr` protocol supports a shorthand notation where the `REGISTRY_HOST` can be omitted to default to the public
    registry. The default registry depends on the wrapped executable: for OpenTofu, it is `registry.opentofu.org`,
    and for Terraform, it is `registry.terraform.io`.<Since version="v1.2.0"> When Terragrunt cannot tell which of the
    two the executable is, it uses `registry.opentofu.org`.</Since> Additionally, if the environment variable `TG_TF_DEFAULT_REGISTRY_HOST`
...
> **Note:**
> Only one `ignore` or `retry` block handles an error, even when several of them match it.
> Within a block, a matching negative pattern keeps the error from being ignored or retried, wherever that pattern sits in the list.
