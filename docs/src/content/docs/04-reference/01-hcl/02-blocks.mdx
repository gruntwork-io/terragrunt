...
- `source` (attribute): Specifies where to find OpenTofu/Terraform configuration files. This parameter supports the same syntax as the
  ...
  - The `source` parameter can pull modules from OCI registries using the `oci` protocol, in the format
    ...
    `latest` tag is used. <Before version="v1.2.0">This requires the [`oci` experiment](/reference/experiments/active#oci). </Before>See
...
- `version` (attribute): A version constraint used to resolve a `tfr://` registry module, as an alternative to pinning an exact version in the source URL.
  ...
  - The `--source` and `TG_SOURCE` overrides replace the `version` attribute along with the source. They accept an exact version pin only (for example `?version=3.4.0`); a constraint there is an error.
  - Terragrunt resolves the constraint against the registry on every run. When a newly published version satisfies the constraint, the next run downloads it, with no `--source-update` needed.
...
- `include_in_copy` (attribute): A list of glob patterns (e.g., `["*.txt"]`) that should always be copied from the same directory containing `terragrunt.hcl` into the
  ...
  - With the [`symlinks` experiment](/reference/experiments/active#symlinks) enabled, a pattern may name a symlinked directory, and the contents of the link target are copied into the working directory. A link that points back at a directory already being copied, or at a parent of one, is skipped, and Terragrunt logs a warning naming it.
...
- `copy_terraform_lock_file` (attribute): In certain use cases, you don't want to check the terraform provider lock
  ...
  [Lock File Handling](/features/units/lock-files). This attribute allows you to disable the copy
...
- `generate` (attribute): Configure Terragrunt to automatically generate a `.tf` file that configures the remote state
  ...
  - `if_exists` (attribute): What to do if a file already exists at `path`.
    ...
    - `overwrite_terragrunt_or_skip` (overwrite the existing file if it was generated by terragrunt; otherwise, leave it as-is; requires the `overwrite-terragrunt-or-skip` experiment)
...
- `skip_bucket_root_access`: *DEPRECATED* If provided, will be ignored. A log warning will be issued in the console output when Terragrunt bootstraps the backend. Enable the [`skip-bucket-root-access`](/reference/strict-controls/active#skip-bucket-root-access) strict control to turn that warning into an error.
- `enable_bucket_root_access`: When `true`, Terragrunt adds a bucket policy statement granting `s3:*` on the bucket and its objects to `arn:aws:iam::<account-id>:root`. Despite that ARN's name, a resource policy naming it grants access to the AWS account as a whole, not only to its root user. Terragrunt does not add this statement by default, and adds it to buckets that already exist as well as to ones it creates.
...
- `skip_accesslogging_bucket_policy`: When set to `true`, the S3 bucket where access logs are stored will not be configured with a bucket policy granting S3 permission to deliver access logs to it.
- `skip_accesslogging_bucket_acl`: *DEPRECATED* Opted out of the bucket ACL that used to carry this grant, an approach [AWS recommends against](https://docs.aws.amazon.com/AmazonS3/latest/userguide/enable-server-access-logging.html#grant-log-delivery-permissions-general). It now has no effect, since Terragrunt sets no ACL. Use `skip_accesslogging_bucket_policy` to skip the grant. Terragrunt warns when it is used. See the [`skip-accesslogging-bucket-acl`](/reference/strict-controls/active#skip-accesslogging-bucket-acl) strict control.
...
### Includes in stack files

A `terragrunt.stack.hcl` file can also declare `include` blocks. The included file adds its `unit` and `stack` blocks to the stack, so a set of components shared by several stacks can live in one file:
...
A relative `path` resolves against the directory of the stack file that declares the include. The included file is evaluated with the including stack's `local` and `values` in scope. It may declare only `unit` and `stack` blocks. A `locals` block or a nested `include` in the included file fails generation, and so does a unit or stack name that collides with one in the including file. The `expose` and `merge_strategy` attributes are not accepted here.
...
- `config_path` (attribute): Path to a Terragrunt unit (folder with a `terragrunt.hcl` file) that should be included
  as a dependency in this configuration. The path can also point at a stack (folder with a `terragrunt.stack.hcl` file). See [Depending on a stack](#depending-on-a-stack).
...
A `dependency` block's `config_path` can point at a directory that contains a `terragrunt.stack.hcl` file. The dependency then reads the outputs of every unit the stack generates.
...
The `networking` stack creates a VPC with a public and a private subnet. Both subnets come from the same `subnet` unit, which outputs the subnet's `id`, `cidr_block`, and `availability_zone`:

```hcl
# networking/terragrunt.stack.hcl
...
unit "public" {
  source = "github.com/acme/infrastructure-catalog//units/subnet?ref=v1.0.0"
  path   = "public"
...
Generating the stack creates a directory for each unit under `networking/.terragrunt-stack`:
...
`app` reads each unit's outputs under the unit's name in the stack file:
...
inputs = {
  ...
  public_subnet_id  = dependency.networking.outputs.public.id
  private_subnet_id = dependency.networking.outputs.private.id
}
```

#### Output addresses

A stack dependency reads each unit at the same address `terragrunt stack output` gives it. Once `networking` is applied, running `terragrunt stack output` in `networking` prints one entry per unit:
...
`dependency.networking.outputs` has the same shape, so `dependency.networking.outputs.public.id` is `"subnet-11111111111111111"` and `dependency.networking.outputs.vpc.cidr_block` is `"10.0.0.0/16"`.

Now disable the `private` unit:
...
Terragrunt no longer generates `private`, so its entry is gone from the output:

<Before version="1.2.0">

```bash
$ terragrunt stack output --experiment block-iteration
public = {
  availability_zone = "us-east-1a"
  cidr_block        = "10.0.1.0/24"
  id                = "subnet-11111111111111111"
...
`dependency.networking.outputs` loses the `private` attribute the same way, so `app` fails with an `Unsupported attribute` error on `dependency.networking.outputs.private.id`. The dependency also leaves out a unit that has no outputs.

An [expanded](#expansion) unit nests its elements under their keys. Here a `subnets` unit expands into one subnet per availability zone:
...
`terragrunt stack output` prints each element under `subnets`, keyed by availability zone:
...
`app` reaches each element by its key:
...
inputs = {
  vpc_id      = dependency.networking.outputs.vpc.id
  subnet_a_id = dependency.networking.outputs.subnets["us-east-1a"].id
  subnet_b_id = dependency.networking.outputs.subnets["us-east-1b"].id
}
```

#### Nested stacks

A nested `stack` block adds a level named after the stack. Here `networking` creates its subnets through a nested `subnets` stack:
...
stack "subnets" {
  source = "github.com/acme/infrastructure-catalog//stacks/subnets?ref=v1.0.0"
  path   = "subnets"

  values = {
    availability_zone  = "us-east-1a"
    public_cidr_block  = "10.0.1.0/24"
    private_cidr_block = "10.0.101.0/24"
  }
}
```

```hcl
# stacks/subnets/terragrunt.stack.hcl in the catalog
...
unit "public" {
  ...
  values = {
    ...
    cidr_block        = values.public_cidr_block
  }
}
...
unit "private" {
  ...
  values = {
    availability_zone = values.availability_zone
    cidr_block        = values.private_cidr_block
  }
}
```

The nested stack generates its own `.terragrunt-stack` directory inside `networking/.terragrunt-stack/subnets`:
...
`terragrunt stack output` prints the units of `subnets` under the stack's name:
...
`app` reads them through the same name:
...
inputs = {
  vpc_id            = dependency.networking.outputs.vpc.id
  public_subnet_id  = dependency.networking.outputs.subnets.public.id
  private_subnet_id = dependency.networking.outputs.subnets.private.id
}
```

Each further level of nesting adds another name. A nested stack is left out when it has `enabled = false`, has not been generated, or has no units with outputs. An expanded nested stack keys its elements the same way as an expanded unit. Here `subnets` expands into one nested stack per availability zone:
...
stack "subnets" {
  ...
  values = {
    availability_zone  = each.key
    public_cidr_block  = each.value.public
    private_cidr_block = each.value.private
  }
}
```

`terragrunt stack output` prints the `public` and `private` units of each element under its key:
...
`app` reaches a unit in one element through the element's key, then the unit's name:
...
inputs = {
  vpc_id             = dependency.networking.outputs.vpc.id
  public_subnet_a_id = dependency.networking.outputs.subnets["us-east-1a"].public.id
  public_subnet_b_id = dependency.networking.outputs.subnets["us-east-1b"].public.id
}
```

<Aside type="caution" title="Name collisions">
  A unit and a nested stack with the same name in one stack file share an address. A dependency on that stack fails once both have outputs, so give one of them a different name.
</Aside>

#### Mock outputs

`mock_outputs` for a stack dependency uses the same addresses as its outputs, with a map or object at each level. For the nested `subnets` stack:
...
dependency "networking" {
  ...
  mock_outputs = {
    ...
    subnets = {
      public = {
        id = "subnet-public-mock"
      }
      private = {
        id = "subnet-private-mock"
      }
    }
  }
}
...
When no unit in the stack has outputs yet, the whole `mock_outputs` value stands in, as it does for a unit dependency. In a partially applied stack, the units that have not been applied are missing from the outputs, and reading them fails. Set `mock_outputs_merge_strategy_with_state` to fill them from `mock_outputs`:

- `shallow` fills a missing top-level unit or nested stack. It does not fill a unit missing from a nested stack that has some outputs, e.g. `subnets.private` when only `subnets.public` is applied.
- `deep_map_only` fills missing units at every level.

<Before version="1.2.0">

Running with [`--dependency-fetch-output-from-state`](/reference/cli/commands/run#dependency-fetch-output-from-state) on a backend that supports it also fills them. A unit whose state object is missing uses its own `mock_outputs` entry when mocks are allowed for the command, and a unit with no entry is left out. When mocks are not allowed, the dependency fails and names that unit.

</Before>

<Since version="1.2.0">

Terragrunt reads dependency outputs from state by default on a backend that supports it, and that also fills them. A unit whose state object is missing uses its own `mock_outputs` entry when mocks are allowed for the command, and a unit with no entry is left out. When mocks are not allowed, the dependency fails and names that unit. [`--no-dependency-fetch-output-from-state`](/reference/cli/commands/run#no-dependency-fetch-output-from-state) turns this off.

</Since>

#### Run order

In the [run queue](/features/stacks/run-queue), a dependency on a stack becomes a dependency on every unit the stack generates, including units in nested stacks, so `app` waits for all of them. `terragrunt find --dependencies` and `terragrunt list --tree --dag` show these edges. A unit can depend on a stack, but a stack cannot declare dependencies of its own.

Terragrunt reads outputs only from generated units. `run --all` generates the stacks under its working directory before running, so run it from a directory that contains both the stack and the dependent unit, or run `terragrunt stack generate` in the stack directory first.
...
- `paths` (attribute): A list of paths to units that should be marked as a dependency. A path to a stack directory expands to every unit that stack generates, as described in [Depending on a stack](#depending-on-a-stack).
...
- `if_exists` (attribute): What to do if a file already exists at `path`.
  ...
  An additional value is available:

  - `overwrite_terragrunt_or_skip` (overwrite the existing file if it was generated by terragrunt; otherwise, leave it as-is)

  <Aside type="tip" title="Experimental">
    The `overwrite_terragrunt_or_skip` value is gated behind the `overwrite-terragrunt-or-skip` experiment. Enable it with `--experiment overwrite-terragrunt-or-skip` (or `TG_EXPERIMENT=overwrite-terragrunt-or-skip`) until it graduates to general availability. See the [experiment documentation](/reference/experiments/active#overwrite-terragrunt-or-skip).
  </Aside>
...
Reach a `count` instance by its index, e.g. `dependency.shard[0]`.

<Aside type="note" title="Number keys">
  Instance keys are strings. Terragrunt converts a number index to a string, so `dependency.shard[0]` and `dependency.shard["0"]` reach the same instance. The same applies to a `for_each` over a set of numbers.
</Aside>
...
A stack file can declare an expanded `dependency` inside the [`autoinclude`](#autoinclude) block of a `unit`, and a `unit` or `stack` block can declare both `expansion` and `autoinclude`. See the `autoinclude` reference for how generation writes each.
...
```bash
terragrunt stack output 'aurora["web"].role'
terragrunt stack output 'shard[0].role'
```
...
<Aside type="tip" title="Optional components">
  To make a component optional, prefer `enabled` to `count`.

  If you have been using OpenTofu/Terraform for a while, you might have a habit of using `count` to control the presence of resources. That was the only way to make resources conditional for a good portion of their histories. OpenTofu introduced the `enabled` attribute to improve the ergonomics of making resources conditional, and Terragrunt adopted it in the design of resource expansion for the same reason.

  An `expansion` block with a `count` changes the address of a conditional resource to `canary[0]` when the resource is present. This causes unnecessary churn in configurations and makes it harder to introduce conditionality without forcing consumers to adopt a breaking change.
</Aside>

A disabled `unit` or `stack` is skipped during stack generation and does not appear in `terragrunt stack output`.
...
Generating this stack creates `.terragrunt-stack/app` and no `canary` directory. After an apply, the output lists only `app`:
...
You can use both `enabled` _and_ `expansion` in the same configuration block. `enabled` is evaluated once per element, so `each.key` and `count.index` are available to it and a single block can expand into a collection with some elements dropped:
...
- `for_each` accepts a set, map, or object. Wrap a list in `toset()` to iterate it.
- Element keys must be strings or numbers. Numbers are stringified, so `for_each = toset([1, 2])` is equivalent to `for_each = toset(["1", "2")]`.
- `count` must be a non-negative integer.
- `for_each` and `count` expressions have to resolve to known, non-null values while the configuration is being parsed. As a result, dependency outputs cannot be used to set `for_each` and `count` values.
- A single block expands into at most 1,000,000 instances.
- Unit and stack addresses must be unique after expansion, and so must their generated paths.

An `expansion` block with a zero length `for_each` value or a `count = 0` produces no instances. A stack file whose blocks all resolve to nothing stays valid: Terragrunt logs that there is nothing to generate and moves on. Only a stack file that declares no `unit` or `stack` block at all is an error.

### Prefer `for_each` over `count`

`count` addresses instances by position, so removing an element shifts every element after it. This can result in surprises, as keys don't remain stable. You might intend to remove the second item in a `count = 3`, and see a plan to update the second instance to the properties of the third element and remove the third. Keys under `for_each` come from the collection itself, so the elements you left alone keep the addresses they had.

Use `count` when the instances are interchangeable and you only ever add to or remove from the end of the set.

This stack expands the same list of regions with `count`:
...
It generates `app/0` for `us-east-1`, `app/1` for `us-west-2`, and `app/2` for `eu-west-1`.

Removing `"us-west-2"` from the list leaves two instances: `app/0` stays on `us-east-1`, `app/1` switches to `eu-west-1`, and `app/2` is orphaned. A run then updates the infrastructure in `app/1` to `eu-west-1`, and the `eu-west-1` infrastructure in `app/2` still needs destroying.

With `for_each`, each instance keeps its region as its key:
...
Removing `"us-west-2"` orphans only `app/us-west-2`. `app/us-east-1` and `app/eu-west-1` are unchanged.

### Lifecycle

Adding an `expansion` block to an existing block changes its address. Shrinking a `for_each` or lowering a `count` removes addresses. Terragrunt has no equivalent of OpenTofu's `moved` block, so you handle each change yourself.

Removing an instance leaves its infrastructure deployed. To destroy it, see [Destroying removed components](/features/filter/git#destroying-removed-components).

Cleaning up components within `.terragrunt-stack` that are no longer generated by `terragrunt.stack.hcl` due to a change in expansion does not happen by default. Use `--source-update` to explicitly request that the `.terragrunt-stack` is cleaned before regenerating.

### Selecting instances

`--filter` does not accept keyed addresses like `aurora["web"]`, because `[` starts a Git expression. Select one element with a [path expression](/features/filter/path) for its generated directory:

```bash
terragrunt run --all --filter './.terragrunt-stack/aurora/web' -- apply
```

<Aside type="note" title="Keyed addresses">
  A future release may add a way to select an instance by its keyed address. Until then, use its generated path.
</Aside>

### `render` output is a preview

`terragrunt render` shows an expanded block as it was written, followed by the elements it expands into, commented out:
...
The elements stay commented out because they all repeat the block's label, which a Terragrunt configuration is not allowed to do. Terragrunt only recognizes the configurations of the block above them. These comments are here just to help you understand how the block will be expanded internally when parsed by Terragrunt.

JSON configurations (`terragrunt.hcl.json`) render HCL the same way. `render` writes the block as the equivalent HCL to the JSON in `terragrunt.hcl.json` and previews the elements under it. `terragrunt render --format json` has nowhere to put a comment, so it emits the block as written, with its `expansion` block and references. Rendering either output again returns it unchanged.

The [`render` command reference](/reference/cli/commands/render#expanded-dependencies) has examples of both.
...
`autoinclude` works with [`expansion`](#expansion). When the `unit` or `stack` block declares an `expansion`, each element gets its own generated file, and `each.key`, `each.value`, and `count.index` in the autoinclude body resolve to that element.

A `dependency` block in the body can declare its own `expansion` block. Generation evaluates `for_each` or `count` in the stack file's context and writes the result into the generated `expansion` block as a literal. `config_path` resolves once per element and is written as a lookup by `each.key` or `count.index`. Any other `each.*` or `count.index` reference inside the dependency block is kept verbatim, and the generated unit expands the dependency again when it is parsed:
...
When the `unit` block and the `dependency` block are both expanded, the dependency's `for_each` or `count` expression sees the unit's element. Everywhere else in the dependency block, `each` and `count` refer to the dependency's own element.
...
An [expanded](#expansion) component exposes one path per element, under the key its [address](#addressing-expanded-units-in-stack-output) uses: `unit.vpc["us-east-1"].path` for a `for_each`, or `unit.shard[0].path` for a `count`. A label declared both with and without an `expansion` fails the parse, since `unit.<name>` cannot refer to both.
...
The merge applies to any unit with a sibling `terragrunt.autoinclude.hcl`. `stack generate` normally writes this file; treat it as a generated artifact rather than something to edit by hand.

### Depending on a sibling stack

A dependency's `config_path` can point at a sibling stack via `stack.<name>.path`. The generated dependency behaves as described in [Depending on a stack](#depending-on-a-stack): outputs aggregate every unit the stack generates, keyed by unit and nested stack name.
...
Here the `network` stack contains a unit named `vpc` with a `vpc_id` output, so the dependent unit reads it as `dependency.network.outputs.vpc.vpc_id`.
