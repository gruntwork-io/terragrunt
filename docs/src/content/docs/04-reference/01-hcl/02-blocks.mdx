...
- `source` (attribute): Specifies where to find OpenTofu/Terraform configuration files. This parameter supports the same syntax as the
  ...
  - The `source` parameter can also pull modules from any OCI-compliant registry (e.g., GitHub Container Registry,
    Amazon ECR, Docker Hub, self-hosted registries) using the `oci://` scheme. The format is:
    `oci://<registry>/<repository>:<tag-or-digest>[//<subdir>]`. For example:
    ```hcl
    # Pull a module from GHCR
    source = "oci://ghcr.io/my-org/terraform-modules:v1.2.3"

    # Pull a submodule using the // subdir selector
    source = "oci://ghcr.io/my-org/terraform-modules:v1.2.3//modules/vpc"
    ```
    OCI artifacts must be packaged as a gzip-compressed tar archive. Use [ORAS](https://oras.land/) to push:
    ```bash
