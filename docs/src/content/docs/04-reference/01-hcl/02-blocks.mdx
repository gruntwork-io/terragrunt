...
- `generate` (attribute): Configure Terragrunt to automatically generate a `.tf` file that configures the remote state
  ...
  - `if_exists` (attribute): What to do if a file already exists at `path`.
    ...
    - `overwrite_terragrunt_or_skip` (overwrite the existing file if it was generated by terragrunt; skip silently if it was not) requires the [`overwrite-terragrunt-or-skip`](/reference/experiments/active#overwrite-terragrunt-or-skip) experiment
...
- `disable_signature` (attribute): When `true`, disables including a signature in the generated file. This means that
  there will be no difference between `overwrite_terragrunt` and `overwrite` for the `if_exists` setting. For
  `overwrite_terragrunt_or_skip`, disabling the signature means Terragrunt cannot recognize files it previously
  generated, so all existing files will be treated as non-Terragrunt files and skipped. Defaults to `false`. Optional.
