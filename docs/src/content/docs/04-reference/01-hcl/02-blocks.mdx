...
- `include_in_copy` (attribute): A list of glob patterns (e.g., `["*.txt"]`) that should always be copied from the same directory containing `terragrunt.hcl` into the
  ...
  - With the [`symlinks` experiment](/reference/experiments/active#symlinks) enabled, a pattern may name a symlinked directory, and the contents of the link target are copied into the working directory. A link that points back at a directory already being copied is copied as an empty directory instead of being followed again.
