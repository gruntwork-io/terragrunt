...
- `source` (attribute): Specifies where to find OpenTofu/Terraform configuration files. This parameter supports the same syntax as the
  ...
  - The `version` query parameter is optional. When omitted, Terragrunt queries the registry's list-versions endpoint
    and downloads the latest stable (non-prerelease) version. Pin a version explicitly for reproducible builds in
    production.
