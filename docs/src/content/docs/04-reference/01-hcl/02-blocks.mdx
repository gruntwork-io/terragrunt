...
A `dependency` block's `config_path` can point at a directory that contains a `terragrunt.stack.hcl` file. Given a `networking` stack that declares `vpc` and `subnets` units:
...
inputs = {
  ...
  subnet_ids = dependency.networking.outputs.subnets.ids
}
```

The outputs are keyed by the unit's name in the stack file, so `dependency.networking.outputs.vpc.vpc_id` reads the `vpc_id` output of the `vpc` unit, the same address `terragrunt stack output` gives it. A unit with `enabled = false` is left out, and so is a unit that has no outputs. An [expanded](#expansion) unit nests its elements under their keys: `dependency.networking.outputs.aurora["web"].id`.

A nested `stack` block adds one level, named after the stack, so `dependency.networking.outputs.nested.vpc.vpc_id` reads the `vpc` unit of a `nested` stack inside `networking`. That is the address `terragrunt stack output` gives it. A nested stack whose units have no outputs is left out, and `mock_outputs` entries for its units nest under the stack's name the same way.

`mock_outputs` for a stack dependency follows the same shape, keyed by unit name, and must be a map or object. When no unit in the stack has outputs yet, the whole `mock_outputs` value stands in, as it would for a unit dependency. A partially applied stack needs one more setting, because the units without outputs are missing from the aggregated result:

- Set `mock_outputs_merge_strategy_with_state = "shallow"` to fill each missing unit from its `mock_outputs` entry.
- Or run with [`--dependency-fetch-output-from-state`](/reference/cli/commands/run#dependency-fetch-output-from-state) on a backend that supports it. A unit whose state object is missing then uses its own `mock_outputs` entry when mocks are allowed for the command. A unit with no entry is left out. When mocks are not allowed, the dependency fails and names that unit.

In the [run queue](/features/stacks/run-queue), a dependency on a stack expands to a dependency on every unit the stack generates, including units in nested stacks, so the dependent unit waits for all of them. `terragrunt find` and `terragrunt list` show the expanded edges. The relationship is one-way. A unit can depend on a stack, but a stack cannot declare dependencies of its own.

The stack's units have to exist on disk before their outputs can be read. `run --all` generates stacks under its working directory before running, so run it from a directory that contains both the stack and the dependent unit, or run `terragrunt stack generate` in the stack directory first.
...
<Aside type="tip" title="Optional components">
  To make a component optional, prefer the `enabled` over the `count` attribute.
...
A dependency's `config_path` can point at a sibling stack via `stack.<name>.path`. The generated dependency behaves as described in [Depending on a stack](#depending-on-a-stack): outputs aggregate every unit the stack declares, keyed by unit name.
