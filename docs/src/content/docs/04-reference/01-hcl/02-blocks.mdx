...
Only units the stack file declares directly are aggregated. Units generated by a nested `stack` block are not, so to read their outputs, point a separate `dependency` at the nested stack's generated directory.
