...
`if`, `no_run` and `exclude_dependencies` also accept the strings `"true"` and `"false"`.

<Since version="v1.2.0">

Terragrunt doesn't read dependency outputs while it discovers units, for example in `run --all`, `find --dag` or `list --dag`. Discovery skips an `exclude` block that reads a dependency output, directly or through a function such as `tostring()`, and logs a warning that names the file.
