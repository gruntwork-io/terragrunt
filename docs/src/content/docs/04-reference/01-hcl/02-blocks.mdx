...
- `source` (attribute): Specifies where to find OpenTofu/Terraform configuration files. This parameter supports the same syntax as the
  ...
  - The `source` parameter can also pull modules from any OCI-compliant registry (e.g., GitHub Container Registry,
    Amazon ECR, Docker Hub, self-hosted registries) using the `oci://` scheme. Two URL notations are supported:
    the standard OCI image reference (`oci://registry/repo:tag`) and the OpenTofu-style query parameter syntax
    (`oci://registry/repo?tag=TAG`). For example:
    ```hcl
    # OpenTofu module packaging spec notation (recommended for cross-tool compatibility)
    ...
    ```
    Supported archive formats: ZIP (`archive/zip`, per the OpenTofu module packaging spec), gzip-compressed
    tar (`application/vnd.oci.image.layer.v1.tar+gzip`), and legacy Terraform tar format
    (`application/vnd.terraform.module.v1+tar`). Use [ORAS](https://oras.land/) to push:
    ```bash
    # OpenTofu module packaging spec (zip, recommended)
    ...
    ```
    Credentials are resolved in order: `TG_OCI_TOKEN` (Bearer token),
    `TG_OCI_USERNAME`+`TG_OCI_PASSWORD` (Basic auth), Docker credential store (`~/.docker/config.json`),
    then unauthenticated (public registries). The `oci://` scheme is also supported in `unit {}` and
    `stack {}` source blocks in `terragrunt.stack.hcl`, including when `update_source_with_cas = true` is set.
