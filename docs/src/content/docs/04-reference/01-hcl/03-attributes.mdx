...
When unset, Terragrunt assumes a one-hour session (`3600` seconds). Assumed credentials are cached for the effective session duration within the same process, so a later step such as `--json-out-dir` plan export can reuse them.
