...
When unset, Terragrunt defaults the STS session to one hour (`3600` seconds). In the same process, it reuses cached credentials until five minutes before they expire, so later steps such as `--json-out-dir` plan export can reuse the first role assumption.
