...
All [OpenTofu built-in functions](https://opentofu.org/docs/language/functions/) through OpenTofu 1.13 are supported in Terragrunt config files:
...
These functions are `internal` to the OpenTofu codebase, so Terragrunt keeps its own copy of their implementations and follows OpenTofu releases.

Terragrunt provides functions of its own for problems specific to Terragrunt, e.g. executing arbitrary shell commands with [run\_cmd](#run_cmd) in whatever language you like.
...
Example with multiple JSON files (`...` expands a list into positional arguments):
...
locals {
  filename   = mark_as_read("/path/to/my/file-read-by-tofu.txt")
  many_files = [for f in fileset("./config", "*.yaml") : file("${get_terragrunt_dir()}/config/${f}")]
}
...
`fileset` and `file` record the files they read, so `mark_as_read` is needed only for a file Terragrunt does not read itself, such as `file-read-by-tofu.txt` above.
...
`base64gzip_compat(str)` returns the value `base64gzip(str)` returned in Terragrunt v1.1.3 and earlier, and keeps returning it <Before version="v1.2.0">after Terragrunt 1.2 switches</Before><Since version="v1.2.0">now that Terragrunt 1.2 has switched</Since> `base64gzip()` to the current Go encoder.

`base64gzip_compat` requires the [`base64gzip-compat`](/reference/experiments/active#base64gzip-compat) experiment. Calling it without the experiment enabled returns an error.

Terragrunt v1.1.4 was built with Go 1.27, which changed the compressed bytes its gzip encoder produces. The bytes still decompress to the same content, but a resource that compares the encoded value, such as an EC2 instance with `user_data_base64` and `user_data_replace_on_change = true`, sees a change and plans a replacement. Use this function where the encoded value must stay stable across Terragrunt upgrades:
...
<Before version="v1.2.0">
By default, `base64gzip()` returns the same bytes. Enable the [`legacy-base64gzip`](/reference/strict-controls/active#legacy-base64gzip) strict control to use the current encoder before Terragrunt 1.2 makes that behavior the default.
</Before>

<Since version="v1.2.0">
Since Terragrunt 1.2, `base64gzip()` returns the current Go encoder's bytes. Call `base64gzip_compat()` where the encoded value must not change.
</Since>