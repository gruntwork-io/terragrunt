...
`base64gzip_compat(str)` returns the value `base64gzip(str)` returned in Terragrunt v1.1.3 and earlier, and keeps returning it <Before version="1.2.0">after Terragrunt 1.2 switches</Before><Since version="1.2.0">now that Terragrunt 1.2 has switched</Since> `base64gzip()` to the current Go encoder.
