...
The `base64gzip_compat` function ships with v1.1.5.
...
`base64gzip_compat` requires the [`base64gzip-compat`](/reference/experiments/active#base64gzip-compat) experiment. Calling it without the experiment enabled returns an error. The name may still change to match OpenTofu.
