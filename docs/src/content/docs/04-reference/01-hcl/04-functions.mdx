...
A relative `config_path` resolves against the directory of the configuration file containing the call, which for a
configuration pulled in by an `include` block is that file's own directory.
...
A relative `file_path` resolves against the directory of the configuration file containing the call, which for a
configuration pulled in by an `include` block is that file's own directory.
...
A relative path is resolved against the directory of the configuration file containing the call, the same directory
`file` reads it from. When the call lives in a configuration pulled in by an `include` block, that is the included
file's own directory, not the directory of the unit doing the including:

```hcl
# common/included.hcl, pulled in by an include block

locals {
  # Resolves to common/settings.json, the file that `file` reads.
  settings = jsondecode(file(mark_as_read("settings.json")))
}
```
...
Relative patterns are resolved against the directory of the configuration file containing the call, the same way `mark_as_read` resolves a relative file path.
...
Pass a leading `--terragrunt-boundary` argument to set the boundary explicitly. Use it to scope the walk to a subdirectory, or to widen it when a scan outside the repository root is intended. A relative value resolves against the directory of the configuration file containing the call.
