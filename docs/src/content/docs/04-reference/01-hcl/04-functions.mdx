...
`base64gzip_compat(str)` returns the value `base64gzip(str)` returned in Terragrunt v1.1.3 and earlier, and keeps returning it now that Terragrunt 1.2 has switched `base64gzip()` to the current Go encoder.
