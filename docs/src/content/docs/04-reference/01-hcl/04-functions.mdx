...
`deep_merge(map1, map2, ...)` deeply merges maps/objects. For overlapping keys, values from later arguments override earlier ones. Nested maps are merged recursively, and lists are appended.

When merging multiple files, precedence follows argument order. With `sort(fileset(...))`, later filenames in alphabetical order take precedence for overlapping keys.

Any `null` arguments are ignored.

This function is additive and does not change the behavior of OpenTofu/Terraform `merge(...)`.

This is useful when composing nested configuration from multiple files:
