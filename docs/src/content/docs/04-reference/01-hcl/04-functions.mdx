...
`base64gzip_compat(str)` returns the value `base64gzip(str)` returned in Terragrunt v1.1.3 and earlier, and keeps returning it after `base64gzip()` moves to the current Go encoder.
...
`base64gzip()` itself returns the same bytes until a future version switches it to the current encoder; see the [`legacy-base64gzip`](/reference/strict-controls/active#legacy-base64gzip) strict control.
