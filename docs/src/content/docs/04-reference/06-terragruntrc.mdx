...
before the file is looked for; the rest of the command line is parsed afterwards.
...
Every engineer who clones the repository then gets the same provider cache configuration, with no
exports in their shell profile. While the feature is experimental, each of them still has to enable
the experiment once, and CI has to set `TG_EXPERIMENT=terragruntrc`; everything else a team would
otherwise write down in a README lives in the committed file.
