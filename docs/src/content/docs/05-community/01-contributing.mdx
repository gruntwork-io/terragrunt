...
The largest group of opt-in tests are the ones that drive a real OpenTofu or Terraform binary. They're prefixed with `TestTF*` and tagged with the `tf` build tag:
...
Terragrunt also includes end-to-end tests for the Azure Storage (`azurerm`) remote-state backend. These are prefixed with `TestAzure*` and use the `azure_integration` build tag, which is separate from the `azure` tag used by the hermetic `internal/azurehelper` unit tests. Keeping them separate means a green hermetic run never implies live coverage:

```bash
GOFLAGS='-tags=azure_integration' go test -run 'TestAzure*' ./test
```
...
The resource group is not configured separately: it is looked up from the storage account. Set `TG_AZURE_TEST_RESOURCE_GROUP` only to skip that lookup.
...
- `Storage Blob Data Contributor` on the test storage account, because the tests authorize blob access with Microsoft Entra rather than an account key.
- `Reader` on the subscription, so the resource group can be resolved from the storage account name.
