...
Terragrunt also includes end-to-end tests for the Azure Storage (`azurerm`) remote-state backend. These are prefixed with `TestAzure*` and use the `azure_integration` build tag, because they create and destroy real Azure resources. The `internal/azurehelper` unit tests need no tag and no credentials: they run as part of the normal suite.
