...
If you need to diagnose performance bottlenecks, you can enable CPU and memory profiling using environment variables:

```bash
# CPU profiling (Terragrunt + downstream OpenTofu)
TG_CPU_PROFILE=terragrunt_cpu.prof terragrunt run -- plan

# Memory profiling (Terragrunt only, snapshot at exit)
TG_MEM_PROFILE=terragrunt_mem.prof terragrunt run -- plan
```

When using `terragrunt run --all`, never point `TG_CPU_PROFILE` or `TG_MEM_PROFILE` at a single file. All the parallel OpenTofu processes (plus Terragrunt itself) would write to the same file and corrupt the data.

Instead use the `_DIR` variants. They tell Terragrunt to create a directory and automatically place profiles in a hierarchy that mirrors your units:

```bash
# Recommended for run --all
...
```
/tmp/profiles/
├── terragrunt_cpu.prof          # Main Terragrunt process
├── terragrunt_mem.prof
├── modules/vpc/
│   └── tofu_cpu.prof            # One OpenTofu process per unit
└── live/prod/app/
    └── tofu_cpu.prof
```

See the Performance documentation for a detailed explanation of how `_DIR` collection works.

For more details on profiling and performance analysis, see the [Performance documentation](/troubleshooting/performance/#runtime-cpu-profiling-with-tg_cpu_profile).

You can also use CLI flags when the `profiling` experiment is enabled:

```bash
terragrunt --experiment profiling --profile-dir /tmp/profiles -- run -- plan
```

See the [Performance documentation](/troubleshooting/performance) for the full list of flags and environment variables.
