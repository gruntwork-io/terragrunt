...
If you need to diagnose performance bottlenecks, you can enable CPU profiling for both Terragrunt and OpenTofu by setting the `TG_CPU_PROFILE` environment variable:

```bash
TG_CPU_PROFILE=cpu.prof terragrunt run -- plan
```

This produces a `terragrunt_cpu.prof` file (Terragrunt profile) in the current directory, and automatically sets `TOFU_CPU_PROFILE` for downstream OpenTofu processes so they produce their own `cpu.prof` in their working directories.

When using `run --all`, use a relative path to ensure each OpenTofu unit writes a separate profile file. The Terragrunt profile captures the entire orchestration in a single file.
