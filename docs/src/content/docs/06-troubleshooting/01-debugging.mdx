...
This writes the Terragrunt CPU profile to the specified file, and automatically sets `TOFU_CPU_PROFILE` for downstream OpenTofu processes so they produce their own profile in their working directories.
