...
For `run --all`, use the `_DIR` variants to collect all profiles in a structured directory:

```bash
TG_CPU_PROFILE_DIR=/tmp/profiles TG_MEM_PROFILE_DIR=/tmp/profiles terragrunt run --all -- plan
```

This creates `terragrunt_cpu.prof`, `terragrunt_mem.prof`, and per-module `tofu_cpu.prof` files preserving the directory hierarchy.
