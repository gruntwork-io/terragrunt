...
If you need to diagnose performance bottlenecks, collect CPU, memory, and goroutine profiles:

```bash
# CPU profiling (Terragrunt process)
TG_PROFILE_CPU=terragrunt_cpu.prof terragrunt run -- plan

# Memory profiling (Terragrunt process, snapshot at the end of the run)
TG_PROFILE_MEM=terragrunt_mem.prof terragrunt run -- plan
```

When using `terragrunt run --all`, prefer `TG_PROFILE_DIR`. It tells Terragrunt to create a directory and automatically place profiles in a hierarchy that mirrors your units, including CPU profiles from downstream OpenTofu processes (OpenTofu 1.11 or later):
...
- /tmp/profiles
  ...
  - modules
    - vpc
      - tofu_cpu_plan.prof One OpenTofu process per command per unit
  - live
    - prod
      - app
        - tofu_cpu_plan.prof
...
For more details on profiling and performance analysis, see the [Performance documentation](/troubleshooting/performance/#runtime-cpu-profiling-with-tg_profile_cpu).

You can also use CLI flags:

```bash
terragrunt --profile-dir /tmp/profiles run -- plan
```
