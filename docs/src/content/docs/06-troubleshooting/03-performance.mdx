...
<Before version="1.2.0">You can significantly improve the performance of dependency blocks by using the [`dependency-fetch-output-from-state`](/reference/experiments/active#dependency-fetch-output-from-state) experiment. When the experiment is active for a supported backend, Terragrunt resolves outputs by fetching and parsing the backend state object directly, avoiding any overhead incurred by calling the `output -json` command of OpenTofu/Terraform.</Before>
<Since version="1.2.0">On a supported backend, Terragrunt resolves dependency outputs by fetching and parsing the backend state object directly, avoiding the overhead of calling the `output -json` command of OpenTofu/Terraform. This is the default. Pass [`--no-dependency-fetch-output-from-state`](/reference/cli/commands/run#no-dependency-fetch-output-from-state) to go back to the output command.</Since>
...
OpenTofu/Terraform may change the state schema. A compatible-looking schema change could affect direct-read results. Pin the OpenTofu/Terraform version, because compatible-looking schema changes can affect direct-read results.
...
```shell
hyperfine -w 3 -r 5 'terragrunt run --all plan --no-dependency-fetch-output-from-state' 'terragrunt run --all plan'
```
