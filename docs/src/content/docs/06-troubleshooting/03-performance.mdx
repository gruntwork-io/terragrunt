...
| Flag                  | Description |
|-----------------------|-------------|
| `--profile-cpu`       | Path for the Terragrunt CPU profile. Also sets `TOFU_CPU_PROFILE` for OpenTofu (unless a dir is used). |
| `--profile-mem`       | Path for the Terragrunt heap memory profile. |
| `--profile-goroutine` | Path for a goroutine profile (memory dump). |
| `--profile-dir`       | Directory for structured collection. Defaults the files above inside this directory. |
...
| `TG_PROFILE_DIR` (experiment) | New-style env var equivalent of `--profile-dir`. |
