...
This produces two profile files:
...
| `cpu.prof` | OpenTofu | CPU profile of the downstream OpenTofu process. When `TG_CPU_PROFILE` is set, Terragrunt automatically sets `TOFU_CPU_PROFILE` to the same value for any downstream OpenTofu invocations. OpenTofu writes this file in its own working directory (typically inside `.terragrunt-cache/`). |
...
You can then analyze either profile with `go tool pprof`:
