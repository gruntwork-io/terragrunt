...
<Card title="Direct read failures" icon="warning">
...
<Since version="1.1.5">
Outside `render` and `render-json`, Terragrunt retries a failed direct state read with `tofu output` or `terraform output`, unless the state object is missing. If native output retrieval succeeds, only the direct-read speedup is lost. Missing state and the two render commands retain their existing mock-output behavior.
</Since>
...
OpenTofu/Terraform may change the state schema. A compatible-looking schema change could affect direct-read results. Pin the OpenTofu/Terraform version when using this experiment.

</Card>

<Card title="OpenTofu state encryption" icon="warning">

<Before version="1.1.5">
When client-side state encryption is enabled, the encrypted envelope carries no plaintext `outputs` object. Terragrunt treats the dependency as having no outputs. Referencing a missing output can then fail with `Unsupported attribute`, while configured mock-output merging can supply a mock value instead. Disable the experiment with `--no-dependency-fetch-output-from-state` to force native output retrieval.
</Before>

<Since version="1.1.5">
The direct reader does not evaluate OpenTofu's encryption configuration or key providers. Outside `render` and `render-json`, Terragrunt recognizes the encrypted envelope and retries with `tofu output` or `terraform output`. If the configured binary can decrypt the state and native output retrieval succeeds, only the speedup is lost. The render commands retain their existing mock-output behavior. Disable the experiment when a render command must resolve real outputs from encrypted state.
</Since>
...
Runtime profiling for Terragrunt ships with v1.1.2.
...
Runtime CPU profiling for Terragrunt ships with v1.1.2.
...
OpenTofu CPU profile path controls ship with v1.1.2.
...
Profile directory collection for Terragrunt ships with v1.1.2.
...
Runtime memory profiling for Terragrunt ships with v1.1.2.
...
Goroutine profiling for Terragrunt ships with v1.1.2.
...
Profiling CLI flags for Terragrunt ship with v1.1.2.
...
Profiling environment variables for Terragrunt ship with v1.1.2.
...
Runtime profile analysis guidance ships with v1.1.2.
