...
The experiment supports S3, GCS, and Azure Storage (`azurerm`) backends.<Before version="1.2.0"> Azure also requires the [`azure-backend`](/reference/experiments/active#azure-backend) experiment.</Before> Unsupported backends, and supported backends whose configuration the direct reader cannot reproduce, fall back to the normal OpenTofu/Terraform output command.
