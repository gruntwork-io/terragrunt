...
##### CLI Flags for Profiling (requires the `profiling` experiment)

You can also enable profiling using CLI flags. These are gated behind the `profiling` experiment:

```shell
terragrunt --experiment profiling --profile-cpu terragrunt_cpu.prof --profile-mem terragrunt_mem.prof --profile-goroutine terragrunt_goroutine.prof -- run -- plan
```
