...
You can profile Terragrunt during normal operation by setting `TG_PROFILE_CPU` when the `profiling` experiment is enabled:

```shell
TG_EXPERIMENT=profiling TG_PROFILE_CPU=terragrunt_cpu.prof terragrunt run -- plan
```
...
```shell
TG_EXPERIMENT=profiling TG_PROFILE_CPU=terragrunt_cpu.prof TOFU_CPU_PROFILE=tofu_cpu.prof terragrunt run -- plan
```
...
```shell
TG_EXPERIMENT=profiling TG_PROFILE_DIR=/tmp/profiles terragrunt run --all -- plan
```
...
The subdirectory layout matches the layout of your units relative to the working directory. This guarantees that no two OpenTofu processes write to the same file in parallel.
...
```shell
TG_EXPERIMENT=profiling TG_PROFILE_MEM=terragrunt_mem.prof terragrunt run -- plan
```

This is a snapshot taken at the end of the run after forcing a GC. Unlike CPU profiling, memory profiles are not collected from OpenTofu because there is no `TOFU_MEM_PROFILE` equivalent.
...
```shell
TG_EXPERIMENT=profiling TG_PROFILE_GOROUTINE=terragrunt_goroutine.prof terragrunt run -- plan
```
