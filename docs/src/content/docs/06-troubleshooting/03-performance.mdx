...
- When a unit runs OpenTofu multiple times (for example, auto-init followed by plan), the profile contains the last OpenTofu invocation in that unit.
...
| `TG_PROFILE_DIR` | Directory for structured collection with `run --all`. Terragrunt writes conventional profile files here, and each unit gets a subdirectory for `tofu_cpu.prof` (OpenTofu 1.11 or later). |
