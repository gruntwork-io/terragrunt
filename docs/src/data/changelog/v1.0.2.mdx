...
- **`stack.<name>.path`** references for depending on an entire stack — the DAG expands the stack into its constituent units at runtime
...
- **Partial evaluation of `local.*` in autoinclude** — expressions mixing `local.*` and `dependency.*` are now partially evaluated: locals resolve to literals while dependency references are preserved for runtime
