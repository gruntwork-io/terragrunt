...
Bootstrap-only settings such as `location`, the storage account SKU options, the `skip_*` flags, `enable_soft_delete`, `soft_delete_retention_days`, and `msi_resource_id` are consumed by Terragrunt and filtered out before `tofu init -backend-config`, so the underlying `azurerm` backend receives only keys it understands.
