...
Authentication is file-based: configured credentials come from OpenTofu's CLI config (`oci_credentials` and `oci_default_credentials` blocks in `.tofurc` / `.terraformrc`), plus read-only ambient discovery of Docker and containers auth files (`credHelpers`, `credsStore`, and inline `auths`). Terragrunt selects the most specific matching source across both, so one configuration serves both `tofu` and Terragrunt.
