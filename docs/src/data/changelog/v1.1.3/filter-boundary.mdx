...
Enable the new [`bounded-discovery`](/reference/experiments/active#bounded-discovery) experiment to unlock the `--filter-boundary` flag (env: `TG_FILTER_BOUNDARY`), which replaces the git repository root as a single enclosure for that traversal:

```bash
cd environments/staging
terragrunt run --all plan --experiment bounded-discovery --filter '...{vpc}' --filter-boundary .
```
