...
The `mutable-generate` experiment has been added. With it enabled, the contents a `generate` block produces are stored in the [Content Addressable Store (CAS)](/features/caching/cas), and the file written at `path` is a read-only link to that stored copy rather than a file of its own.
