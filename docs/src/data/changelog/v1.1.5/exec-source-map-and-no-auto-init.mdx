...
`terragrunt exec` rejected [`--source`](/reference/cli/commands/exec#source), [`--source-map`](/reference/cli/commands/exec#source-map), and [`--no-auto-init`](/reference/cli/commands/exec#no-auto-init) as invalid flags, one message per flag: ``flag `--source-map` is not a valid flag for `exec` ``. It reads configuration and downloads source the same way `run` does, so there was no way to point `exec` at a local copy of a module, or to stop it from running `init`. All three flags are now registered on `exec`.
