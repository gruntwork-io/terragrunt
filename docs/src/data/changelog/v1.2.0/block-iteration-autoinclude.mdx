...
Discovery also mishandled a `dependency` whose `config_path` pointed at a stack directory. With an expanded unit, it failed the same way but dropped the dependency instead of reporting the error, so `run --all` did not wait for the units in that stack. With `enabled = false` on a `unit` or `stack` block, it kept a dependency on a component that stack generation never writes, and `run --all` failed:
...
The dependency now covers every element of an expanded unit and skips disabled units and stacks, including disabled elements of an expanded block.
