...
Pin the artifact with `tag` or `digest`. Setting neither selects the `latest` tag, and a `//subdir` selector reaches a directory inside the module. Credentials come from OpenTofu's CLI config, from ambient Docker config, and from credential helpers such as `ecr-login`, so one source string resolves the same way under both `tofu` and Terragrunt.
