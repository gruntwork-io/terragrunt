...
This will not stabilize before v1.3, so treat the tool set, the flag names, and the shape of every result as subject to change until then. The [experiment documentation](/reference/experiments/active#mcp-command) lists the criteria that have to be met first.
