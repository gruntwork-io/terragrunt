...
- `--allow=env` passes the server's environment variables to configurations and the commands the tools run.
  ...
  Unless it is granted, tool calls start from an empty environment, so `get_env()` returns its default. The cloud SDKs Terragrunt uses to read remote state itself, and the SOPS decrypter, still look for credentials in the server's own environment.
