...
deletes the state blob at `path/to/my/key.tfstate` in the `tfstate` container of the `mystateacct` storage account. The container, storage account, and resource group are left in place. Use `terragrunt backend delete --force` to bypass the confirmation prompt and the versioning safety check.
