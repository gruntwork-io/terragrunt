...
1. If the backend source for both the source and destination units are the same (both are S3, GCS, or `azurerm`), Terragrunt will use the AWS/GCP/Azure SDK to move state between the two units transparently without interacting with OpenTofu/Terraform. This is the preferred method, when possible.
...
Migration of `azurerm` state is gated behind the [`azure-backend`](/reference/experiments/active#azure-backend) experiment. Without the experiment the command returns an error.

</Aside>

When both source and destination units use the `azurerm` backend with the **same storage account**, Terragrunt copies the state blob server-side via the Azure Blob Storage `Copy Blob` API, then deletes the source blob. Cross-storage-account migrations are rejected up-front: re-issue the command from a unit configured against the destination account, or use `terraform state mv` directly.
