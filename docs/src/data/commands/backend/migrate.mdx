...
examples:
  ...
  - description: |
      Migrate Azure backend state between units (requires experiment flag).
    code: |
      terragrunt --experiment azure-backend backend migrate old-unit new-unit
flags:
...
1. If the backend source for both the source and destination units are the same (both are S3, GCS, or Azure), Terragrunt will use the cloud SDK to move state between the two units transparently without interacting with OpenTofu/Terraform. This is the preferred method, when possible.
...
For the `azurerm` backend, migration works the same way. When both source and destination use the Azure backend, Terragrunt uses the Azure SDK to copy the state blob directly between containers without invoking OpenTofu/Terraform. The source blob is deleted after the copy is verified.

```bash
cp -R old-unit-name new-unit-name
terragrunt --experiment azure-backend backend migrate old-unit-name new-unit-name
rm -rf old-unit-name
```

For cross-backend migrations (e.g., S3 to Azure), Terragrunt falls back to using `tofu state pull` and `tofu state push` to transfer the state.
