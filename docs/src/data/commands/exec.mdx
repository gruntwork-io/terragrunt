...
`--no-auto-init` also covers units named in `dependency` blocks. Terragrunt initializes a dependency when resolving its outputs requires it, with or without `--in-download-dir`. In either case Terragrunt logs a warning in place of `init`, and what runs next may fail if it needs an initialized directory.
