...
In a tree that also contains explicit stacks, the explicit-stack outputs keep their declared `stack.unit` names and implicit units are merged in alongside them. Units materialized by an explicit stack are never repeated under a path key, and on a top-level key collision the explicit stack's value wins (the skipped implicit unit is reported with a warning).
