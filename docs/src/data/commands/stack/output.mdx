...
When enabled and no `terragrunt.stack.hcl` file is present, `terragrunt stack output` discovers the units under the working directory and aggregates their outputs keyed by each unit's directory path (relative to the working directory, forward slashes on every OS):
