...
Gated behind the [`stack-output-implicit`](/reference/experiments/active/#stack-output-implicit---what-it-does) experiment. Enable with `--experiment stack-output-implicit` (or `TG_EXPERIMENT=stack-output-implicit`).

When enabled, `terragrunt stack output` discovers units that are not declared in any `terragrunt.stack.hcl` file and aggregates their outputs keyed by each unit's directory path (relative to the working directory, forward slashes on every OS):
...
Indexing and drill-down work the same way:
