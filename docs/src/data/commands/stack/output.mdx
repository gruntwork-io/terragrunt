...
`terragrunt stack output` also works when no `terragrunt.stack.hcl` file is present. In that case it discovers the units under the working directory and aggregates their outputs keyed by each unit's directory path (relative to the working directory):
