...
The experiment also unlocks the [`--filter-boundary`](/reference/cli/commands/find#filter-boundary) flag (env: `TG_FILTER_BOUNDARY`), which applies one boundary to every `--filter` expression on the command, in both directions:
...
An inline `(dir)` operand overrides the flag for the expression that carries it. See the [`--filter-boundary` reference](/reference/cli/commands/find#filter-boundary) for how the boundary is resolved.
