...
`terragrunt browse` reuses the same discovery the rest of the discovery commands rely on, so the configurations it shows match what `find` and `list` discover. Running `terragrunt browse` without the experiment enabled is an error.
