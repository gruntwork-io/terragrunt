...
<Since version="1.1.5">
Reading state directly is an optimization. Outside `render` and `render-json`, Terragrunt retries a failed direct read with `tofu output` or `terraform output`, unless the state object is missing. If native output retrieval succeeds, only the speedup is lost. Missing state and the two render commands retain their existing mock-output behavior.
</Since>
...
<Since version="1.1.5">
The direct reader cannot decrypt state protected by OpenTofu's [client-side state encryption](https://opentofu.org/docs/language/state/encryption/) because it does not evaluate OpenTofu's encryption configuration or key providers. Outside `render` and `render-json`, Terragrunt recognizes the encrypted envelope and retries with `tofu output` or `terraform output`. If the configured binary can decrypt the state and native output retrieval succeeds, only the speedup is lost. The render commands retain their existing mock-output behavior. Disable the experiment when a render command must resolve real outputs from encrypted state.
</Since>
