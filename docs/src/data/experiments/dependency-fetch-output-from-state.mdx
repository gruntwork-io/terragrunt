...
Reading state directly is an optimization and never decides whether a run succeeds. When a direct read fails, Terragrunt retrieves the outputs through `tofu output` or `terraform output` instead, so only the speedup is lost. A dependency whose state does not exist yet still resolves to that dependency block's `mock_outputs`.

**OpenTofu state encryption:**

When OpenTofu's [client-side state encryption](https://opentofu.org/docs/language/state/encryption/) is enabled, the state file is encrypted before upload, and Terragrunt holds no key material to decrypt it. Terragrunt recognizes the encrypted envelope and retrieves the outputs through `tofu output` or `terraform output` for that dependency, so the outputs are correct and only the speedup is lost. You do not need to disable this feature when you use state encryption.
