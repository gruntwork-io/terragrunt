...
2. The `mark_glob_as_read(pattern)` HCL function. It expands a glob using [`gobwas/glob`](https://github.com/gobwas/glob) syntax and marks every matching file as read, returning the list of absolute paths that matched. See the [HCL reference](/reference/hcl/functions/#mark_glob_as_read) for pattern syntax and examples.
