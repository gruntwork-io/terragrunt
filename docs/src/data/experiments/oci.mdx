...
search order OpenTofu uses. The static variables apply to every registry the
process contacts, not a single host, so do not set them when talking to
unrelated or untrusted registries. Credential helpers (such as `ecr-login`)
are not invoked yet, so registries that need per-run token minting only work
while an externally obtained login is present in an ambient file. When the
