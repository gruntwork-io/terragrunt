...
Support for downloading modules from OCI Distribution registries using `oci://` sources.

### `oci` - What it does

This experiment flag is no longer needed, as `oci://` sources are now
supported by default.

OpenTofu 1.10 can download modules from an OCI Distribution registry using an
`oci://` source. Terragrunt supports the same sources natively, so a single
source string works the same way in both `tofu` and Terragrunt against
registries such as Amazon ECR, GitHub Container Registry, Azure Container
Registry, Google Artifact Registry, and self-hosted or air-gapped registries.

Terragrunt accepts `oci://` source URLs in `terraform { source = "..." }`
blocks, and in the `unit` and `stack` blocks of a `terragrunt.stack.hcl`. For
example:
...
helper-backed registries work without a baked-in login.
...
To transition the `oci` feature to a stable release, the following were completed:
