...
`credsStore`, such as `ecr-login`) are invoked whenever the registry requires
authentication, so ECR and other helper-backed registries work without a
baked-in login. When the experiment is disabled, `oci://` sources remain unsupported.
