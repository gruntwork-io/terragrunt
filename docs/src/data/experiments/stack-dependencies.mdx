...
As of `v1.1.0-rc1` this experiment is complete and the feature is enabled by default, so the `--experiment stack-dependencies` flag is no longer required:
