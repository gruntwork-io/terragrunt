...
- [x] Catalog source resolution for nested stack copies uses the `cas` experiment via `update_source_with_cas = true`. Relative `source` attributes in catalog `terragrunt.stack.hcl` files are rewritten to `cas::` references at generation time, so the generated tree under `.terragrunt-stack/` is self-contained.
