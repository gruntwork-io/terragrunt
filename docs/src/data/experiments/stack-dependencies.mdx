...
- [x] Phased parser for `terragrunt.stack.hcl`: first parse the file body, then evaluate locals and other production-context values, then apply include merges, then decode unit/stack blocks and run autoinclude resolution. This allows unit/stack blocks to use `local.*`, `values.*`, and Terragrunt functions from the production eval context.
- [x] `terragrunt run --all` discovery walks generated nested stack files via a path-only decode shape that does not require the unit `source` to be evaluable
...
- [x] Catalog source preservation across stack copy: nested stack files copied into `.terragrunt-stack/<name>/` get a `.terragrunt-stack-origin` sidecar so the recursion resolves bare relative `../../units/...` paths against the original catalog, not the copy. Only applies to local stack sources; remote and CAS-fetched stacks are unchanged.
...
- [x] Same-name stack/unit pair (`stack "foo" { unit "foo" { ... } }`) addressable via `stack.foo.foo.path` and end-to-end covered by integration test.
- [x] Unicode purged from `internal/hclparse` source files (ASCII-only); deliberate unicode test fixtures retained.
