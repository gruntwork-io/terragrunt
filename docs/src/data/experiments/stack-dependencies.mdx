...
- [x] Lazy-expression parsing for `source`/`path` so non-literal HCL expressions in unrelated unit attributes do not block autoinclude resolution
- [x] `terragrunt run --all` discovery walks generated nested stack files when they contain terragrunt function calls in unit `source`
