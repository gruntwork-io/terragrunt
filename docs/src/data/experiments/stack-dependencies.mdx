...
- [x] Phased parser: locals evaluate before include merge before unit/stack decode before autoinclude resolution, so any attribute that the production eval context can resolve (including `local.*`, `values.*`, terragrunt functions) works on unit/stack blocks
