...
- Lazy-expression parsing: non-literal HCL expressions (`get_repo_root()`, `format()`, `local.*`, `values.*`) in unit/stack `source`, `path`, and `values` attributes no longer block autoinclude resolution
- `terragrunt run --all` discovery walks generated nested stack files even when they contain terragrunt function calls in unit `source`/`path`
- Stack-level autoinclude `values = {...}` propagates into nested-stack units' `terragrunt.values.hcl` so nested unit `inputs = { x = values.x }` resolves
- Unified strict + permissive parser into a single code path
