...
Terragrunt splits the value into a program and its arguments with shell quoting rules, then runs the program without a shell. An unquoted shell operator such as `|`, `;`, `&&`, or `>` is an error. To run a pipeline, put it in a script and pass the script.
</Since>
