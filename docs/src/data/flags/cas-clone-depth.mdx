...
This flag sets the ambient depth for all CAS Git clones. An individual source can override it with a [`depth` query parameter](/features/caching/cas/git#source-url-parameters) on its URL (for example `...vpc.git?depth=1`); a positive URL `depth` takes precedence over this flag for that source.
