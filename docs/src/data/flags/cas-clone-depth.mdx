...
This flag sets the depth for all CAS Git clones. A [`depth` query parameter](/features/caching/cas/git#source-url-parameters) on a source URL (for example `...vpc.git?depth=1`) is stripped before `git` is invoked and never changes the depth, which always comes from this flag.
