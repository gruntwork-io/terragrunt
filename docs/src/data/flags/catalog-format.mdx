...
name: format
description: |
  Format the catalog as specified. Supported values (tui, jsonl, md). Default: tui when stdin and stdout are terminals, jsonl otherwise.
type: string
env:
  - TG_FORMAT
---
...
- `tui` renders the catalog as an interactive Terminal User Interface.
...
Without `--format`, the catalog uses `tui` when standard input and standard output are both terminals, and `jsonl` otherwise (e.g., when piped to another program).
...
```bash
$ terragrunt catalog --format=jsonl | head -1
{"kind":"module","title":"VPC","description":"Creates a VPC.","tags":["networking"],"source":"github.com/acme/infrastructure-modules","dir":"modules/vpc","version":"v0.14.2","url":"https://github.com/acme/infrastructure-modules/tree/main/modules/vpc","component_source":"github.com/acme/infrastructure-modules//modules/vpc","doc":"Everything a VPC needs.\n"}
```
