...
<Since version="1.1.5">
The direct reader does not evaluate OpenTofu's encryption configuration or key providers. Outside `render` and `render-json`, Terragrunt recognizes the encrypted envelope and retries with `tofu output` or `terraform output`. If the configured binary can decrypt the state and native output retrieval succeeds, only the speedup is lost. The render commands retain their existing mock-output behavior. Use `--no-dependency-fetch-output-from-state` when a render command must resolve real outputs from encrypted state.
</Since>
</Aside>
