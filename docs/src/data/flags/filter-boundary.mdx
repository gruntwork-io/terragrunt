...
This flag is gated behind the [`bounded-discovery`](/reference/experiments/active#bounded-discovery) experiment. Enable it with `--experiment=bounded-discovery` (or `TG_EXPERIMENT=bounded-discovery`); otherwise setting `--filter-boundary` returns an error. The flag's behavior may change while the experiment is in progress.
</Aside>
...
terragrunt find --experiment bounded-discovery --filter '...{vpc}' --filter-boundary .
```
