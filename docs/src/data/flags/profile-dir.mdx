...
In addition, each unit's downstream OpenTofu process writes its own CPU profile to `tofu_cpu_<command>.prof` (for example `tofu_cpu_plan.prof`) inside a unit-specific subdirectory (supported by OpenTofu 1.11 or later; Terraform ignores it). Units outside the working directory are placed under an `external/` subdirectory so they never collide.
