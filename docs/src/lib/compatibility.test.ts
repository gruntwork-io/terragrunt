...
describe("filterByReleasedMin", () => {
  test("keeps entries whose terragrunt_min is at or below the latest release", () => {
    ...
    const result = filterByReleasedMin(entries, "v1.0.5");
  ...
  test("compares numerically not lexically (1.0.10 > 1.0.5)", () => {
    ...
    expect(filterByReleasedMin(entries, "v1.0.5")).toEqual([]);
    expect(filterByReleasedMin(entries, "v1.0.10").map((e) => e.id)).toEqual(["a"]);
  });

  test("returns all entries when showUnreleased is true", () => {
    const entries = [makeEntry("a", "1.0.0"), makeEntry("future", "99.0.0")];
    const result = filterByReleasedMin(entries, "v1.0.0", true);
    expect(result.map((e) => e.id)).toEqual(["a", "future"]);
  });

  test("returns empty array when latest is v0.0.0 (treats nothing as released)", () => {
    const entries = [makeEntry("a", "1.0.0")];
    expect(filterByReleasedMin(entries, "v0.0.0")).toEqual([]);
  });
});

describe("effectiveTerragruntMax", () => {
  test("returns null when input is already null", () => {
    expect(effectiveTerragruntMax(null, "v1.0.5")).toBe(null);
  });

  test("returns the value when it is at or below the latest release", () => {
    expect(effectiveTerragruntMax("1.0.5", "v1.0.5")).toBe("1.0.5");
    expect(effectiveTerragruntMax("0.24.4", "v1.0.5")).toBe("0.24.4");
  });

  test("returns null when the value is newer than the latest release", () => {
    expect(effectiveTerragruntMax("1.1.0", "v1.0.5")).toBe(null);
  });

  test("compares numerically not lexically", () => {
    expect(effectiveTerragruntMax("1.0.10", "v1.0.5")).toBe(null);
    expect(effectiveTerragruntMax("1.0.10", "v1.0.10")).toBe("1.0.10");
  });

  test("returns the original value unchanged when showUnreleased is true", () => {
    expect(effectiveTerragruntMax("99.0.0", "v1.0.5", true)).toBe("99.0.0");
  });
});
