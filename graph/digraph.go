...
// This was copied, verbatim, from https://raw.githubusercontent.com/golang/tools/master/cmd/digraph/digraph.go. This
// code can be used to parse directed graph data in dot format. Unfortunately, all the methods in the original code
// were private (I suppose it was designed only to be used from the CLI), so to be able to embed it within Terragrunt,
// we've copied the code here and in graph.go, we expose some public methods that use these private methods under the
// hood.
