...
// AssumeIamRole assumes an IAM role and returns the credentials. The
// credentials are never nil when the error is; a response without them is
// reported as [ErrNoAssumedCredentials].
func AssumeIamRole(
	ctx context.Context,
	v *venv.Venv,
	iamRoleOpts iam.RoleOptions,
