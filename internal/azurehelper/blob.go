...
// overwrite an existing destination, copies, then deletes the source. Both
// blobs must live in the storage account this client is bound to.
func (c *BlobClient) MoveBlobIfNecessary(ctx context.Context, srcContainer, srcKey, dstContainer, dstKey string) error {
	...
	dstExists, err := c.BlobExists(ctx, dstContainer, dstKey)
	if err != nil {
		return err
	}

	if dstExists {
		return fmt.Errorf("destination blob %s/%s already exists", dstContainer, dstKey)
	}
...
// private containers under any auth method. Both blobs must live in the
// storage account this client is bound to.
func (c *BlobClient) CopyBlob(ctx context.Context, srcContainer, srcKey, dstContainer, dstKey string) error {
	...
	return c.PutBlobFromReader(ctx, dstContainer, dstKey, body)
}
