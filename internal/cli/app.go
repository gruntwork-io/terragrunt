...
func NewApp(l log.Logger, opts *options.TerragruntOptions) *App {
	...
	app.Name = AppName
	app.Usage = "Terragrunt is a flexible orchestration tool that allows Infrastructure as Code written in " +
		...
		if cmdName != "" {
			if cliCtx.Command == nil || cliCtx.Command.Subcommand(cmdName) == nil {
				...
				return clihelper.NewExitError(
					errors.Errorf(
						"unknown command: %q. Terragrunt no longer forwards unknown commands by default. "+
							"Use 'terragrunt run -- %s ...' or a supported shortcut. Learn more: "+
							"https://terragrunt.gruntwork.io/docs/migrate/cli-redesign/#use-the-new-run-command",
						cmdName,
						cmdName,
					),
