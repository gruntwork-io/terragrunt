...
func TestWelcomeDiscoveryErrorView_AllSourcesFailedDetail(t *testing.T) {
	...
	m = updateModel(m, tui.DiscoveryCompleteMsg{Err: srcErr}).(tui.WelcomeModel)
...
func TestComponentListView_LoadingTitle(t *testing.T) {
	...
	updated, _ = m.Update(tui.DiscoveryCompleteMsg{Err: nil})
	m = updated.(tui.Model)
...
func TestComponentListView_PartialSourceFailureNotice(t *testing.T) {
	...
	updated, _ = m.Update(tui.DiscoveryCompleteMsg{Err: srcErr})
	m = updated.(tui.Model)
...
func TestWelcomeStreamingFlowWithRacing(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		for i := 1; i < len(items); i++ {
			prev := items[i-1].(*tui.ComponentEntry).Title()
			curr := items[i].(*tui.ComponentEntry).Title()
...
func blockingLoad(ctx context.Context, _ tui.StatusFunc, _ chan<- *tui.ComponentEntry) error {
	<-ctx.Done()
