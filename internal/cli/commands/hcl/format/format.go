...
func RunForFiles(ctx context.Context, l log.Logger, opts *options.TerragruntOptions, baseDir string, files []string) error {
	...
	for _, file := range files {
		...
		g.Go(func() error {
			if err := formatTgHCL(l, opts, file); err != nil {
				mu.Lock()
...
func formatTgHCL(l log.Logger, opts *options.TerragruntOptions, tgHclFile string) error {
	...
	if opts.Diff && fileUpdated {
		if _, err := opts.Writers.Writer.Write(bytesDiff(contents, newContents, tgHclFile)); err != nil {
