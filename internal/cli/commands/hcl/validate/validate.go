...
func GetVarFlagsFromArgList(argList []string) ([]string, []string, error) {
	...
	for _, arg := range argList {
		// Split with shell quoting rules to remove the quoting. For example, the string:
		// -var="'"foo"'"='bar'
		// becomes:
		// -var='foo'=bar
		argWords, err := split.LegacyCommand(arg)
		if err != nil {
		...
		// back the split words even if there are several.
		unquotedArg := strings.Join(argWords, " ")

		if strings.HasPrefix(unquotedArg, "-var=") {
			// -var is passed in in the format -var=VARNAME=VALUE, so we split on '=' and take the middle value.
			splitArg := strings.Split(unquotedArg, "=")
		...
		if after, ok := strings.CutPrefix(unquotedArg, "-var-file="); ok {
