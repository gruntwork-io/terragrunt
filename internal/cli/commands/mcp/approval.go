...
func approvalMessage(
	...
	fmt.Fprintf(b, "Approve running `terragrunt run --all %s%s`?\n\n", command, approvalRunFlags(input))
...
// approvalRunFlags renders a tool call's run arguments as the CLI flags they
// stand for, so the approved command line names the filters and parallelism
// the run uses. The arguments must already have passed [runAllInput.validate].
//
// Each filter query is agent text, so it is sanitized to one line and
// shell-quoted as one word, and cannot write past its own flag.
func approvalRunFlags(input runAllInput) string {
	b := &strings.Builder{}

	for _, query := range input.Filter {
		b.WriteString(" --filter=" + split.Quote(tui.SanitizeLabel(query)))
	}

	if input.Parallelism > 0 {
		b.WriteString(" --parallelism=" + strconv.Itoa(input.Parallelism))
	}
