...
// that walk a directory, with the environment [serverDeps.callEnv] returns.
// Hand that env map to [serverDeps.callVenv], so credentials land in the same
// map the parse reads.
...
// terragrunt.hcl, with the environment [serverDeps.callEnv] returns.
