...
func TestRenderJSON_Basic(t *testing.T) {
	...
	err := render.Run(t.Context(), logger.CreateLogger(), new(venv.OSVenv().WithWriter(&outputBuffer)), opts)
...
func TestRenderJSON_WriteToFile(t *testing.T) {
	...
	err := render.Run(t.Context(), logger.CreateLogger(), new(venv.OSVenv().WithWriter(io.Discard)), opts)
	require.NoError(t, err)
...
func TestRenderJSON_HCLFormat(t *testing.T) {
	...
	err := render.Run(t.Context(), logger.CreateLogger(), new(venv.OSVenv().WithWriter(&renderedBuffer)), opts)
	require.NoError(t, err)
