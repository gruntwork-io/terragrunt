...
func promptForValues(
	...
	// Redirected output, a CI job, and a pipe all land here, which is why
	// this is a fallback rather than the failure the catalog command raises:
	// scaffolding has a perfectly good non-interactive result to fall back to.
	if err := viewtui.EnsureOSTTY(l); err != nil {
