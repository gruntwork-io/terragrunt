...
// placeholders rather than failing the way the catalog command does.
//
// It skips where a terminal is available, since a regression would open the
// form for real and block.
func TestRunInteractiveWithoutTerminalSkipsTheForm(t *testing.T) {
	...
	if in, out, err := tea.OpenTTY(); err == nil {
		...
		t.Skip("a controlling terminal is available; a regression would open the form for real")
	}
