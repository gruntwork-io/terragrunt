...
func TestExitCodeFor(t *testing.T) {
	...
	t.Run("nil error returns success exit code", func(t *testing.T) {
		...
		l, _ := newTestLogger()
		assert.Equal(t, 0, cli.ExitCodeFor(l, []string{"terragrunt"}, nil, 0, newReporter(t)))
		assert.Equal(t, 2, cli.ExitCodeFor(l, []string{"terragrunt"}, nil, 2, newReporter(t)))
	})
	...
	t.Run("regular error returns 1 with logged message", func(t *testing.T) {
		...
		l, buf := newTestLogger()
		code := cli.ExitCodeFor(l, []string{"terragrunt"}, stdErrors.New("regular failure"), 0, newReporter(t))
...
func newReporter(t *testing.T) *panicreport.Reporter {
	t.Helper()
