...
func shouldContinueWithFileExists(l log.Logger, path string, ifExists GenerateConfigExists) (bool, error) {
	...
	case ExistsOverwriteTerragrunt:
		...
		l.Debugf(
			"The file path %s already exists, but was a previously generated file by terragrunt. "+
				"Since if_exists for code generation is set to \"overwrite_terragrunt\", regenerating file.",
			path)
...
func shouldRemoveWithFileExists(l log.Logger, path string, ifDisable GenerateConfigDisabled) (bool, error) {
	...
	case DisabledRemoveTerragrunt:
		// If file was not generated, error out because remove_terragrunt if_disabled setting only handles
		// if the existing file was generated by terragrunt.
		wasGenerated, err := fileWasGeneratedByTerragrunt(path)
		if err != nil {
			return false, err
		}
		...
		l.Debugf(
			"The file path %s already exists, but was a previously generated file by terragrunt. "+
				"Since if_disabled for code generation is set to \"remove_terragrunt\", removing file.",
			path)
...
func RemoteStateConfigToTerraformCode(backend string, config map[string]any, encryption map[string]any) ([]byte, error) {
	...
	for _, key := range backendKeys {
		...
		if key == assumeRoleWithWebIdentityConfigKey {
			assumeRoleWithWebIdentityValue, isAssumeRoleWithWebIdentity :=
				config[assumeRoleWithWebIdentityConfigKey].(string)
	...
	keyProviderBlockBody := encryptionBlockBody.AppendNewBlock(
		EncryptionKeyProviderKey,
		[]string{keyProvider, encryptionResourceName}).Body()

	// Append method block
	methodBlock := encryptionBlockBody.AppendNewBlock(
		encryptionMethodKey,
		[]string{encryptionDefaultMethod, encryptionResourceName}).Body()
