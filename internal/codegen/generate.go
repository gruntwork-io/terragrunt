...
func WriteToFile(l log.Logger, fs vfs.FS, basePath string, config *GenerateConfig) error {
	...
	targetFileExists, err := vfs.FileExists(fs, targetPath)
	if err != nil {
	...
	if config.Disable {
		...
		if targetFileExists {
			if shouldRemove, err := shouldRemoveWithFileExists(l, fs, targetPath, config.IfDisabled); err != nil {
				return err
	...
	if targetFileExists {
		shouldContinue, err := shouldContinueWithFileExists(l, fs, targetPath, config.IfExists)
	...
	if err := vfs.WriteFile(fs, targetPath, contentsToWrite, ownerWriteGlobalReadPerms); err != nil {
		return errors.New(err)
	}
...
func shouldContinueWithFileExists(l log.Logger, fs vfs.FS, path string, ifExists GenerateConfigExists) (bool, error) {
	...
	case ExistsOverwriteTerragrunt:
		...
		wasGenerated, err := fileWasGeneratedByTerragrunt(fs, path)
		if err != nil {
			return false, err
		}
...
func shouldRemoveWithFileExists(l log.Logger, fs vfs.FS, path string, ifDisable GenerateConfigDisabled) (bool, error) {
	// TODO: Make exhaustive
...
func fileWasGeneratedByTerragrunt(fs vfs.FS, path string) (bool, error) {
