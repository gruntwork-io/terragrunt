...
func shouldContinueWithFileExists(l log.Logger, path string, ifExists GenerateConfigExists) (bool, error) {
	...
	case ExistsOverwriteTerragruntOrSkip:
		...
		if !wasGenerated {
			l.Debugf("The file path %s already exists, was not generated by terragrunt,"+
				" and if_exists is set to \"overwrite_terragrunt_or_skip\". Skipping.", path)

			return false, nil
		}

		l.Debugf("The file path %s already exists, was previously generated by terragrunt,"+
			" and if_exists is set to \"overwrite_terragrunt_or_skip\". Regenerating.", path)
