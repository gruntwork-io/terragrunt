...
func (tsc *ThreadSafeComponents) EnsureComponent(c Component) {
	if !tsc.containsComponent(c) {
		tsc.addComponent(c)
	}
}

// containsComponent checks if a component is in the components slice.
func (tsc *ThreadSafeComponents) containsComponent(c Component) bool {
	tsc.mu.RLock()
	defer tsc.mu.RUnlock()

	return slices.Contains(tsc.components, c)
}
...
// AddComponent adds a component to the components list.
// This is a public wrapper around the private addComponent method.
func (tsc *ThreadSafeComponents) AddComponent(c Component) {
	tsc.addComponent(c)
}
