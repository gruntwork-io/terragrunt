...
func (dd *DependentDiscovery) DiscoverDependents(
	...
	g.Go(func() error {
		...
		for candidate := range candidates {
			...
			parseErr := Parse(candidate, walkCtx, l, dd.opts, dd.suppressParseErrors, dd.parserOptions)
			if parseErr != nil {
				mu.Lock()
			...
			for _, dep := range deps {
				...
				isExternal := dd.isExternal(c.Path())
			...
			if dependsOnTarget {
				isExternal := dd.isExternal(candidate.Path())
...
func (dd *DependentDiscovery) isExternal(componentPath string) bool {
	if dd.workingDir == "" {
		return true
	}
