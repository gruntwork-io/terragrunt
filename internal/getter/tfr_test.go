...
// TestRegistryGetterMissingVersion pins the typed error returned when a
// tfr:// URL is missing the required version query parameter.
func TestRegistryGetterMissingVersion(t *testing.T) {
	t.Parallel()
	...
	_, err := client.Get(t.Context(), &getter.Request{
		Src:     "tfr://" + server.Listener.Addr().String() + "/foo/bar/baz",
