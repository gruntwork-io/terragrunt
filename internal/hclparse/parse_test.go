...
var (
	testStackDir = venvtest.Root("/test/stack")
	testGenDir   = venvtest.Root("/test/stack/.terragrunt-stack/app")
)

// hclPath returns a relative path as the generated file quotes it: OS-native
// separators, escaped the way hclwrite escapes them.
func hclPath(elem ...string) string {
	return strconv.Quote(filepath.Join(elem...))
}
...
`
	...
	assert.Contains(t, content, hclPath("..", "vpc"))
...
func TestParseStackFile_LocalEvaluatedOnce(t *testing.T) {
	...
	includePath := venvtest.Root("/includes/extra.stack.hcl")

	onceFn := function.New(&function.Spec{
		Type: function.StaticReturnType(cty.String),
		Impl: func([]cty.Value, cty.Type) (cty.Value, error) {
			calls.Add(1)
			return cty.StringVal(includePath), nil
		},
	})
