...
const rateLimitBody = `{"error":"slow_down","message":"Too many catalog requests. Try again shortly."}`
...
func TestFetchCatalogRequest(t *testing.T) {
	...
	assert.Equal(t, portalBaseURL+"/api/v1/catalog", got.URL.String())
...
func TestFetchCatalogReportsARejectedCredential(t *testing.T) {
	...
	body := `{"error":"invalid_token","message":"The access token is invalid, expired, or revoked."}`
...
// as a portal with nothing to serve.
...
func TestFetchCatalogReportsRefusal(t *testing.T) {
	...
	tc := []struct {
		...
		{
			name:     "the credential lacks the catalog scope",
			status:   http.StatusForbidden,
			body:     `{"error":"insufficient_scope","message":"The access token must carry the catalog:read scope."}`,
			wantCode: portal.ErrorCode("insufficient_scope"),
		},
		{
	...
	stub := &catalogStub{answers: []catalogAnswer{
		{
			...
			header: http.Header{"Location": {"http://portal.example.com/api/v1/catalog"}},
		},
		{status: http.StatusOK, body: twoRepositoriesBody},
	}}
	...
	for _, status := range statuses {
		t.Run(http.StatusText(status), func(t *testing.T) {
			...
			synctest.Test(t, func(t *testing.T) {
				...
				stub := servedWith(status, `{"error":"server_error","message":"Internal server error."}`)
