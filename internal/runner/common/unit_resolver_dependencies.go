...
// prompting the user whether to run them and setting appropriate flags.
	...
	for _, unit := range unitsMap {
		...
		if !r.Stack.TerragruntOptions.IgnoreExternalDependencies {
			...
			if dependentUnit != nil {
				...
				shouldRun, err = r.confirmShouldRunExternalDependency(ctx, dependentUnit, l, unit, unit.TerragruntOptions)
				if err != nil {
					return err
				}
			}
		...
		unit.AssumeAlreadyApplied = !shouldRun
		// Mark external dependencies as excluded if they shouldn't be run
...
// run. If the user selects "yes", then Terragrunt will run that unit as well.
...
func (r *UnitResolver) confirmShouldRunExternalDependency(ctx context.Context, unit *Unit, l log.Logger, dependency *Unit, opts *options.TerragruntOptions) (bool, error) {
	...
	return shell.PromptUserForYesNo(ctx, l, "Should Terragrunt run the external dependency?", opts)
}
