// Package runner discovers Terragrunt units and runs them as a pool of workers over the
// dependency graph.
package runner
...
// CloneUnitOptions clones the stack's options and logger for a single unit.
func CloneUnitOptions(
	...
	// DisplayPath is relative to the discovery context, which keeps log prefixes short.
...
func BuildUnitOpts(
	...
	if stackOpts != nil {
		_, stackDefaultDownloadDir = util.DefaultWorkingAndDownloadDirs(
			stackOpts.TerragruntConfigPath,
		)
	}
	...
	unitOpts, unitLogger, err := CloneUnitOptions(
		stackOpts,
		unit,
		configPath,
		stackDefaultDownloadDir,
		l,
	)
	if err != nil {
		return nil, nil, err
		...
		if unitConfig != nil {
			...
			if sourceErr != nil {
				return nil, nil, fmt.Errorf(
					"failed to compute source for unit %s: %w",
					unit.DisplayPath(),
					sourceErr,
				)
			}
...
// Callers that still need discovery should use [New] instead.
func NewFromComponents(
	...
	// Excluded units are dropped here so they are neither listed nor scheduled.
...
func (rnr *Runner) Run(
	...
	// Per-unit opts are cloned from stackOpts below, so these must be set first.
	...
	if r != nil {
		for _, u := range rnr.Stack.Units {
			if u.Excluded() {
				...
				// Units excluded by --queue-exclude-dir or exclude blocks are already
				// reported during unit resolution, with the correct reason.
				if run.Result == "" {
	...
	for {
		...
		for unit, dependents := range dependentUnits {
			for _, dependent := range dependents {
				...
				list := util.RemoveDuplicates(
					append(dependentUnits[unit], dependentUnits[dependent]...),
				)
...
func applyPreventDestroyExclusions(l log.Logger, units []*component.Unit) {
	...
	// Second pass: a protected unit's dependencies must survive too, or destroying
	// them would break it.
	...
	for _, unit := range units {
		if dependencyPaths[unit.Path()] && !protectedUnits[unit.Path()] {
			...
			l.Debugf(
				"Unit %s is excluded because it's a dependency of a protected unit",
				unit.Path(),
			)
		}
	}
...
func applyFilterAllowDestroyExclusions(
	l log.Logger,
	opts *options.TerragruntOptions,
	units []*component.Unit,
) {
	...
	for _, unit := range units {
		...
		if discoveryCtx.Ref != "" &&
			iacargs.New(discoveryCtx.Args...).IsDestroyCommand(discoveryCtx.Cmd) {
			unit.SetExcluded(true)
...
func logTaskOutcome(ctx context.Context, l log.Logger, unitPath, command string, err error) {
	...
	if err != nil {
		l.Debugf(
			"Runner Pool Task: finished unit=%s command=%s outcome=%s err=%v",
			unitPath,
			command,
			outcome,
			err,
