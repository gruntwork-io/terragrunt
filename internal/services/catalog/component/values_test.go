...
func optionalNames(refs component.ValuesReferences) []string {
...
`
	...
	refs, err := component.CollectValuesReferences(fsys, path)
...
func TestCollectValuesReferences_MissingFile(t *testing.T) {
	...
	refs, err := component.CollectValuesReferences(fsys, "/missing/nope.hcl")
	require.NoError(t, err)
	assert.True(t, refs.IsEmpty())
}
...
func TestWriteValuesStub_HappyPath(t *testing.T) {
	...
	written, err := component.WriteValuesStub(fsys, dir, refs)
	require.NoError(t, err)
	...
	assert.Contains(t, out, "# Auto-generated by Terragrunt")
...
func TestWriteValuesStub_EmptyIsNoOp(t *testing.T) {
	...
	written, err := component.WriteValuesStub(fsys, dir, component.ValuesReferences{})
	require.NoError(t, err)
...
func TestWriteValuesStub_SkipsWhenExists(t *testing.T) {
	...
	refs := component.ValuesReferences{Required: []string{"foo", "bar"}}
...
func TestWriteValuesFile_AppliesUserValues(t *testing.T) {
	...
	written, err := component.WriteValuesFile(fsys, dir, refs, values)
	require.NoError(t, err)
	assert.True(t, written)
...
func TestWriteValuesFile_InvalidHCLFailsLoud(t *testing.T) {
	...
	refs := component.ValuesReferences{Required: []string{"broken"}}

	_, err := component.WriteValuesFile(fsys, dir, refs, map[string]string{
		"broken": `[1, 2`, // unterminated list
	})

	var invalidErr *component.InvalidHCLValueError
...
func TestWriteValuesFile_EmptyValuesIsEquivalentToStub(t *testing.T) {
	...
	refs := component.ValuesReferences{
		...
		Optional: []component.OptionalValue{{Name: "env", Default: cty.StringVal("prod")}},
	}

	_, err := component.WriteValuesStub(stubFS, dir, refs)
	require.NoError(t, err)

	_, err = component.WriteValuesFile(fileFS, dir, refs, nil)
	require.NoError(t, err)
