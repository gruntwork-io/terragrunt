...
// warnOnRepeatedClaims logs a warning when a component (stack or unit) is claimed by more than one parent in the same invocation.
func warnOnRepeatedClaims(l log.Logger, levelNodes []*StackNode, claimedBy map[string]string) {
	for _, node := range levelNodes {
		...
		prev, ok := claimedBy[node.FilePath]
		if !ok {
		...
		kind := "Stack"
		if !strings.HasSuffix(node.FilePath, config.DefaultStackFile) {
			kind = "Unit"
		}

		l.Warnf("%s %q is being generated by %q, but it was already generated by %q. "+
			"You have contention between multiple stacks generating to the same directory. This is probably a bug in your stack configurations.",
			kind, node.FilePath, parent, prev)
