...
// warnOnRepeatedClaims logs a warning when a stack file is claimed by more than one parent in the same invocation.
func warnOnRepeatedClaims(l log.Logger, levelNodes []*StackNode, claimedBy map[string]string) {
	for _, node := range levelNodes {
		...
		const msg = "stack %s is being generated by %s, but it was previously generated by %s. " +
			"Overwriting stack. You might have a bug in your configurations."

		l.Warnf(msg, node.FilePath, parent, prev)
