...
// resolve. Writers is held as a pointer so per-call overrides via
// [writer.Writers.WithWriter] and [writer.Writers.WithErrWriter] produce
// fresh pointers without mutating the caller's value; never mutate its
