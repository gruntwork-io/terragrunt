...
func TestAutoIncludeParserStageError(t *testing.T) {
	...
	err := config.AutoIncludeParserStageError{Stage: "rescope", File: "/abs/dir/terragrunt.stack.hcl", Err: cause}
	...
	t.Run("errors.As extracts the typed wrapper", func(t *testing.T) {
		...
		var extracted config.AutoIncludeParserStageError
	...
	t.Run("Stage values are distinguishable", func(t *testing.T) {
		...
		for _, stage := range stages {
			e := config.AutoIncludeParserStageError{Stage: stage, File: "f", Err: cause}
