...
func TestDependencyStateMalformedDirectStateReturnsTypedError(t *testing.T) {
	...
	for _, testCase := range testCases {
        ...
        prefix       = "environment/service"`,
			...
			require.NoError(t, err, "malformed direct state must fall back instead of aborting")
			assert.Equal(t, "from-native-output", cfg.Inputs["result"])
...
// the outputs still resolve through OpenTofu instead of aborting the run.
...
// TestDependencyStateEncryptedFallsBackToNativeOutput pins that OpenTofu
// client-side state encryption resolves through OpenTofu instead of silently
// resolving to empty outputs.
func TestDependencyStateEncryptedFallsBackToNativeOutput(t *testing.T) {
	t.Parallel()
	...
	for _, testCase := range testCases {
        ...
        prefix       = "environment/service"`,
			...
			require.NoError(t, err)
			assert.Equal(t, "from-native-output", cfg.Inputs["result"],
				"encrypted state must resolve through OpenTofu, not as empty outputs or the mock")
			assert.NotEmpty(t, recorder.invocations(), "encrypted state must fall back to OpenTofu")
		})
	}
}
