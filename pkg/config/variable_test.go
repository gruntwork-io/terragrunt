...
func TestScanVariables(t *testing.T) {
	...
	inputs, err := config.ParseVariables(logger.CreateLogger(), vfs.NewOSFS(), experiment.NewExperiments(), controls.New(), "../../test/fixtures/inputs")
...
func TestScanDefaultVariables(t *testing.T) {
	...
	inputs, err := config.ParseVariables(logger.CreateLogger(), vfs.NewOSFS(), experiment.NewExperiments(), controls.New(), "../../test/fixtures/inputs-defaults")
	require.NoError(t, err)
	assert.Len(t, inputs, 11)
