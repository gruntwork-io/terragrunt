...
func PromptUserForInputWithRunner(ctx context.Context, l log.Logger, prompt string, runnerOpts *runnertypes.RunnerOptions) (string, error) {
	...
	n, err := runnerOpts.ErrWriter.Write([]byte(prompt))
	if err != nil {
		l.Error(err)
...
// PromptUserForYesNoWithRunner prompts the user for a yes/no response using RunnerOptions.
// This is the RunnerOptions-based variant of PromptUserForYesNo.
func PromptUserForYesNoWithRunner(ctx context.Context, l log.Logger, prompt string, runnerOpts *runnertypes.RunnerOptions) (bool, error) {
	resp, err := PromptUserForInputWithRunner(ctx, l, prompt+" (y/n) ", runnerOpts)
	if err != nil {
		return false, errors.New(err)
	}
