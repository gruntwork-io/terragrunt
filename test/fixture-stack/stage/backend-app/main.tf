...
data "template_file" "text" {
  template = "[I am a backend-app template. Data from my dependencies: vpc = ${data.terraform_remote_state.vpc.text}, mysql = ${data.terraform_remote_state.mysql.text}, search-app = ${data.terraform_remote_state.search_app.text}]"
}
