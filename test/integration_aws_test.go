...
const (
	...
	testFixtureAssumeRoleWebIdentityBackend = "fixtures/assume-role-web-identity/backend"
...
func TestAwsAssumeRoleWebIdentityBackend(t *testing.T) {
	if os.Getenv("CIRCLECI") != "true" {
		t.Skip("Skipping test because it requires valid CircleCI OIDC credentials to work")
	}
	...
	tmpEnvPath := helpers.CopyEnvironment(t, testFixtureAssumeRoleWebIdentityBackend)
	cleanupTerraformFolder(t, tmpEnvPath)
	testPath := util.JoinPath(tmpEnvPath, testFixtureAssumeRoleWebIdentityBackend)

	originalTerragruntConfigPath := util.JoinPath(testFixtureAssumeRoleWebIdentityBackend, "terragrunt.hcl")
	tmpTerragruntConfigFile := util.JoinPath(testPath, "terragrunt.hcl")
	s3BucketName := "terragrunt-test-bucket-" + strings.ToLower(helpers.UniqueID())
