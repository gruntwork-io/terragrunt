...
func TestRunAllIgnoreError(t *testing.T) {
	...
	stdout, stderr, err := helpers.RunTerragruntCommandWithOutput(t, "terragrunt run --all --terragrunt-non-interactive --terragrunt-working-dir "+rootPath+" -- apply -auto-approve")
...
func TestRunAllFail(t *testing.T) {
	...
	_, _, err := helpers.RunTerragruntCommandWithOutput(t, "terragrunt run --all --feature unstable=false --terragrunt-non-interactive --terragrunt-working-dir "+rootPath+" -- apply -auto-approve")
