...
func TestTerragruntHookRunAllApply(t *testing.T) {
	...
	helpers.RunTerragrunt(t, "terragrunt run --all --terragrunt-log-level trace --terragrunt-non-interactive --terragrunt-working-dir "+rootPath+" -- apply -auto-approve")
...
func TestTerragruntInfo(t *testing.T) {
	...
	err := helpers.RunTerragruntCommand(t, "terragrunt info print --terragrunt-non-interactive --terragrunt-working-dir "+rootPath, &showStdout, &showStderr)
	require.NoError(t, err)
