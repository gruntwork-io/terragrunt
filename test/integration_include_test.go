...
func TestTerragruntWorksWithIncludeLocals(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(filepath.Base(tc), func(t *testing.T) {
			...
			helpers.RunTerragrunt(t, "terragrunt run-all apply -auto-approve --queue-include-external-dependencies --non-interactive --log-level trace --working-dir "+childPath)
