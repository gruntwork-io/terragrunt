...
func TestTFNoAutoInit(t *testing.T) {
	t.Parallel()
	...
	err := helpers.RunTerragruntCommand(
		t,
		"terragrunt apply --no-auto-init --non-interactive --working-dir "+rootPath,
		&stdout,
		&stderr,
	)
...
func TestTFAutoInitHonorsMarkerWhenModulesCached(t *testing.T) {
	t.Parallel()
	...
	assert.Equal(
		...
		1,
		strings.Count(stdout.String(), "has been successfully initialized!"),
		"second plan should re-run init because the source-change marker was written",
	)
	assert.NotContains(t, stderr.String(), "Required plugins are not installed")
}

func TestTFAutoInitSkippedWhenOnlyUncopiedFilesChange(t *testing.T) {
	t.Parallel()
...
func TestTFYamlDecodeRegressions(t *testing.T) {
	t.Parallel()
...
func TestTFMockOutputsMergeWithState(t *testing.T) {
	t.Parallel()
	...
	err := helpers.RunTerragruntCommand(
		t,
		"terragrunt apply --non-interactive -auto-approve --working-dir "+modulePath,
		&stdout,
		&stderr,
	)
	...
	err = helpers.RunTerragruntCommand(
		t,
		"terragrunt apply --non-interactive -auto-approve --working-dir "+deepMapPath,
		&stdout,
		&stderr,
	)
	...
	err = helpers.RunTerragruntCommand(
		t,
		"terragrunt apply --non-interactive -auto-approve --working-dir "+shallowPath,
		&stdout,
		&stderr,
	)
...
// TestTFDependencyOutputInGenerateBlock tests that dependency outputs can be used in generate blocks.
...
func TestTFDependencyOutputInGenerateBlock(t *testing.T) {
	t.Parallel()
...
// TestTFDependencyOutputInGenerateBlockDirectRun tests that dependency outputs work when running directly
// This test verifies that even in the broken version, running directly (without --all) works
func TestTFDependencyOutputInGenerateBlockDirectRun(t *testing.T) {
	t.Parallel()
...
// TestTFDependencyOutputInInputsStillWorks verifies that dependency outputs can be used in inputs
func TestTFDependencyOutputInInputsStillWorks(t *testing.T) {
	t.Parallel()
...
func TestTFDependencyEmptyConfigPath_ReportsError(t *testing.T) {
	t.Parallel()
	...
	_, stderr, runErr := helpers.RunTerragruntCommandWithOutput(
		t,
		"terragrunt plan --non-interactive --working-dir "+consumerPath,
	)
...
func TestTFSensitiveValues(t *testing.T) {
	t.Parallel()
	...
	assert.Equal(
		t,
		"number",
		outputs["password_length"].Type,
		"password_length should be of type number",
	)
...
// TestTFDisabledDependencyEmptyConfigPath_NoCycleError tests that disabled dependencies with empty
...
func TestTFDisabledDependencyEmptyConfigPath_NoCycleError(t *testing.T) {
	t.Parallel()
...
// TestTFOutputFlushOnInterrupt verifies that buffered output is flushed when context is cancelled.
func TestTFOutputFlushOnInterrupt(t *testing.T) {
	...
	case <-firstWrite:
		...
		t.Logf(
			"First write detected (%d bytes, %d writes), cancelling context",
			len(outputBeforeCancel),
			writesBeforeCancel,
		)
		cancel()
	case <-cmdErr:
	...
	t.Logf(
		"Output length: before cancel=%d, after cancel=%d, final=%d",
		len(outputBeforeCancel),
		len(outputAfterCancel),
		len(output),
	)
	t.Logf(
		"Total writes: before cancel=%d, after cancel=%d, final=%d",
		writesBeforeCancel,
		writesAfterCancel,
		totalWrites,
	)
	...
	require.Greater(
		...
		len(outputAfterCancel),
		len(outputBeforeCancel),
		"Output should increase after cancellation due to flush",
	)
	require.Greater(
		t,
		totalWrites,
		writesBeforeCancel,
		"Additional writes should occur after cancellation (flush writes)",
	)
	require.NotEmpty(t, output, "Expected output to be flushed after cancellation")
}

// TestTFRunAllDoesNotIncludeExternalDepsInQueue tests that running `terragrunt run --all` from a subdirectory
...
func TestTFRunAllDoesNotIncludeExternalDepsInQueue(t *testing.T) {
	t.Parallel()
...
// TestTFRunAllFromParentDiscoversAllModules verifies that running from the parent directory
// correctly discovers all modules in the hierarchy. This is the control test for
// TestRunAllDoesNotEscapeWorkingDir.
func TestTFRunAllFromParentDiscoversAllModules(t *testing.T) {
	t.Parallel()
...
func TestTFNotExistingDependency(t *testing.T) {
	t.Parallel()
	...
	_, _, err := helpers.RunTerragruntCommandWithOutput(
		t,
		"terragrunt apply --non-interactive --working-dir "+invalidPath,
	)
	require.Error(t, err)
	...
	_, _, err = helpers.RunTerragruntCommandWithOutput(
		t,
		"terragrunt apply --non-interactive --working-dir "+parentFindFail,
	)
	require.Error(t, err)
	...
	assert.Contains(
		t,
		err.Error(),
		"Error in function call; Call to function \"find_in_parent_folders\" failed: ParentFileNotFoundError: Could not find a wrong-dir-name",
	)
}

// TestTFDependencyIncludeError tests that include directives for configs with dependency blocks
...
func TestTFDependencyIncludeError(t *testing.T) {
	t.Parallel()
...
// TestTFReadTerragruntConfigDependencyInStack tests that read_terragrunt_config()
...
func TestTFReadTerragruntConfigDependencyInStack(t *testing.T) {
	t.Parallel()
...
// TestTFReadTerragruntConfigDependencyInStackWithRacing exercises the same scenario
// as TestTFReadTerragruntConfigDependencyInStack under the race detector.
...
func TestTFReadTerragruntConfigDependencyInStackWithRacing(t *testing.T) {
	t.Parallel()
...
// TestTFChainedDepsExposedIncludeNoErrorLog verifies that chaining dependencies with exposed
...
func TestTFChainedDepsExposedIncludeNoErrorLog(t *testing.T) {
	t.Parallel()
	...
	helpers.RunTerragrunt(
		t,
		"terragrunt run --non-interactive --working-dir "+ancestorPath+" -- apply -auto-approve",
	)
	...
	helpers.RunTerragrunt(
		t,
		"terragrunt run --non-interactive --working-dir "+depPath+" -- apply -auto-approve",
	)
...
// TestTFExposedIncludeErrorIdentifiesIncludeBlock is a regression test for
...
func TestTFExposedIncludeErrorIdentifiesIncludeBlock(t *testing.T) {
	t.Parallel()
...
func TestTFQueueDisplayOrder(t *testing.T) {
	...
	t.Run("up", func(t *testing.T) {
		...
		_, stderr, err := helpers.RunTerragruntCommandWithOutput(
			t,
			"terragrunt run --all --non-interactive --no-color --working-dir "+rootPath+" -- plan",
		)
		require.NoError(t, err)
	...
	t.Run("down", func(t *testing.T) {
		...
		_, stderr, err := helpers.RunTerragruntCommandWithOutput(
			t,
			"terragrunt run --all --non-interactive --no-color --working-dir "+rootPath+" -- plan -destroy",
		)
		require.NoError(t, err)
...
// TestTFDAGQueueDisplay verifies that the run queue is displayed as a DAG tree
// showing dependency hierarchy, and that the header message differs for
// up (plan) vs down (plan -destroy) commands.
// Regression test for https://github.com/gruntwork-io/terragrunt/issues/5035
func TestTFDAGQueueDisplay(t *testing.T) {
	t.Parallel()
...
// TestTFForgedModuleManifestDoesNotEscapeCache pins that a forged .terragrunt-module-manifest shipped inside a downloaded git module does not delete a sentinel file outside the cache.
func TestTFForgedModuleManifestDoesNotEscapeCache(t *testing.T) {
	t.Parallel()
	...
	require.NoError(
		...
		srv.CommitFile(
			...
			encodeForgedManifest(t, forgedFile(sentinel)),
			"forged manifest",
		),
	)
	...
	require.NoError(
		t,
		os.WriteFile(filepath.Join(consumer, "terragrunt.hcl"), []byte(tgConfig), 0o644),
	)
	...
	require.NotEmpty(
		t,
		cachedMain,
		"the benign main.tf from the module must be present in the cache",
	)
	...
	require.NoError(t, os.WriteFile(
		forgedManifestPath,
		encodeForgedManifest(
			t,
			forgedFile(staleCanary),
			forgedFile(sentinel),
			forgedDir(sentinelDir),
		),
		0o600,
	))
	require.FileExists(
		t,
		forgedManifestPath,
		"test setup must plant the forged cache manifest before the second init",
	)
	...
	assert.NoFileExists(
		t,
		staleCanary,
		"forged manifest must have been processed and removed the in-cache canary",
	)
	assert.FileExists(
		t,
		sentinel,
		"forged manifest entry must not have deleted the sentinel outside the cache",
	)
	...
	assert.NotContains(
		...
		manifestPaths,
		staleCanary,
		"fresh manifest should not retain the forged in-cache canary entry",
	)
	assert.NotContains(
		t,
		manifestPaths,
		sentinel,
		"fresh manifest should not retain the forged out-of-cache file entry",
	)
	assert.NotContains(
		t,
		manifestPaths,
		sentinelDir,
		"fresh manifest should not retain the forged out-of-cache directory entry",
	)
}

// TestTFDependencyExtraArgsEnvVarsResolveOutput checks that dependency output resolution honors extra_arguments env_vars
func TestTFDependencyExtraArgsEnvVarsResolveOutput(t *testing.T) {
	t.Parallel()
	...
	_, _, err := helpers.RunTerragruntCommandWithOutput(
		t,
		"terragrunt apply -auto-approve --non-interactive --working-dir "+moduleAPath,
	)
	require.NoError(t, err)
	...
	_, _, err = helpers.RunTerragruntCommandWithOutput(
		t,
		"terragrunt apply -auto-approve --non-interactive --working-dir "+moduleBPath,
	)
	require.NoError(t, err)
	...
	stdout, _, err := helpers.RunTerragruntCommandWithOutput(
		t,
		"terragrunt output -raw test_output --non-interactive --working-dir "+moduleBPath,
	)
	require.NoError(t, err)
	assert.Contains(t, stdout, "hello from module-a")
}

// TestTFDependencyHookOutputResolution checks a unit can resolve outputs of a dependency whose before_hook, after_hook, and error_hook reference its own dependency.
func TestTFDependencyHookOutputResolution(t *testing.T) {
	t.Parallel()
...
// TestTFDependencyExtraArgsOutputResolution checks a unit can resolve outputs of a dependency whose extra_arguments env_vars and arguments reference its own dependency.
func TestTFDependencyExtraArgsOutputResolution(t *testing.T) {
	t.Parallel()
...
// TestTFDependencyRemoteStateOutputResolution checks a unit can resolve outputs of a dependency whose remote_state config references its own dependency.
func TestTFDependencyRemoteStateOutputResolution(t *testing.T) {
	t.Parallel()
...
func TestTFDependencyGenuineErrorSurfaces(t *testing.T) {
	t.Parallel()
	...
	stdout, stderr, err := helpers.RunTerragruntCommandWithOutput(
		t,
		"terragrunt run --non-interactive --working-dir "+moduleBPath+" -- plan",
	)
	require.Error(t, err)
	assert.Contains(t, stdout+stderr, "nonexistent_function_xyz")
}

// TestTFDependencyOutputLocalOptimization checks that a unit can resolve outputs of a dependency that manages its state
...
// full-run fallback.
func TestTFDependencyOutputLocalOptimization(t *testing.T) {
	t.Parallel()
	...
	stdout, _, err := helpers.RunTerragruntCommandWithOutput(
		t,
		"terragrunt run --non-interactive --working-dir "+moduleBPath+" -- output -raw echo",
	)
	require.NoError(t, err)
	assert.Contains(t, stdout, "argocd")
}
