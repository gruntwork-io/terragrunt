...
func TestTFTerragruntProviderCacheWithFilesystemMirror(t *testing.T) {
	...
	expectedProviderInstallation = fmt.Sprintf(
		strings.Join(strings.Fields(expectedProviderInstallation), " "),
		providersMirrorPath,
		providerCacheDir,
	)
	...
	require.NoError(
		...
		util.DoWithRetry(
			t.Context(),
			"Run terragrunt init with provider cache",
			3,
			0,
			logger.CreateLogger(),
			log.DebugLevel,
			func(ctx context.Context) error {
				...
				terraformrcBytes, readErr := os.ReadFile(
					filepath.Join(cacheWorkingDir, ".terraformrc"),
				)
				if readErr != nil {
...
func TestTFTerragruntProviderCacheWithNetworkMirror(t *testing.T) {
	// In this test we use os.Setenv to set the Terraform env var TF_CLI_CONFIG_FILE.
	...
	networkMirrorURL := runNetworkMirrorServer(
		t,
		ctx,
		"/providers/",
		providersNetkworMirrorPath,
		token,
	)
...
// TestTFTerragruntProviderCacheWithAbsoluteModuleURL tests that the provider cache correctly handles
...
func TestTFTerragruntProviderCacheWithAbsoluteModuleURL(t *testing.T) {
	rootPath := t.TempDir()
	appPath := filepath.Join(rootPath, "app")
	providerCacheDir := filepath.Join(rootPath, "providers-cache")
...
func TestTFExtraArguments(t *testing.T) {
	out := new(bytes.Buffer)
	helpers.RunTerragruntRedirectOutput(
		t,
		"terragrunt apply -auto-approve --non-interactive --tf-forward-stdout --working-dir "+testFixtureExtraArgsPath,
		out,
		os.Stderr,
	)
	t.Log(out.String())
...
func TestTFExtraArgumentsWithEnv(t *testing.T) {
	out := new(bytes.Buffer)
...
func TestTFExtraArgumentsWithEnvVarBlock(t *testing.T) {
	out := new(bytes.Buffer)
	helpers.RunTerragruntRedirectOutput(
		t,
		"terragrunt apply -auto-approve --non-interactive --tf-forward-stdout --working-dir "+testFixtureEnvVarsBlockPath,
		out,
		os.Stderr,
	)
	t.Log(out.String())
...
func TestTFExtraArgumentsWithRegion(t *testing.T) {
	out := new(bytes.Buffer)
...
func TestTFPreserveEnvVarApplyAll(t *testing.T) {
	...
	for _, mod := range []string{assertEnvVarModule, noRequireEnvVarModule} {
		...
		err := helpers.RunTerragruntCommand(
			t,
			"terragrunt output text -no-color --non-interactive --working-dir "+mod,
			&stdout,
			os.Stderr,
		)
		require.NoError(t, err)
...
func TestTFPriorityOrderOfArgument(t *testing.T) {
	out := new(bytes.Buffer)
	injectedValue := "Injected-directly-by-argument"
	helpers.RunTerragruntRedirectOutput(
		t,
		fmt.Sprintf(
			"terragrunt apply -auto-approve -var extra_var=%s --non-interactive --tf-forward-stdout --working-dir %s",
			injectedValue,
...
func TestTFTerragruntSourceMapEnvArg(t *testing.T) {
...
func TestTFTerragruntLogLevelEnvVarOverridesDefault(t *testing.T) {
...
func TestTFTerragruntProduceTelemetryTraces(t *testing.T) {
...
func TestTFTerragruntProduceTelemetryMetrics(t *testing.T) {
	if helpers.IsWindows() {
		t.Skip("Skipping test on Windows since bash script execution is not supported")
	}
	...
	output, _, err := helpers.RunTerragruntCommandWithOutput(
		t,
		"terragrunt apply -no-color -auto-approve --non-interactive --working-dir "+rootPath,
	)
	require.NoError(t, err)
...
func TestTFTerragruntProduceTelemetryTracesWithRootSpanAndTraceID(t *testing.T) {
	if helpers.IsWindows() {
		t.Skip("Skipping test on Windows since bash script execution is not supported")
	}
...
func TestTFTerragruntProviderCache(t *testing.T) {
	...
	for subDir, providers := range providers {
		...
		for _, entry := range entries {
			...
			require.NotEmpty(
				t,
				cacheWorkingDir,
				"Should find cache working directory for %s",
				appPath,
			)
...
// TestTFTerragruntProviderCacheWithDependency verifies that the provider cache server
...
func TestTFTerragruntProviderCacheWithDependency(t *testing.T) {
	...
	for _, provider := range []string{
		...
		assert.DirExists(
			t,
			providerPath,
			"Provider cache dir should contain %s at %s",
			provider,
			providerPath,
		)
	}
...
func TestTFParseTFLog(t *testing.T) {
	...
	for _, prefixName := range []string{"app", "dep"} {
		assert.Contains(
			t,
			stderr,
			"INFO   ["+prefixName+"] "+wrappedBinary(t.Context())+`: TF_LOG: Go runtime version`,
		)
	}
}

func TestTFTerragruntTelemetryPassTraceParent(t *testing.T) {
	t.Setenv("TG_TELEMETRY_TRACE_EXPORTER", "console")
...
func TestTFTerragruntTelemetryPassTraceParentEnvVariable(t *testing.T) {
...
func TestTFRunnerPoolTelemetry(t *testing.T) {
	t.Setenv("TG_TELEMETRY_TRACE_EXPORTER", "console")
	...
	telemetryOutput, _, err := helpers.RunTerragruntCommandWithOutput(
		t,
...
func TestTFVersionIsInvokedInDifferentDirectory(t *testing.T) {
	...
	versionCmdPattern := regexp.MustCompile(
		`Running command: ` + regexp.QuoteMeta(wrappedBinary(t.Context())) + ` -version`,
	)
	...
	assert.Contains(
		t,
		stderr,
		"prefix=dependency-with-custom-version msg=Running command: "+wrappedBinary(
			t.Context(),
		)+" -version",
	)
}

func TestTFVersionIsInvokedOnlyOnce(t *testing.T) {
