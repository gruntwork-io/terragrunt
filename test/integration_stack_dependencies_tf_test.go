...
// TestTFStackDepsNestedStackOutputs pins, end to end, that a dependency on a stack reads the outputs
// of a unit generated by the stack's nested stack, addressed under the nested stack's name.
func TestTFStackDepsNestedStackOutputs(t *testing.T) {
	t.Parallel()

	helpers.CleanupTerraformFolder(t, testFixtureStackDepsNestedStackOutputs)
	tmpEnvPath := helpers.CopyEnvironment(t, testFixtureStackDepsNestedStackOutputs)
	gitPath := filepath.Join(tmpEnvPath, testFixtureStackDepsNestedStackOutputs)
	...
	stdout, stderr, err := helpers.RunTerragruntCommandWithOutput(
		t,
		"terragrunt output -raw received --non-interactive --working-dir "+appDir,
	)
