...
func TestTflintFindsNoIssuesWithValidCode(t *testing.T) {
	...
	assert.NotContains(
		t,
		errOut.String(),
		"Tflint found issues in the project. Check for the tflint logs above.",
	)
	...
	assert.NotContains(
		t,
		errOut.String(),
		"--config ./.terragrunt-cache",
		"TFLint config should not be inside cache directory",
	)
}
...
func TestTflintInitSameModule(t *testing.T) {
	...
	helpers.RunTerragrunt(
		t,
		"terragrunt run --log-level debug --all init --non-interactive --working-dir "+runPath,
	)
}
...
func TestTflintFindsNoIssuesWithValidCodeDifferentDownloadDir(t *testing.T) {
	...
	err := helpers.RunTerragruntCommand(
		t,
		fmt.Sprintf(
			"terragrunt plan --log-level debug --working-dir %s --download-dir %s",
			modulePath,
			downloadDir,
		),
		out,
	...
	assert.NotContains(
		t,
		errOut.String(),
		fmt.Sprintf("--config %s/", relPath),
		"TFLint config should not be inside download directory",
	)
}
...
func TestTflintExternalTflint(t *testing.T) {
	...
	err := helpers.RunTerragruntCommand(
		t,
		"terragrunt plan --log-level debug --working-dir "+runPath,
		out,
		errOut,
	)
	require.NoError(t, err)
