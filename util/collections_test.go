...
func TestRemoveDuplicatesFromList(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		assert.Equal(t, testCase.expected, f(testCase.list), "For list %v", testCase.list)
...
func TestCommaSeparatedStrings(t *testing.T) {
	...
	for _, testCase := range testCases {
		assert.Equal(t, testCase.expected, CommaSeparatedStrings(testCase.list), "For list %v", testCase.list)
		t.Logf("%v passed", testCase.list)
	}
}
