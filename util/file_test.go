...
func TestFolderPathAsFile(t *testing.T) {
	tests := []struct {
		name     string
		input    string
		expected string
		...
		{
			name:     "Mix of valid and invalid characters",
			input:    "example:/path<to>file?",
			expected: "example--path-to-file-",
		},
		{
			name:     "Leading invalid characters",
			input:    "><|example",
			expected: "example",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := FolderPathAsFile(tt.input); got != tt.expected {
				t.Errorf("FolderPathAsFile(%q) = %q, want %q", tt.input, got, tt.expected)
			}
		})
	}
}
