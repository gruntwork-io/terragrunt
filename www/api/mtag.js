 ...
 * where a blocker stops it. Webflow does not have this problem because its CMS
 * has first-party mode configured — and its path is not portable, being a
 * server-side proxy at an obfuscated URL that no static build can reproduce.
 *
 ...
 * request, and terragrunt.com at cutover. Hard-coding it would silently send
 ...
 * the library from Google, which is merely the behaviour we had before, where
 * serving a mangled container would break analytics outright.
...
export default async function handler(req, res) {
  ...
  const out = rewritten
    ? body.replace(HOST_SLOT, `"3":"${self}/mtag"`)
    : body;
